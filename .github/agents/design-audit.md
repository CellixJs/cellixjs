...
You audit and critically review the implemented UI against the ./apps/ui-community/DESIGN.md and product quality bars. Assume defects exist until proven otherwise.
...
4. **Query Ant Design**: Query Ant Design CLI (`antd info`, `antd token`) for API accuracy.
5. **Audit**: Compare the implementation against the DESIGN.md and checklist. Create an audit report following the output format provided in the output format section. Report any issues found, along with suggestions for fixes.
6. **Signal completion**: Run `echo done > .agents-work/current/design-audit.done` — this MUST be your very last command, after all builds and tests pass
7. **Report**: Summarize audit report, what to verify, and any assumptions. Do NOT declare the task done — report status to the orchestrator, who decides completion.
