/**
 * Rolldown bundler script for @ocom/api deployment artifact.
 *
 * This script takes the TypeScript-compiled output (dist/src/index.js) as its
 * entry point and bundles it — including all workspace package dependencies — into
 * a single optimised ESM file at deploy/dist/index.js. The deploy/ directory is
 * the artifact used both for local Azure Functions development and CI/CD deployment.
 *
 * Usage: node apps/api/scripts/build-deploy.mjs
 * Prerequisites: tsc --build must have been run first.
 */
...
async function ensureCompiledEntryExists() {
	await fs.access(distEntryFile).catch(() => {
		throw new Error(
			`Expected compiled entrypoint at ${distEntryFile}. Run the TypeScript build before packaging.`,
		);
	});
}
...
async function resolvePreferredBundleEntry(packageName) {
	...
	const exportTarget =
		resolveExportTarget(exportsEntry?.require) ??
...
async function resolveInstalledPackageRoot(packageName) {
	try {
		...
		for (const pnpmEntry of pnpmEntries.sort((a, b) => a.localeCompare(b))) {
			...
			const exists = await fs
				.access(candidatePackageJsonPath)
				.then(() => true)
				.catch(() => false);
			if (!exists) {
				continue;
			...
			const candidatePackageJson = JSON.parse(
				await fs.readFile(candidatePackageJsonPath, 'utf8'),
			);
