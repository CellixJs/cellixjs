...
3. Creates a system-scoped application services instance (via `ApplicationServicesFactory.forSystem`), scoped to only the `canManageCommunitySettings` permission this handler needs.
...
- `@ocom/application-services` — application services factory providing request-scoped services (HTTP) and system-scoped services (background/queue-triggered work, e.g. via `forSystem`).
