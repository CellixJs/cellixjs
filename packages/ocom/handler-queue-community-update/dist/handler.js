 ...
 */
export const communityUpdateQueueHandlerCreator = (applicationServicesFactory, queueStorageService) => {
...
function buildTriggeredQueueMessage(message, invocationContext) {
...
function pickString(invocationContext, keys) {
    const metadata = invocationContext.triggerMetadata;
    if (!metadata) {
        return undefined;
    }
...
function pickNumber(invocationContext, keys) {
    const metadata = invocationContext.triggerMetadata;
    if (!metadata) {
        return undefined;
    }
