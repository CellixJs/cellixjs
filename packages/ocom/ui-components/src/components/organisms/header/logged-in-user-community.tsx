...
export const LoggedInUserCommunity: React.FC<LoggedInUserCommunityProps> = (props) => {
	const userData: LoggedInUserProps = {
		data: {
			...
			profileImage: props.data?.memberForCurrentUser?.profile?.avatarDocumentId ? `https://ownercommunity.blob.core.windows.net/${props.data?.communityId}/${props.data?.memberForCurrentUser.profile.avatarDocumentId}` : undefined,
		},
	};
	...
	return (
		...
		>
			<LoggedInUser
				key={props.data?.userCurrent?.id}
				data={userData.data}
				onLogoutClicked={props.handleLogout}
			/>
		</div>
	);
};
