...
const feature = await loadFeature(path.resolve(__dirname, 'features/verified-token-service.feature'));
...
test.for(feature, ({ Scenario, BeforeEachScenario }) => {
	...
	Scenario('Verifying a valid JWT token', ({ Given, When, Then, And }) => {
		...
		Then('it should return the verified JWT payload with expected claims', () => {
			expect(vi.mocked(jwtVerify)).toHaveBeenCalledWith('valid.jwt.token', expect.any(Function), {
				audience: 'audience1',
				clockTolerance: '5 minutes',
