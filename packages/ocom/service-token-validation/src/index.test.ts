...
test.for(feature, ({ Scenario, BeforeEachScenario }) => {
	...
	let mockGetVerifiedJwt: Mock<(bearerToken: string, configKey: string) => Promise<Awaited<ReturnType<VerifiedTokenService['getVerifiedJwt']>> | null>>;
	...
	Scenario('Constructing ServiceTokenValidation with missing environment variables', ({ Given, When, Then }) => {
		...
		When('the ServiceTokenValidation is constructed', () => {
			const portalTokens = new Map([['portal1', 'MISSING']]);
