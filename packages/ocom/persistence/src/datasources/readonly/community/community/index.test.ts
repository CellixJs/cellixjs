...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	Scenario(
		'Creating Community Read Repository Implementation',
		({ When, Then, And }) => {
			When(
				'I call CommunityReadRepositoryImpl with models and passport',
				() => {
			...
			And(
				'the CommunityReadRepositoryImpl should have CommunityReadRepo property',
				() => {
			...
			And(
				'the CommunityReadRepo should be a CommunityReadRepository instance',
				() => {
					...
					expect(typeof result.CommunityReadRepo.getByIdWithCreatedBy).toBe(
						'function',
					);
					expect(typeof result.CommunityReadRepo.getByEndUserExternalId).toBe(
						'function',
					);
				},
			);
