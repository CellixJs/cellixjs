...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	Scenario(
		'Creating Member Read Repository Implementation',
		({ When, Then, And }) => {
			...
			And(
				'the MemberReadRepo should be a MemberReadRepository instance',
				() => {
			...
			And('the MemberReadRepo should have all required methods', () => {
				...
				expect(
					typeof result.MemberReadRepo.getMembersForEndUserExternalId,
				).toBe('function');
