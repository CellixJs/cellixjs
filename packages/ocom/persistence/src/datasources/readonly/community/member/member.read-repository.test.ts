...
test.for(feature, ({ Scenario, BeforeEachScenario }) => {
	...
	BeforeEachScenario(() => {
		...
		mockDataSource = {
			...
			findById: vi.fn(async (id: string) =>
				id === 'member-123' ? mockMemberDoc : null,
			),
		...
		vi.mocked(MemberDataSourceImpl).mockImplementation(
			() =>
				mockDataSource as unknown as InstanceType<typeof MemberDataSourceImpl>,
		);
		vi.mocked(MemberConverter).mockImplementation(
			() => mockConverter as unknown as MemberConverter,
		);
	...
	Scenario('Getting members by community ID', ({ When, Then }) => {
		When(
			'I call getByCommunityId with "507f1f77bcf86cd799439011"',
			async () => {
	...
	Scenario('Getting member by ID with role', ({ Given, When, Then }) => {
		...
		Then(
			'I should receive the MemberEntityReference object with role populated',
			() => {
	...
	Scenario(
		'Getting members for end user external ID',
		({ Given, When, Then }) => {
			...
			When(
				'I call getMembersForEndUserExternalId with "user-456"',
				async () => {
			...
			Then(
				'I should receive an array of MemberEntityReference objects for that user',
				() => {
