...
test.for(feature, ({ Scenario, BeforeEachScenario }) => {
	...
	Scenario(
		'Creating Mongo Data Source Implementation',
		({ When, Then, And }) => {
	...
	Scenario('Finding documents with filter', ({ Given, When, Then }) => {
		...
		Then(
			'I should receive an array of documents with id field appended',
			async () => {
	...
	Scenario('Finding documents with options', ({ Given, When, Then }) => {
		...
		When(
			'I call find with filter and options including limit, skip, and sort',
			async () => {
	...
	Scenario(
		'Finding document by ID with population',
		({ Given, When, Then }) => {
			Given('a Mongoose model with findById method', () => {
				vi.mocked(model.findById).mockReturnValue({
					populate: vi.fn().mockReturnThis(),
			...
			When(
				'I call findById with valid ID and populateFields option',
				async () => {
					await dataSource.findById('507f1f77bcf86cd799439011', {
	...
	Scenario('Aggregating documents', ({ Given, When, Then }) => {
		...
		Then(
			'I should receive aggregated results with id field appended',
			async () => {
