...
test.for(feature, ({ Scenario, BeforeEachScenario }) => {
	...
	Scenario('Finding documents with options', ({ Given, When, Then }) => {
		...
		When('I call find with filter and options including limit, skip, and sort', async () => {
			await dataSource.find({ value: { $gt: 20 } } as unknown as Partial<TestDocument>, {
				limit: 10,
				skip: 5,
				sort: { createdAt: -1 },
			});
		...
		Then('I should receive filtered and sorted documents', async () => {
			const result = await dataSource.find({ value: { $gt: 20 } } as unknown as Partial<TestDocument>, {
				limit: 10,
				skip: 5,
				sort: { createdAt: -1 },
			});
	...
	Scenario('Finding documents with projection', ({ Given, When, Then }) => {
		...
		Then('I should receive documents with specified fields only', async () => {
			...
			expect(result).toHaveLength(1);
			expect(model.find).toHaveBeenCalledWith({ name: 'test-doc' }, { name: 1, value: 1 }, {});
		});
	});
	...
	Scenario('Finding one document with population', ({ Given, When, Then }) => {
		...
		When('I call findOne with filter and populateFields option', async () => {
			await dataSource.findOne({ name: 'test-doc' }, { populateFields: ['author', 'comments'] });
		});

		Then('I should receive a document with populated fields', async () => {
			const result = await dataSource.findOne({ name: 'test-doc' }, { populateFields: ['author', 'comments'] });
			expect(result).toEqual({
				...mockLeanDoc,
				id: mockLeanDoc._id,
			});
	...
	Scenario('Aggregating documents', ({ Given, When, Then }) => {
		...
		When('I call aggregate with a pipeline', async () => {
			await dataSource.aggregate([{ $match: { value: { $gt: 20 } } }, { $sort: { createdAt: -1 } }]);
		});

		Then('I should receive aggregated results with id field appended', async () => {
			const result = await dataSource.aggregate([{ $match: { value: { $gt: 20 } } }, { $sort: { createdAt: -1 } }]);
			expect(result).toHaveLength(1);
			expect(result[0]).toEqual({
				...mockLeanDoc,
				id: mockLeanDoc._id,
			});
			expect(model.aggregate).toHaveBeenCalledWith([{ $match: { value: { $gt: 20 } } }, { $sort: { createdAt: -1 } }]);
		});
	});
});
