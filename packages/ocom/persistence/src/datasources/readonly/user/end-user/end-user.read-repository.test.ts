...
test.for(feature, ({ Scenario, BeforeEachScenario }) => {
	...
	BeforeEachScenario(() => {
		...
		mockDataSource = {
			...
			findById: vi.fn(async (id: string) =>
				id === 'test-id' ? mockEndUserDoc : null,
			),
			findOne: vi.fn(async (filter: Partial<EndUser>) =>
				filter.externalId === 'ext-123' ? mockEndUserDoc : null,
			),
		...
		vi.mocked(EndUserDataSourceImpl).mockImplementation(
			() =>
				mockDataSource as unknown as InstanceType<typeof EndUserDataSourceImpl>,
		);
		vi.mocked(EndUserConverter).mockImplementation(
			() => mockConverter as unknown as EndUserConverter,
		);
	...
	Scenario('Creating End User Read Repository', ({ When, Then, And }) => {
		When(
			'I create a EndUserReadRepositoryImpl with models and passport',
			() => {
	...
	Scenario('Getting end user by ID when exists', ({ Given, When, Then }) => {
		...
		Then('I should receive the EndUserEntityReference object', () => {
			...
			expect(mockConverter.toDomain).toHaveBeenCalledWith(
				mockEndUserDoc,
				passport,
			);
		});
	...
	Scenario(
		'Getting end user by ID when not exists',
		({ Given, When, Then }) => {
	...
	Scenario(
		'Getting end user by external ID when exists',
		({ Given, When, Then }) => {
	...
	Scenario(
		'Getting end user by external ID when not exists',
		({ Given, When, Then }) => {
