...
import type { ServiceTicketModelType } from '@ocom/data-sources-mongoose-models/community';
...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	BeforeEachScenario(() => {
		...
		result =
			{} as Domain.Contexts.Case.ServiceTicket.V1.ServiceTicketV1UnitOfWork;
	});
	...
	Scenario('Creating a ServiceTicketV1 Unit of Work', ({ When, Then, And }) => {
		When(
			'I call getServiceTicketV1UnitOfWork with the ServiceTicket model and passport',
			() => {
		...
		Then(
			'I should receive a properly initialized ServiceTicketV1UnitOfWork',
			() => {
