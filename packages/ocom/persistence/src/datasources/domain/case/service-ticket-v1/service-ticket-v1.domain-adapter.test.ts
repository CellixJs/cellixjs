...
function makeActivityDetailDoc(
	overrides: Partial<ServiceTicketActivityDetail> = {},
) {
	return {
...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	Scenario(
		'Getting and setting the title property',
		({ Given, When, Then }) => {
	...
	Scenario(
		'Getting and setting the description property',
		({ Given, When, Then }) => {
			Given('a ServiceTicketV1DomainAdapter for the document', () => {
				// Already set up
			});
	...
	Scenario(
		'Getting and setting the status property',
		({ Given, When, Then }) => {
			Given('a ServiceTicketV1DomainAdapter for the document', () => {
				// Already set up
			});
	...
	Scenario(
		'Getting and setting the priority property',
		({ Given, When, Then }) => {
			Given('a ServiceTicketV1DomainAdapter for the document', () => {
				// Already set up
			});
	...
	Scenario(
		'Getting the communityId property when populated',
		({ Given, When, Then }) => {
			Given(
				'a ServiceTicketV1DomainAdapter for the document with populated community',
				() => {
	...
	Scenario(
		'Getting the communityId property when not populated',
		({ Given, When, Then }) => {
			Given(
				'a ServiceTicketV1DomainAdapter for a document with community as an ObjectId',
				() => {
					doc.community = new MongooseSeedwork.ObjectId(
			...
			Then(
				'an error should be thrown indicating "community is not populated"',
				() => {
					expect(() => adapter.communityId).toThrow(
	...
	Scenario('Setting the communityId property', ({ Given, When, Then }) => {
		...
		Then(
			'the document\'s community should be set to the ObjectId "507f1f77bcf86cd799439012"',
			() => {
				expect(doc.community).toEqual(
					new MongooseSeedwork.ObjectId('507f1f77bcf86cd799439012'),
				);
			},
		);
	});
	...
	Scenario(
		'Getting the requestorId property when populated',
		({ Given, When, Then }) => {
			Given(
				'a ServiceTicketV1DomainAdapter for the document with populated requestor',
				() => {
	...
	Scenario(
		'Getting the requestorId property when not populated',
		({ Given, When, Then }) => {
			Given(
				'a ServiceTicketV1DomainAdapter for a document with requestor as an ObjectId',
				() => {
					doc.requestor = new MongooseSeedwork.ObjectId(
			...
			Then(
				'an error should be thrown indicating "requestor is not populated"',
				() => {
					expect(() => adapter.requestorId).toThrow(
						'requestor is not populated',
					);
				},
			);
		},
	...
	Scenario('Setting the requestorId property', ({ Given, When, Then }) => {
		...
		Then(
			'the document\'s requestor should be set to the ObjectId "507f1f77bcf86cd799439013"',
			() => {
				expect(doc.requestor).toEqual(
					new MongooseSeedwork.ObjectId('507f1f77bcf86cd799439013'),
				);
			},
		);
	});
	...
	Scenario(
		'Getting and setting the propertyId property',
		({ Given, When, Then }) => {
			Given('a ServiceTicketV1DomainAdapter for the document', () => {
				// Already set up
			});
			...
			When(
				'I set the propertyId property to "507f1f77bcf86cd799439014"',
				() => {
					adapter.propertyId = '507f1f77bcf86cd799439014';
				},
			);
			Then(
				'the document\'s property should be set to the ObjectId "507f1f77bcf86cd799439014"',
				() => {
					expect(doc.property).toEqual(
						new MongooseSeedwork.ObjectId('507f1f77bcf86cd799439014'),
					);
				},
			);
		},
	...
	Scenario(
		'Getting and setting the assignedToId property',
		({ Given, When, Then }) => {
			Given('a ServiceTicketV1DomainAdapter for the document', () => {
				// Already set up
			});
			...
			When(
				'I set the assignedToId property to "507f1f77bcf86cd799439015"',
				() => {
					adapter.assignedToId = '507f1f77bcf86cd799439015';
				},
			);
			Then(
				'the document\'s assignedTo should be set to the ObjectId "507f1f77bcf86cd799439015"',
				() => {
					expect(doc.assignedTo).toEqual(
						new MongooseSeedwork.ObjectId('507f1f77bcf86cd799439015'),
					);
				},
			);
		},
	...
	Scenario(
		'Getting and setting the serviceId property',
		({ Given, When, Then }) => {
			Given('a ServiceTicketV1DomainAdapter for the document', () => {
				// Already set up
			});
			...
			Then(
				'the document\'s service should be set to the ObjectId "507f1f77bcf86cd799439016"',
				() => {
					expect(doc.service).toEqual(
						new MongooseSeedwork.ObjectId('507f1f77bcf86cd799439016'),
					);
				},
			);
		},
	...
	Scenario('Getting the activityLog property', ({ Given, When, Then }) => {
		...
		Then(
			'it should return a MongoosePropArray of ServiceTicketV1ActivityDetail',
			() => {
				expect(adapter.activityLog).toBeDefined();
				expect(adapter.activityLog).toBeInstanceOf(
					MongooseSeedwork.MongoosePropArray,
				);
			},
		);
	});
	...
	Scenario('Getting the messages property', ({ Given, When, Then }) => {
		...
		Then(
			'it should return a MongoosePropArray of ServiceTicketV1Message',
			() => {
				expect(adapter.messages).toBeDefined();
				expect(adapter.messages).toBeInstanceOf(
					MongooseSeedwork.MongoosePropArray,
				);
			},
		);
	...
	Scenario(
		...
		({ Given, When, Then }) => {
			...
			Then(
				"the document's updateIndexFailedDate should be set to that Date",
				() => {
	...
	Scenario(
		'Getting activity detail properties',
		({ Given, When, Then, And }) => {
			let activityDetail: Domain.Contexts.Case.ServiceTicket.V1.ServiceTicketV1ActivityDetailProps;

			Given(
				'a ServiceTicketV1ActivityDetailDomainAdapter for a document',
				() => {
					const activityDoc = makeActivityDetailDoc();
					const adapter = new ServiceTicketV1ActivityDetailDomainAdapter(
						activityDoc,
					);
	...
	Scenario(
		'Loading activityBy when already populated',
		({ Given, When, Then }) => {
			let activityDoc: ServiceTicketActivityDetail;
			let result: Domain.Contexts.Community.Member.MemberEntityReference;

			Given(
				'a ServiceTicketV1ActivityDetailDomainAdapter for a document with populated activityBy',
				() => {
			...
			Then('it should return a Member entity reference', () => {
				const memberRef =
					result as Domain.Contexts.Community.Member.MemberEntityReference;
	...
	Scenario('Loading activityBy when not populated', ({ Given, When, Then }) => {
		...
		Given(
			'a ServiceTicketV1ActivityDetailDomainAdapter for a document with activityBy as an ObjectId',
			() => {
				const memberDoc = makeMemberDoc();
				activityDoc = makeActivityDetailDoc({
					activityBy: new MongooseSeedwork.ObjectId('507f1f77bcf86cd799439013'),
				});
	...
	Scenario(
		'Loading initiatedBy when already populated',
		({ Given, When, Then }) => {
			let messageDoc: ServiceTicketMessage;
			let result: Domain.Contexts.Community.Member.MemberEntityReference;

			Given(
				'a ServiceTicketV1MessageDomainAdapter for a document with populated initiatedBy',
				() => {
					const memberDoc = makeMemberDoc();
	...
	Scenario(
		'Loading initiatedBy when not populated',
		({ Given, When, Then }) => {
			let messageDoc: ServiceTicketMessage;
			let result: Domain.Contexts.Community.Member.MemberEntityReference;

			Given(
				'a ServiceTicketV1MessageDomainAdapter for a document with initiatedBy as an ObjectId',
				() => {
					const memberDoc = makeMemberDoc();
					messageDoc = makeMessageDoc({
						initiatedBy: new MongooseSeedwork.ObjectId(
							'507f1f77bcf86cd799439013',
						),
					});
	...
	Scenario(
		'Getting the community property when not set',
		({ Given, When, Then }) => {
			Given(
				'a ServiceTicketV1DomainAdapter for the document without community',
				() => {
	...
	Scenario(
		'Getting the requestor property when populated',
		({ Given, When, Then }) => {
			Given(
				'a ServiceTicketV1DomainAdapter for the document with populated requestor',
				() => {
	...
	Scenario(
		'Getting the requestor property when not set',
		({ Given, When, Then }) => {
			Given(
				'a ServiceTicketV1DomainAdapter for the document without requestor',
				() => {
	...
	Scenario(
		'Loading the community when already populated',
		({ Given, When, Then }) => {
	...
	Scenario(
		'Loading the community when not populated',
		({ Given, When, Then }) => {
			let result: Domain.Contexts.Community.Community.CommunityEntityReference;
			Given(
				'a ServiceTicketV1DomainAdapter for the document with community as an ObjectId',
				() => {
					const communityDoc = makeCommunityDoc();
					doc = makeServiceTicketDoc({
	...
	Scenario(
		'Loading the requestor when already populated',
		({ Given, When, Then }) => {
	...
	Scenario(
		'Loading the requestor when not populated',
		({ Given, When, Then }) => {
			let result: Domain.Contexts.Community.Member.MemberProps;
			Given(
				'a ServiceTicketV1DomainAdapter for the document with requestor as an ObjectId',
				() => {
					const memberDoc = makeMemberDoc();
					doc = makeServiceTicketDoc({
						requestor: new MongooseSeedwork.ObjectId(
							'507f1f77bcf86cd799439013',
						),
					});
					vi.mocked(doc.populate).mockImplementation((path) => {
	...
	Scenario(
		'Setting the community property with a domain object',
		({ Given, When, Then }) => {
			...
			Given('a ServiceTicketV1DomainAdapter for the document', () => {
				...
				communityDomainObj = new Domain.Contexts.Community.Community.Community(
					communityAdapter,
					makeMockPassport(),
				);
				// Already set up
			...
			Then(
				"the document's community should be set to the domain object's document",
				() => {
	...
	Scenario(
		'Setting the community property with missing id',
		({ Given, When, Then }) => {
			let communityRef: Domain.Contexts.Community.Community.CommunityEntityReference;
			...
			Given('a ServiceTicketV1DomainAdapter for the document', () => {
				communityRef = {
					id: '',
				} as Domain.Contexts.Community.Community.CommunityEntityReference; // Missing id
				// Already set up
			});
	...
	Scenario(
		'Setting the requestor property with a domain object',
		({ Given, When, Then }) => {
			...
			Then(
				"the document's requestor should be set to the domain object's document",
				() => {
	...
	Scenario(
		'Setting the requestor property with missing id',
		({ Given, When, Then }) => {
			let memberRef: Domain.Contexts.Community.Member.MemberEntityReference;
			...
			Given('a ServiceTicketV1DomainAdapter for the document', () => {
				memberRef = {
					id: '',
				} as Domain.Contexts.Community.Member.MemberEntityReference; // Missing id
				// Already set up
			});
			...
			Then(
				'an error should be thrown indicating "member reference is missing id"',
				() => {
					try {
						setRequestorWithoutId();
						throw new Error('Expected error was not thrown');
						...
						expect((error as Error).message).toBe(
							'member reference is missing id',
						);
					}
				},
			);
	...
	Scenario(
		'Setting the community property with null reference',
		({ Given, When, Then }) => {
	...
	Scenario(
		'Setting the requestor property with null reference',
		({ Given, When, Then }) => {
	...
	Scenario(
		'Setting the community property with undefined reference',
		({ Given, When, Then }) => {
	...
	Scenario(
		'Setting the requestor property with undefined reference',
		({ Given, When, Then }) => {
...
test.for(
	...
	({ Scenario, Background, BeforeEachScenario }) => {
		...
		BeforeEachScenario(() => {
			...
			result =
				{} as Domain.Contexts.Case.ServiceTicket.V1.ServiceTicketV1<ServiceTicketV1DomainAdapter>;
		});
		...
		Scenario(
			'Converting a Mongoose ServiceTicket document to a domain object',
			({ Given, When, Then, And }) => {
				...
				And(
					'the domain object\'s description should be "Test Description"',
					() => {
						expect(
							(
								result as Domain.Contexts.Case.ServiceTicket.V1.ServiceTicketV1<ServiceTicketV1DomainAdapter>
		...
		Scenario(
			'Converting a domain object to a Mongoose ServiceTicket document',
			({ Given, And, When, Then }) => {
				...
				And(
					'a ServiceTicketV1 domain object with title "New Ticket", description "New Description", status "closed", priority 2, and valid community and requestor',
					() => {
						const communityDoc = makeCommunityDoc();
						const memberDoc = makeMemberDoc({ community: communityDoc });
						...
						domainObj =
							new Domain.Contexts.Case.ServiceTicket.V1.ServiceTicketV1(
								adapter,
								passport,
							);
					},
				...
				When(
					'I call toPersistence with the ServiceTicketV1 domain object',
					() => {
				...
				And(
					"the document's requestor should be set to the correct requestor document",
					() => {
