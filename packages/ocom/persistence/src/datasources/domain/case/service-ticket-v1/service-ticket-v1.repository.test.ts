...
import {
	...
	type ServiceTicketV1DomainAdapter,
...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	BeforeEachScenario(() => {
		...
		Object.assign(ModelMock, {
			findById: vi.fn((id: string) => ({
				exec: vi.fn(async () =>
					id === '507f1f77bcf86cd799439011' ? serviceTicketDoc : null,
				),
			})),
		});
	...
	Scenario(
		'Getting a service ticket by id that does not exist',
		({ When, Then }) => {
			let gettingServiceTicketThatDoesNotExist: () => Promise<
			...
			When('I call getById with "nonexistent-id"', () => {
				gettingServiceTicketThatDoesNotExist = async () =>
					await repo.getById('nonexistent-id');
			});
			Then(
				'an error should be thrown indicating "ServiceTicket with id nonexistent-id not found"',
				async () => {
					await expect(gettingServiceTicketThatDoesNotExist).rejects.toThrow();
					await expect(gettingServiceTicketThatDoesNotExist).rejects.toThrow(
						/ServiceTicket with id nonexistent-id not found/,
					);
				},
			);
		},
	...
	Scenario(
		'Creating a new service ticket instance',
		({ Given, When, Then, And }) => {
			...
			When(
				'I call getNewInstance with title "New Ticket", description "New Description", community, and requestor',
				async () => {
					result = await repo.getNewInstance(
						new Domain.Contexts.Case.ServiceTicket.V1.ValueObjects.Title(
							'New Ticket',
						),
						new Domain.Contexts.Case.ServiceTicket.V1.ValueObjects.Description(
							'New Description',
						),
						communityDomainObject,
						requestorDomainObject,
			...
			And(
				'the domain object\'s description should be "New Description"',
				() => {
