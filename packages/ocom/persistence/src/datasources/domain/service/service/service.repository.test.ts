...
test.for(repositoryFeature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	BeforeEachScenario(() => {
		...
		Object.assign(ModelMock, {
			findById: vi.fn((id: string) => ({
				...
				exec: vi.fn(async () =>
					id === '123' ? makeServiceDoc({ _id: id }) : null,
				),
			})),
		});
	...
	Background(({ Given }) => {
		Given(
			'a ServiceRepository instance with a mock model and converter',
			() => {
	...
	Scenario(
		'Getting a service by ID when the service exists',
		({ Given, When, Then, And }) => {
			...
			And(
				'the model\'s findById method should have been called with "123"',
				() => {
	...
	Scenario(
		'Getting a service by ID when the service does not exist',
		({ Given, When, Then }) => {
			let gettingServiceThatDoesNotExist: () => Promise<
				Domain.Contexts.Service.Service.Service<Domain.Contexts.Service.Service.ServiceProps>
			>;
			Given('no service document exists in the database with ID "999"', () => {
				// Mock is set up in BeforeEachScenario
			});
			When('I call getById with "999"', () => {
				gettingServiceThatDoesNotExist = async () =>
					await repository.getById('999');
			});
			Then(
				'an error should be thrown indicating "Service with id 999 not found"',
				async () => {
	...
	Scenario('Getting a new service instance', ({ Given, When, Then }) => {
		...
		When(
			'I call getNewInstance with service name "New Service", description "New description", and community reference',
			async () => {
