...
test.for(
	...
	({ Scenario, Background, BeforeEachScenario }) => {
		...
		Scenario(
			'Getting and setting the serviceName property',
			({ Given, When, Then }) => {
		...
		Scenario(
			...
			({ Given, When, Then }) => {
				...
				Then(
					'the document\'s description should be "Updated description"',
					() => {
		...
		Scenario(
			'Getting and setting the isActive property',
			({ Given, When, Then }) => {
				Given('a ServiceDomainAdapter for the document', () => {
					adapter = new ServiceDomainAdapter(doc);
				});
		...
		Scenario(
			...
			({ Given, When, Then }) => {
				...
				Given(
					'a ServiceDomainAdapter for a document with community as an ObjectId',
					() => {
						doc = makeServiceDoc({
		...
		Scenario(
			...
			({ Given, And, When, Then }) => {
				...
				Then(
					"the document's community should be set to the community's id as ObjectId",
					() => {
						expect(doc.community).toBeInstanceOf(MongooseSeedwork.ObjectId);
						expect(
							(doc.community as MongooseSeedwork.ObjectId).toString(),
						).toBe(communityDomainObj.id);
					},
				);
			},
		);
...
test.for(
	...
	({ Scenario, Background, BeforeEachScenario }) => {
		...
		Scenario(
			'Converting a Mongoose Service document to a domain object',
			({ Given, When, Then, And }) => {
				...
				And('the domain object\'s serviceName should be "Test Service"', () => {
					expect(
						(
							result as Domain.Contexts.Service.Service.Service<ServiceDomainAdapter>
						).serviceName,
					).toBe('Test Service');
				});
				And(
					'the domain object\'s description should be "Test service description"',
					() => {
						expect(
							(
								result as Domain.Contexts.Service.Service.Service<ServiceDomainAdapter>
							).description,
						).toBe('Test service description');
					},
				);
			},
		);
		...
		Scenario(
			'Converting a domain object to a Mongoose Service document',
			({ Given, And, When, Then }) => {
				...
				And(
					'a Service domain object with serviceName "New Service", description "New description", and valid community',
					() => {
						communityAdapter = new CommunityDomainAdapter(communityDoc);
						communityDomainObj =
							new Domain.Contexts.Community.Community.Community(
						...
						adapter.setCommunityRef(communityDomainObj);
						domainObj = new Domain.Contexts.Service.Service.Service(
							adapter,
							passport,
						);
					},
