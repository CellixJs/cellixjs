...
export class ServiceRepository
	extends MongooseSeedwork.MongoRepositoryBase<ServiceModelType, PropType, Domain.Passport, Domain.Contexts.Service.Service.Service<PropType>>
	implements Domain.Contexts.Service.Service.ServiceRepository<PropType>
{
	...
	getNewInstance(serviceName: string, description: string, community: Domain.Contexts.Community.Community.CommunityEntityReference): Promise<Domain.Contexts.Service.Service.Service<PropType>> {
		const adapter = this.typeConverter.toAdapter(new this.model());
		return Promise.resolve(Domain.Contexts.Service.Service.Service.getNewInstance(adapter, serviceName, description, community, this.passport));
	}
}
