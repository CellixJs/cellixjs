...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	BeforeEachScenario(() => {
		...
		result =
			{} as Domain.Contexts.Property.Property.Property<PropertyDomainAdapter>;
		results = [];
		...
		Object.assign(ModelMock, {
			findById: vi.fn((id: string) => ({
				...
				exec: vi.fn(async () =>
					id === '507f1f77bcf86cd799439011' ? propertyDoc : null,
				),
			})),
			find: vi.fn(() => ({
				populate: vi.fn().mockReturnThis(),
...
import type { 
	...
	Scenario('Getting a property by id that does not exist', ({ When, Then }) => {
		let gettingPropertyThatDoesNotExist: () => Promise<
			Domain.Contexts.Property.Property.Property<PropertyDomainAdapter>
		>;
		When('I call getById with "nonexistent-id"', () => {
			gettingPropertyThatDoesNotExist = async () =>
				await repo.getById('nonexistent-id');
		});
		Then(
			'an error should be thrown indicating "Property with id nonexistent-id not found"',
			async () => {
...
import type { 
		...
		And(
			'the array should contain at least one property with name "Test Property"',
			() => {
				const testProperty = results.find(
					(property) => property.propertyName === 'Test Property',
				);
	...
	Scenario('Creating a new property instance', ({ Given, When, Then, And }) => {
		...
		When(
			'I call getNewInstance with name "New Property" and the community',
			async () => {
	...
	Scenario(
		'Creating a new property instance with an invalid community',
		({ Given, When, Then }) => {
			...
			When(
				'I call getNewInstance with name "Invalid Property" and the invalid community',
				() => {
