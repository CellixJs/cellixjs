...
import type {  value: unknown) {
		...
		populate(path: string) {
			...
			if (
				path === 'community' &&
				this.community instanceof MongooseSeedwork.ObjectId
			) {
				this.community = makeCommunityDoc();
			} else if (
				path === 'owner' &&
				this.owner instanceof MongooseSeedwork.ObjectId
			) {
...
test.for(
	...
	({ Scenario, Background, BeforeEachScenario }) => {
		...
		Scenario(
			'Getting and setting the propertyName property',
			({ Given, When, Then }) => {
				...
				Then(
					'the document\'s propertyName should be "Updated Property"',
					() => {
		...
		Scenario(
			'Getting and setting the propertyType property',
			({ Given, When, Then }) => {
				Given('a PropertyDomainAdapter for the document', () => {
					adapter = new PropertyDomainAdapter(doc);
				});
		...
		Scenario(
			'Getting and setting the listedForSale property',
			({ Given, When, Then }) => {
				Given('a PropertyDomainAdapter for the document', () => {
					adapter = new PropertyDomainAdapter(doc);
				});
		...
		Scenario(
			'Getting and setting the listedForRent property',
			({ Given, When, Then }) => {
				Given('a PropertyDomainAdapter for the document', () => {
					adapter = new PropertyDomainAdapter(doc);
				});
		...
		Scenario(
			'Getting and setting the listedForLease property',
			({ Given, When, Then }) => {
				Given('a PropertyDomainAdapter for the document', () => {
					adapter = new PropertyDomainAdapter(doc);
				});
		...
		Scenario(
			'Getting and setting the listedInDirectory property',
			({ Given, When, Then }) => {
				Given('a PropertyDomainAdapter for the document', () => {
					adapter = new PropertyDomainAdapter(doc);
				});
		...
		Scenario(
			'Getting and setting the tags property',
			({ Given, When, Then }) => {
				Given('a PropertyDomainAdapter for the document', () => {
					adapter = new PropertyDomainAdapter(doc);
				});
		...
		Scenario(
			'Getting and setting the hash property',
			({ Given, When, Then }) => {
				Given('a PropertyDomainAdapter for the document', () => {
					adapter = new PropertyDomainAdapter(doc);
				});
		...
		Scenario(
			'Getting and setting the lastIndexed property',
			({ Given, When, Then }) => {
				let testDate: Date;
				Given('a PropertyDomainAdapter for the document', () => {
					adapter = new PropertyDomainAdapter(doc);
		...
		Scenario(
			'Getting and setting the updateIndexFailedDate property',
			({ Given, When, Then }) => {
				let testDate: Date;
				Given('a PropertyDomainAdapter for the document', () => {
					adapter = new PropertyDomainAdapter(doc);
		...
		Scenario('Getting the location property', ({ Given, When, Then }) => {
			...
			Then(
				'it should return a PropertyLocationAddressDomainAdapter instance',
				() => {
					expect(result).toBeDefined();
			...
			Then(
				'it should return a PropertyLocationPositionDomainAdapter instance',
				() => {
					expect(result).toBeDefined();
		...
		Scenario(
			'Getting the community property when populated',
			({ Given, When, Then }) => {
				Given('a PropertyDomainAdapter for the document', () => {
					adapter = new PropertyDomainAdapter(doc);
				});
				...
				Then(
					'it should return a CommunityDomainAdapter instance with the correct community data',
					() => {
		...
		Scenario(
			'Getting the community property when not populated',
			({ Given, When, Then }) => {
				let gettingCommunityWhenNotPopulated: () => void;
				Given(
					'a PropertyDomainAdapter for a document with community as an ObjectId',
					() => {
						doc = makePropertyDoc({
							community: new MongooseSeedwork.ObjectId(),
						});
						adapter = new PropertyDomainAdapter(doc);
					},
				);
				...
				Then(
					'an error should be thrown indicating "community is not populated or is not of the correct type"',
					() => {
						expect(gettingCommunityWhenNotPopulated).toThrow();
						expect(gettingCommunityWhenNotPopulated).throws(
		...
		Scenario(
			'Setting the community property with a valid Community domain object',
			({ Given, And, When, Then }) => {
				let communityAdapter: CommunityDomainAdapter;
				...
				And('a valid Community domain object', () => {
					communityAdapter = new CommunityDomainAdapter(communityDoc);
					communityDomainObj =
						new Domain.Contexts.Community.Community.Community(
							communityAdapter,
							makeMockPassport(),
						);
				});
				When(
					'I set the community property to the Community domain object',
					() => {
						adapter.community = communityDomainObj;
					},
				);
				Then(
					"the document's community should be set to the community's doc",
					() => {
		...
		Scenario(
			'Setting the community property with an invalid value',
			({ Given, And, When, Then }) => {
				...
				When(
					'I try to set the community property to the invalid object',
					() => {
				...
				Then(
					'an error should be thrown indicating "community reference is missing id"',
					() => {
						expect(settingCommunityWithInvalidValue).toThrow();
						expect(settingCommunityWithInvalidValue).throws(
							/community reference is missing id/,
						);
					},
				);
			},
		...
		Scenario(
			'Getting the owner property when populated',
			({ Given, When, Then }) => {
				Given('a PropertyDomainAdapter for the document', () => {
					adapter = new PropertyDomainAdapter(doc);
				});
		...
		Scenario(
			'Getting the owner property when not populated',
			({ Given, When, Then }) => {
				let gettingOwnerWhenNotPopulated: () => void;
				Given(
					'a PropertyDomainAdapter for a document with owner as an ObjectId',
					() => {
				...
				Then(
					'an error should be thrown indicating "owner is not populated or is not of the correct type"',
					() => {
						expect(gettingOwnerWhenNotPopulated).toThrow();
						expect(gettingOwnerWhenNotPopulated).throws(
		...
		Scenario(
			'Setting the owner property with a valid Member domain object',
			({ Given, And, When, Then }) => {
				let memberAdapter: MemberDomainAdapter;
				...
				And('a valid Member domain object', () => {
					memberAdapter = new MemberDomainAdapter(memberDoc);
					memberDomainObj = new Domain.Contexts.Community.Member.Member(
						memberAdapter,
						makeMockPassport(),
					);
				});
		...
		Scenario(
			'Setting the owner property with an invalid value',
			({ Given, And, When, Then }) => {
				...
				When('I try to set the owner property to the invalid object', () => {
					settingOwnerWithInvalidValue = () => {
						adapter.setOwnerRef(
							memberAdapter as unknown as Domain.Contexts.Community.Member.MemberEntityReference,
						);
					};
				});
				Then(
					'an error should be thrown indicating "owner reference is missing id"',
					() => {
						expect(settingOwnerWithInvalidValue).toThrow();
						expect(settingOwnerWithInvalidValue).throws(
							/owner reference is missing id/,
						);
					},
				);
			},
		...
		Scenario(
			'Getting and setting address properties',
			({ Given, When, Then }) => {
				Given('a PropertyDomainAdapter for the document', () => {
					adapter = new PropertyDomainAdapter(doc);
				});
				...
				When(
					'I get the municipalitySubdivision property from the address',
					() => {
				...
				When(
					'I set the municipalitySubdivision property to "Subdivision"',
					() => {
						adapter.location.address.municipalitySubdivision = 'Subdivision';
					},
				);
				Then(
					'the document\'s address municipalitySubdivision should be "Subdivision"',
					() => {
						expect(doc.location.address.municipalitySubdivision).toBe(
							'Subdivision',
						);
					},
				);
				...
				When(
					'I get the countrySecondarySubdivision property from the address',
					() => {
						result = adapter.location.address.countrySecondarySubdivision;
					},
				);
				Then(
					'the countrySecondarySubdivision should be an empty string',
					() => {
						expect(result).toBe('');
					},
				);
				When(
					'I set the countrySecondarySubdivision property to "Secondary"',
					() => {
						adapter.location.address.countrySecondarySubdivision = 'Secondary';
					},
				);
				Then(
					'the document\'s address countrySecondarySubdivision should be "Secondary"',
					() => {
						expect(doc.location.address.countrySecondarySubdivision).toBe(
							'Secondary',
						);
					},
				);
				When(
					'I get the countryTertiarySubdivision property from the address',
					() => {
				...
				When(
					'I set the countryTertiarySubdivision property to "Tertiary"',
					() => {
						adapter.location.address.countryTertiarySubdivision = 'Tertiary';
					},
				);
				Then(
					'the document\'s address countryTertiarySubdivision should be "Tertiary"',
					() => {
						expect(doc.location.address.countryTertiarySubdivision).toBe(
							'Tertiary',
						);
					},
				);
				...
				Then(
					'the document\'s address countrySubdivision should be "State"',
					() => {
						expect(doc.location.address.countrySubdivision).toBe('State');
					},
				);
				When(
					'I get the countrySubdivisionName property from the address',
					() => {
				...
				When(
					'I set the countrySubdivisionName property to "State Name"',
					() => {
						adapter.location.address.countrySubdivisionName = 'State Name';
					},
				);
				Then(
					'the document\'s address countrySubdivisionName should be "State Name"',
					() => {
						expect(doc.location.address.countrySubdivisionName).toBe(
							'State Name',
						);
					},
				);
				...
				Then(
					'the document\'s address extendedPostalCode should be "6789"',
					() => {
				...
				When(
					'I set the freeformAddress property to "123 Main St, Anytown"',
					() => {
						adapter.location.address.freeformAddress = '123 Main St, Anytown';
					},
				);
				Then(
					'the document\'s address freeformAddress should be "123 Main St, Anytown"',
					() => {
						expect(doc.location.address.freeformAddress).toBe(
							'123 Main St, Anytown',
						);
					},
				);
				...
				Then(
					'the document\'s address streetNameAndNumber should be "123 Main St"',
					() => {
						expect(doc.location.address.streetNameAndNumber).toBe(
							'123 Main St',
						);
					},
				);
		...
		Scenario(
			'Getting and setting position properties',
			({ Given, When, Then }) => {
				...
				When('I set the coordinates property to [41.7128, -75.0060]', () => {
					positionAdapter.coordinates = [41.7128, -75.006];
				});
				Then(
					"the document's position coordinates should be [41.7128, -75.0060]",
					() => {
						expect(doc.location.position.coordinates).toEqual([
							41.7128, -75.006,
						]);
					},
				);
		...
		Scenario(
			'Getting and setting listing detail properties',
			({ Given, When, Then }) => {
				...
				Then(
					'the document\'s listingDetail description should be "Beautiful home"',
					() => {
				...
				Then(
					'the document\'s listingDetail amenities should be ["pool", "gym"]',
					() => {
						expect(doc.listingDetail.amenities).toEqual(['pool', 'gym']);
					},
				);
				When(
					'I get the additionalAmenities property from the listingDetail',
					() => {
				...
				When(
					'I set the images property to ["image1.jpg", "image2.jpg"]',
					() => {
						listingDetailAdapter.images = ['image1.jpg', 'image2.jpg'];
					},
				);
				Then(
					'the document\'s listingDetail images should be ["image1.jpg", "image2.jpg"]',
					() => {
						expect(doc.listingDetail.images).toEqual([
							'image1.jpg',
							'image2.jpg',
						]);
					},
				...
				Then(
					'the document\'s listingDetail video should be "video.mp4"',
					() => {
				...
				Then(
					'the document\'s listingDetail floorPlan should be "floorplan.pdf"',
					() => {
						expect(doc.listingDetail.floorPlan).toBe('floorplan.pdf');
					},
				);
				When(
					'I get the floorPlanImages property from the listingDetail',
					() => {
				...
				When(
					'I set the floorPlanImages property to ["fp1.jpg", "fp2.jpg"]',
					() => {
						listingDetailAdapter.floorPlanImages = ['fp1.jpg', 'fp2.jpg'];
					},
				);
				Then(
					'the document\'s listingDetail floorPlanImages should be ["fp1.jpg", "fp2.jpg"]',
					() => {
						expect(doc.listingDetail.floorPlanImages).toEqual([
							'fp1.jpg',
							'fp2.jpg',
						]);
					},
				...
				Then(
					'the document\'s listingDetail listingAgent should be "John Doe"',
					() => {
						expect(doc.listingDetail.listingAgent).toBe('John Doe');
					},
				);
				When(
					'I get the listingAgentPhone property from the listingDetail',
					() => {
				...
				Then(
					'the document\'s listingDetail listingAgentPhone should be "555-1234"',
					() => {
						expect(doc.listingDetail.listingAgentPhone).toBe('555-1234');
					},
				);
				When(
					'I get the listingAgentEmail property from the listingDetail',
					() => {
				...
				When(
					'I set the listingAgentEmail property to "john@example.com"',
					() => {
						listingDetailAdapter.listingAgentEmail = 'john@example.com';
					},
				);
				Then(
					'the document\'s listingDetail listingAgentEmail should be "john@example.com"',
					() => {
						expect(doc.listingDetail.listingAgentEmail).toBe(
							'john@example.com',
						);
					},
				);
				When(
					'I get the listingAgentWebsite property from the listingDetail',
					() => {
				...
				When(
					'I set the listingAgentWebsite property to "http://john.com"',
					() => {
						listingDetailAdapter.listingAgentWebsite = 'http://john.com';
					},
				);
				Then(
					'the document\'s listingDetail listingAgentWebsite should be "http://john.com"',
					() => {
						expect(doc.listingDetail.listingAgentWebsite).toBe(
							'http://john.com',
						);
					},
				);
				When(
					'I get the listingAgentCompany property from the listingDetail',
					() => {
				...
				When(
					'I set the listingAgentCompany property to "Real Estate Co"',
					() => {
						listingDetailAdapter.listingAgentCompany = 'Real Estate Co';
					},
				);
				Then(
					'the document\'s listingDetail listingAgentCompany should be "Real Estate Co"',
					() => {
						expect(doc.listingDetail.listingAgentCompany).toBe(
							'Real Estate Co',
						);
					},
				);
				When(
					'I get the listingAgentCompanyPhone property from the listingDetail',
					() => {
				...
				When(
					'I set the listingAgentCompanyPhone property to "555-5678"',
					() => {
						listingDetailAdapter.listingAgentCompanyPhone = '555-5678';
					},
				);
				Then(
					'the document\'s listingDetail listingAgentCompanyPhone should be "555-5678"',
					() => {
						expect(doc.listingDetail.listingAgentCompanyPhone).toBe('555-5678');
					},
				);
				When(
					'I get the listingAgentCompanyEmail property from the listingDetail',
					() => {
				...
				When(
					'I set the listingAgentCompanyEmail property to "info@realestate.com"',
					() => {
						listingDetailAdapter.listingAgentCompanyEmail =
							'info@realestate.com';
					},
				);
				Then(
					'the document\'s listingDetail listingAgentCompanyEmail should be "info@realestate.com"',
					() => {
						expect(doc.listingDetail.listingAgentCompanyEmail).toBe(
							'info@realestate.com',
						);
					},
				);
				When(
					'I get the listingAgentCompanyWebsite property from the listingDetail',
					() => {
				...
				When(
					'I set the listingAgentCompanyWebsite property to "http://realestate.com"',
					() => {
						listingDetailAdapter.listingAgentCompanyWebsite =
							'http://realestate.com';
					},
				);
				Then(
					'the document\'s listingDetail listingAgentCompanyWebsite should be "http://realestate.com"',
					() => {
						expect(doc.listingDetail.listingAgentCompanyWebsite).toBe(
							'http://realestate.com',
						);
					},
				);
				When(
					'I get the listingAgentCompanyAddress property from the listingDetail',
					() => {
				...
				When(
					'I set the listingAgentCompanyAddress property to "123 Office St"',
					() => {
						listingDetailAdapter.listingAgentCompanyAddress = '123 Office St';
					},
				);
				Then(
					'the document\'s listingDetail listingAgentCompanyAddress should be "123 Office St"',
					() => {
						expect(doc.listingDetail.listingAgentCompanyAddress).toBe(
							'123 Office St',
						);
					},
				);
		...
		test.for(
			...
			({ Scenario, Background, BeforeEachScenario }) => {
				...
				Scenario(
					'Converting a Mongoose Property document to a domain object',
					({ Given, When, Then, And }) => {
...
import type { 
						...
						And(
							'the domain object\'s propertyName should be "Test Property"',
							() => {
								expect(
									(
				...
				Scenario(
					'Converting a domain object to a Mongoose Property document',
					({ Given, And, When, Then }) => {
						...
						And(
							'a Property domain object with propertyName "New Property", propertyType "apartment", and valid community and owner',
							() => {
								communityAdapter = new CommunityDomainAdapter(communityDoc);
								memberAdapter = new MemberDomainAdapter(memberDoc);
								communityDomainObj =
								...
								adapter.community = communityDomainObj;
								adapter.setOwnerRef(memberDomainObj);
								domainObj = new Domain.Contexts.Property.Property.Property(
									adapter,
									passport,
								);
							},
