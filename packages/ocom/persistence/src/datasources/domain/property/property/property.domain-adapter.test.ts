...
const domainAdapterFeature = await loadFeature(path.resolve(__dirname, 'features/property.domain-adapter.feature'));
const typeConverterFeature = await loadFeature(path.resolve(__dirname, 'features/property.type-converter.feature'));
...
test.for(domainAdapterFeature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	Scenario('Getting and setting position properties', ({ Given, When, Then }) => {
		...
		Then("the document's position coordinates should be [41.7128, -75.0060]", () => {
			expect(doc.location.position.coordinates).toEqual([41.7128, -75.006]);
		});
	});
