...
import {
	StaffUserConverter,
	type StaffUserDomainAdapter,
} from './staff-user.domain-adapter.ts';
...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	BeforeEachScenario(() => {
		...
		result =
			{} as Domain.Contexts.User.StaffUser.StaffUser<StaffUserDomainAdapter>;
		...
		Object.assign(ModelMock, {
			findById: vi.fn((id: string) => ({
				populate: vi.fn(() => ({
					exec: vi.fn(async () =>
						id === '507f1f77bcf86cd799439011' ? staffUserDoc : null,
					),
				})),
			})),
	...
	Background(({ Given }) => {
		Given(
			'a StaffUserRepository instance with a valid model and passport',
			() => {
	...
	Scenario('Getting a staff user by ID', ({ Given, When, Then, And }) => {
		Given(
			'a staff user exists in the database with ID "507f1f77bcf86cd799439011"',
			() => {
	...
	Scenario(
		"Getting a staff user by ID that doesn't exist",
		({ Given, When, Then }) => {
			...
			Then(
				'it should throw an error indicating "StaffUser with id 507f1f77bcf86cd799439012 not found"',
				async () => {
					await expect(
						repo.getById('507f1f77bcf86cd799439012'),
					).rejects.toThrow(
	...
	Scenario(
		'Getting a staff user by external ID',
		({ Given, When, Then, And }) => {
			Given(
				'a staff user exists in the database with externalId "12345678-1234-1234-8123-123456789012"',
				() => {
					// Already mocked in BeforeEachScenario
				},
			);
			When(
				'I call getByExternalId with "12345678-1234-1234-8123-123456789012"',
				async () => {
					result = await repo.getByExternalId(
			...
			And(
				'the staff user by external ID should have the correct properties',
				() => {
					expect(result.firstName).toBe('John');
					expect(result.lastName).toBe('Doe');
					expect(result.email).toBe('john.doe@example.com');
	...
	Scenario(
		"Getting a staff user by external ID that doesn't exist",
		({ Given, When, Then }) => {
			Given(
				'no staff user exists with externalId "87654321-4321-4321-4321-210987654321"',
				() => {
					// Already mocked to return null
				},
			);
			When(
				'I call getByExternalId with "87654321-4321-4321-4321-210987654321"',
				async () => {
					// Test will check the error
				},
			);
			Then(
				'it should throw an error indicating "StaffUser with externalId 87654321-4321-4321-4321-210987654321 not found"',
				async () => {
					await expect(
						repo.getByExternalId('87654321-4321-4321-4321-210987654321'),
					).rejects.toThrow(
	...
	Scenario(
		'Creating a new staff user instance',
		({ Given, When, Then, And }) => {
			Given('valid parameters for a new staff user', () => {
				// Already set up
			});
			When(
				'I call getNewInstance with externalId "12345678-1234-1234-8123-123456789012", firstName "John", lastName "Doe", email "john.doe@example.com"',
				async () => {
