...
import type { StaffUserModelType } from '@ocom/data-sources-mongoose-models';
...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	Scenario('Creating StaffUser Persistence', ({ When, Then, And }) => {
		...
		And('the StaffUserUnitOfWork should be properly initialized', () => {
			...
			expect(result.StaffUserUnitOfWork).toHaveProperty(
	...
	Scenario(
		'Creating StaffUser Persistence with missing StaffUser model',
		({ Given, When, Then }) => {
			...
			Then(
				'an error should be thrown indicating "StaffUser model is not available in the mongoose context"',
				() => {
					expect(caughtError).toBeDefined();
					expect(caughtError.message).toContain(
						'StaffUser model is not available in the mongoose context',
					);
				},
			);
		},
