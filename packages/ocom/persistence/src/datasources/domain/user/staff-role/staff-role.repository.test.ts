...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	BeforeEachScenario(() => {
		...
		Object.assign(ModelMock, {
			findById: vi.fn((id: string) => ({
				exec: vi.fn(async () =>
					id === staffRoleDoc._id ? staffRoleDoc : null,
				),
			})),
			findOne: vi.fn((query: { roleName: string }) => ({
				exec: vi.fn(async () =>
					query.roleName === staffRoleDoc.roleName ? staffRoleDoc : null,
				),
			})),
			prototype: {},
		});
	...
	Scenario(
		'Getting a staff role by id that does not exist',
		({ When, Then }) => {
			...
			Then(
				'an error should be thrown indicating "StaffRole with id nonexistent-id not found"',
				async () => {
					await expect(getById).rejects.toThrow();
					await expect(getById).rejects.toThrow(
						/StaffRole with id nonexistent-id not found/,
					);
				},
			);
		},
	...
	Scenario(
		'Getting a staff role by roleName that does not exist',
		({ When, Then }) => {
			let getByRoleName: () => Promise<unknown>;
			When('I call getByRoleName with "nonexistent-role"', () => {
				getByRoleName = async () =>
					await repo.getByRoleName('nonexistent-role');
			});
			Then(
				'an error should be thrown indicating "StaffRole with roleName nonexistent-role not found"',
				async () => {
					await expect(getByRoleName).rejects.toThrow();
					await expect(getByRoleName).rejects.toThrow(
						/StaffRole with roleName nonexistent-role not found/,
					);
				},
			);
		},
