...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	Scenario('Creating StaffRole Persistence', ({ When, Then, And }) => {
		...
		And('the StaffRoleUnitOfWork should be properly initialized', () => {
			...
			expect(result.StaffRoleUnitOfWork).toHaveProperty(
	...
	Scenario(
		'Creating StaffRole Persistence with missing StaffRole model',
		({ Given, When, Then }) => {
			...
			Then(
				'an error should be thrown indicating "StaffRole model is not available in the mongoose context"',
				() => {
					expect(caughtError).toBeDefined();
					expect(caughtError.message).toContain(
						'StaffRole model is not available in the mongoose context',
					);
				},
			);
		},
