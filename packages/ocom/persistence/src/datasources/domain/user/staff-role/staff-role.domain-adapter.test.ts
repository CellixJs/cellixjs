...
test.for(
	...
	({ Scenario, Background, BeforeEachScenario }) => {
		...
		Scenario(
			'Getting communityPermissions from permissions',
			({ Given, When, And, Then }) => {
				...
				Then(
					'it should return a StaffRoleCommunityPermissionsAdapter instance',
					() => {
		...
		Scenario(
			'Getting and setting canManageStaffRolesAndPermissions from communityPermissions',
			({ Given, When, And, Then }) => {
				let permissions: StaffRolePermissionsAdapter;
				...
				And('I get the communityPermissions property', () => {
					communityPermissions =
						permissions.communityPermissions as StaffRoleCommunityPermissionsAdapter;
				});
				...
				When(
					'I set the canManageStaffRolesAndPermissions property to true',
					() => {
						communityPermissions.canManageStaffRolesAndPermissions = true;
					},
				);
				Then(
					"the communityPermissions' canManageStaffRolesAndPermissions should be true",
					() => {
						expect(
		...
		Scenario(
			'Getting and setting canManageAllCommunities from communityPermissions',
			({ Given, When, And, Then }) => {
				let permissions: StaffRolePermissionsAdapter;
				let communityPermissions: StaffRoleCommunityPermissionsAdapter;
				Given('a StaffRoleDomainAdapter for the document', () => {
				...
				Then(
					"the communityPermissions' canManageAllCommunities should be true",
					() => {
						expect(
							doc.permissions?.communityPermissions?.canManageAllCommunities,
						).toBe(true);
					},
				);
			},
		...
		Scenario(
			'Getting and setting canDeleteCommunities from communityPermissions',
			({ Given, When, And, Then }) => {
				let permissions: StaffRolePermissionsAdapter;
				let communityPermissions: StaffRoleCommunityPermissionsAdapter;
				Given('a StaffRoleDomainAdapter for the document', () => {
				...
				Then(
					"the communityPermissions' canDeleteCommunities should be true",
					() => {
						expect(
							doc.permissions?.communityPermissions?.canDeleteCommunities,
						).toBe(true);
					},
				);
			},
		...
		Scenario(
			'Getting and setting canChangeCommunityOwner from communityPermissions',
			({ Given, When, And, Then }) => {
				let permissions: StaffRolePermissionsAdapter;
				let communityPermissions: StaffRoleCommunityPermissionsAdapter;
				Given('a StaffRoleDomainAdapter for the document', () => {
				...
				Then(
					"the communityPermissions' canChangeCommunityOwner should be true",
					() => {
						expect(
							doc.permissions?.communityPermissions?.canChangeCommunityOwner,
						).toBe(true);
					},
				);
			},
		...
		Scenario(
			'Getting and setting canReIndexSearchCollections from communityPermissions',
			({ Given, When, And, Then }) => {
				let permissions: StaffRolePermissionsAdapter;
				let communityPermissions: StaffRoleCommunityPermissionsAdapter;
				Given('a StaffRoleDomainAdapter for the document', () => {
				...
				Then(
					"the communityPermissions' canReIndexSearchCollections should be true",
					() => {
						expect(
							doc.permissions?.communityPermissions
		...
		Scenario(
			'Getting propertyPermissions from permissions',
			({ Given, When, And, Then }) => {
				let permissions: StaffRolePermissionsAdapter;
				Given('a StaffRoleDomainAdapter for the document', () => {
					adapter = new StaffRoleDomainAdapter(doc);
				...
				Then(
					'it should return a StaffRolePropertyPermissionsAdapter instance',
					() => {
		...
		Scenario(
			'Getting and setting canManageProperties from propertyPermissions',
			({ Given, When, And, Then }) => {
				let permissions: StaffRolePermissionsAdapter;
				...
				And('I get the propertyPermissions property', () => {
					propertyPermissions =
						permissions.propertyPermissions as StaffRolePropertyPermissionsAdapter;
				});
				...
				Then(
					"the propertyPermissions' canManageProperties should be true",
					() => {
						expect(
							doc.permissions?.propertyPermissions?.canManageProperties,
						).toBe(true);
					},
				);
			},
		...
		Scenario(
			'Getting and setting canEditOwnProperty from propertyPermissions',
			({ Given, When, And, Then }) => {
				let permissions: StaffRolePermissionsAdapter;
				let propertyPermissions: StaffRolePropertyPermissionsAdapter;
				Given('a StaffRoleDomainAdapter for the document', () => {
		...
		Scenario(
			'Getting servicePermissions from permissions',
			({ Given, When, And, Then }) => {
				let permissions: StaffRolePermissionsAdapter;
				Given('a StaffRoleDomainAdapter for the document', () => {
					adapter = new StaffRoleDomainAdapter(doc);
				...
				Then(
					'it should return a StaffRoleServicePermissionsAdapter instance',
					() => {
		...
		Scenario(
			'Getting canManageServices from servicePermissions',
			({ Given, When, And, Then }) => {
				let permissions: StaffRolePermissionsAdapter;
				...
				And('I get the servicePermissions property', () => {
					servicePermissions =
						permissions.servicePermissions as StaffRoleServicePermissionsAdapter;
				});
		...
		Scenario(
			'Getting serviceTicketPermissions from permissions',
			({ Given, When, And, Then }) => {
				let permissions: StaffRolePermissionsAdapter;
				Given('a StaffRoleDomainAdapter for the document', () => {
					adapter = new StaffRoleDomainAdapter(doc);
				...
				Then(
					'it should return a StaffRoleServiceTicketPermissionsAdapter instance',
					() => {
		...
		Scenario(
			'Getting ticket permissions from serviceTicketPermissions',
			({ Given, When, Then, And }) => {
				let permissions: StaffRolePermissionsAdapter;
				...
				And('I get the serviceTicketPermissions property', () => {
					serviceTicketPermissions =
						permissions.serviceTicketPermissions as StaffRoleServiceTicketPermissionsAdapter;
				});
		...
		Scenario(
			'Getting violationTicketPermissions from permissions',
			({ Given, When, And, Then }) => {
				let permissions: StaffRolePermissionsAdapter;
				Given('a StaffRoleDomainAdapter for the document', () => {
					adapter = new StaffRoleDomainAdapter(doc);
				...
				Then(
					'it should return a StaffRoleViolationTicketPermissionsAdapter instance',
					() => {
						expect(result).toBeInstanceOf(
							StaffRoleViolationTicketPermissionsAdapter,
						);
		...
		Scenario(
			'Getting and setting violation ticket permissions',
			({ Given, When, Then, And }) => {
				let permissions: StaffRolePermissionsAdapter;
				...
				And('I get the violationTicketPermissions property', () => {
					violationTicketPermissions =
						permissions.violationTicketPermissions as StaffRoleViolationTicketPermissionsAdapter;
				});
				Then('the canCreateTickets property should return false', () => {
				...
				Then(
					"the violationTicketPermissions' canCreateTickets should be true",
					() => {
						expect(
...
test.for(
	...
	({ Scenario, Background, BeforeEachScenario }) => {
		...
		BeforeEachScenario(() => {
			...
			doc = makeStaffRoleDoc();
			domainObject =
				{} as Domain.Contexts.User.StaffRole.StaffRole<StaffRoleDomainAdapter>;
			result = undefined;
		});
		...
		Scenario(
			...
			({ Given, When, Then, And }) => {
				...
				Then('I should receive a StaffRole domain object', () => {
					...
					expect(result).toBeInstanceOf(
						Domain.Contexts.User.StaffRole.StaffRole,
					);
				});
		...
		Scenario(
			'Converting a domain object to a Mongoose StaffRole document',
			({ Given, When, Then, And }) => {
				Given('a StaffRoleConverter instance', () => {
					converter = new StaffRoleConverter();
				});
				And(
					'a StaffRole domain object with roleName "Supervisor", isDefault true, and roleType "admin"',
					() => {
						...
						domainObject = new Domain.Contexts.User.StaffRole.StaffRole(
							mockAdapter,
							makeMockPassport(),
						);
					},
