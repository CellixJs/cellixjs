...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	BeforeEachScenario(() => {
		...
		Object.assign(ModelMock, {
			findById: vi.fn((id: string) => ({
				exec: vi.fn(async () =>
					id === vendorUserDoc._id ? vendorUserDoc : null,
				),
			})),
			findOne: vi.fn((query: { externalId: string }) => ({
				exec: vi.fn(async () =>
					query.externalId === vendorUserDoc.externalId ? vendorUserDoc : null,
				),
			})),
	...
	Scenario('Getting a VendorUser by external ID', ({ When, Then, And }) => {
		let result: Domain.Contexts.User.VendorUser.VendorUser<VendorUserDomainAdapter>;
		When(
			'I call getByExternalId with externalId "123e4567-e89b-12d3-a456-426614174001"',
			async () => {
	...
	Scenario('Creating a new VendorUser instance', ({ When, Then, And }) => {
		let result: Domain.Contexts.User.VendorUser.VendorUser<VendorUserDomainAdapter>;
		When(
			'I call getNewInstance with externalId "123e4567-e89b-12d3-a456-426614174002", lastName "Smith", and restOfName "John"',
			async () => {
		...
		And('the domain object\'s restOfName should be "John"', () => {
			expect(result.personalInformation.identityDetails.restOfName).toBe(
				'John',
			);
		});
	});
