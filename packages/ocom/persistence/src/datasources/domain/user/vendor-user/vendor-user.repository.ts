...
export class VendorUserRepository extends MongooseSeedwork.MongoRepositoryBase<VendorUserDocument, VendorUserDomainAdapter, Domain.Passport, VendorUserAggregate> implements VendorUserRepositoryContract {
	...
	getNewInstance(externalId: string, lastName: string, restOfName?: string): Promise<VendorUserAggregate> {
		const adapter = this.typeConverter.toAdapter(new this.model());
		adapter.tags = [];
		adapter.accessBlocked = false;
		return Promise.resolve(Domain.Contexts.User.VendorUser.VendorUser.getNewUser(adapter, this.passport, externalId, lastName, restOfName));
	}
}
