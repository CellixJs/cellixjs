...
import { VendorUserConverter, VendorUserDomainAdapter, VendorUserPersonalInformationDomainAdapter, VendorUserIdentityDetailsDomainAdapter, VendorUserContactInformationDomainAdapter } from './vendor-user.domain-adapter.ts';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const domainAdapterFeature = await loadFeature(path.resolve(__dirname, 'features/vendor-user.domain-adapter.feature'));

const typeConverterFeature = await loadFeature(path.resolve(__dirname, 'features/vendor-user.type-converter.feature'));
