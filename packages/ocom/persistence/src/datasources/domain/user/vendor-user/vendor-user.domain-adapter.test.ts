...
function makeVendorUserDoc(overrides: Partial<VendorUser> = {}) {
	...
	if (
		...
		typeof overrides.personalInformation.set !== 'function'
	) {
	...
	({ Scenario, Background, BeforeEachScenario }) => {
		...
		Scenario(
			...
			({ Given, When, Then }) => {
				...
				Then(
					'it should return a VendorUserPersonalInformationDomainAdapter instance',
					() => {
		...
		Scenario(
			...
			({ Given, When, Then }) => {
				...
				Given(
					'a VendorUserDomainAdapter for a document with no personalInformation',
					() => {
						docWithoutPersonalInfo = makeVendorUserDoc({
							personalInformation: {},
		...
		Scenario(
			...
			({ Given, When, And, Then }) => {
				...
				When('I get the personalInformation property', () => {
					personalInformation =
						adapter.personalInformation as VendorUserPersonalInformation;
				});
				And('I get the identityDetails property', () => {
					result = personalInformation.identityDetails;
				});
				Then(
					'it should return a VendorUserIdentityDetailsDomainAdapter instance',
					() => {
						expect(result).toBeInstanceOf(
							VendorUserIdentityDetailsDomainAdapter,
						);
		...
		Scenario(
			...
			({ Given, When, And, Then }) => {
				...
				When('I get the personalInformation property', () => {
					personalInformation =
						adapter.personalInformation as VendorUserPersonalInformationDomainAdapter;
				});
				And('I get the identityDetails property', () => {
					identityDetails =
						personalInformation.identityDetails as VendorUserIdentityDetailsDomainAdapter;
				});
		...
		Scenario(
			...
			({ Given, When, And, Then }) => {
				...
				Then(
					'it should return a VendorUserContactInformationDomainAdapter instance',
					() => {
						expect(result).toBeInstanceOf(
							VendorUserContactInformationDomainAdapter,
						);
		...
		Scenario(
			...
			({ Given, When, And, Then }) => {
				...
				let personalInformation: VendorUserPersonalInformationDomainAdapter;
				Given(
					'a VendorUserDomainAdapter for a document with no contactInformation',
					() => {
						docWithoutContactInfo = makeVendorUserDoc({
							personalInformation: {
								identityDetails: { lastName: 'Doe' },
								contactInformation: {},
							},
						} as Partial<VendorUser>);
		...
		Scenario(
			...
			({ Given, When, And, Then }) => {
				...
				And('I get the contactInformation property', () => {
					contactInformation =
						personalInformation.contactInformation as VendorUserContactInformationDomainAdapter;
				});
...
test.for(
	...
	({ Scenario, Background, BeforeEachScenario }) => {
		...
		BeforeEachScenario(() => {
			...
			doc = makeVendorUserDoc();
			domainObject =
				{} as Domain.Contexts.User.VendorUser.VendorUser<VendorUserDomainAdapter>;
			result = undefined;
		});
		...
		Scenario(
			'Converting a Mongoose VendorUser document to a domain object',
			({ Given, When, Then, And }) => {
		...
		Scenario(
			'Converting a domain object to a Mongoose VendorUser document',
			({ Given, When, Then, And }) => {
				Given('a VendorUserConverter instance', () => {
					converter = new VendorUserConverter();
				});
				And(
					'a VendorUser domain object with userType "vendor-user", externalId "123e4567-e89b-12d3-a456-426614174002", email "new-vendor@example.com", displayName "New Vendor", accessBlocked true, and tags ["tag3"]',
					() => {
						// Create a mock domain object
						const mockAdapter = new VendorUserDomainAdapter(
							makeVendorUserDoc({
						...
						domainObject = new Domain.Contexts.User.VendorUser.VendorUser(
							mockAdapter,
							makeMockPassport(),
						);
					},
