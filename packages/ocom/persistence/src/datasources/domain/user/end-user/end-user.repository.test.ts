...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	BeforeEachScenario(() => {
		...
		Object.assign(ModelMock, {
			findOne: vi.fn((query: { externalId: string }) => ({
				exec: vi.fn(async () =>
					query.externalId === endUserDoc.externalId ? endUserDoc : null,
				),
			})),
			deleteOne: vi.fn(() => ({
				exec: vi.fn(async () => ({})),
	...
	Scenario('Getting an end user by externalId', ({ When, Then, And }) => {
		...
		When(
			'I call getByExternalId with "123e4567-e89b-12d3-a456-426614174001"',
			async () => {
	...
	Scenario(
		'Getting an end user by externalId that does not exist',
		({ When, Then }) => {
			let getByExternalId: () => Promise<unknown>;
			When('I call getByExternalId with "nonexistent-id"', () => {
				getByExternalId = async () =>
					await repo.getByExternalId('nonexistent-id');
			});
			Then(
				'an error should be thrown indicating "User with externalId nonexistent-id not found"',
				async () => {
					await expect(getByExternalId).rejects.toThrow();
					await expect(getByExternalId).rejects.toThrow(
						/User with externalId nonexistent-id not found/,
					);
				},
			);
		},
	...
	Scenario('Creating a new end user instance', ({ When, Then, And }) => {
		let result: Domain.Contexts.User.EndUser.EndUser<EndUserDomainAdapter>;
		When(
			'I call getNewInstance with externalId "123e4567-e89b-12d3-a456-426614174002", lastName "Smith", restOfName "Alice", and email "alice@example.com"',
			async () => {
		...
		And(
			'the domain object\'s externalId should be "123e4567-e89b-12d3-a456-426614174002"',
			() => {
		...
		And('the domain object\'s email should be "alice@example.com"', () => {
			expect(result.personalInformation.contactInformation.email).toBe(
				'alice@example.com',
			);
		});
	});
	...
	Scenario(
		...
		({ When, Then, And }) => {
			let result: Domain.Contexts.User.EndUser.EndUser<EndUserDomainAdapter>;
			When(
				'I call getNewInstance with externalId "123e4567-e89b-12d3-a456-426614174003", lastName "Smith", restOfName undefined, and email "smith@example.com"',
				async () => {
					result = await repo.getNewInstance(
			...
			And(
				'the domain object\'s externalId should be "123e4567-e89b-12d3-a456-426614174003"',
				() => {
	...
	Scenario('Deleting an end user by id', ({ When, Then }) => {
		When(
			'I call delete with "123e4567-e89b-12d3-a456-426614174001"',
			async () => {
				await repo.delete('123e4567-e89b-12d3-a456-426614174001');
			},
		);
		Then(
			'the user with id "123e4567-e89b-12d3-a456-426614174001" should be removed from the database',
			() => {
