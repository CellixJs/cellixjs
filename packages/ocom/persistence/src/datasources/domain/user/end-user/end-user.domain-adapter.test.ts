...
test.for(
	...
	({ Scenario, Background, BeforeEachScenario }) => {
		...
		Scenario(
			'Getting and setting the externalId property',
			({ Given, When, Then }) => {
				Given('an EndUserDomainAdapter for the document', () => {
					adapter = new EndUserDomainAdapter(doc);
				});
				...
				When(
					'I set the externalId property to "123e4567-e89b-12d3-a456-426614174002"',
					() => {
						adapter.externalId = '123e4567-e89b-12d3-a456-426614174002';
					},
				);
				Then(
					'the document\'s externalId should be "123e4567-e89b-12d3-a456-426614174002"',
					() => {
		...
		Scenario(
			'Getting and setting the email property',
			({ Given, When, Then }) => {
				Given('an EndUserDomainAdapter for the document', () => {
					adapter = new EndUserDomainAdapter(doc);
				});
		...
		Scenario(
			'Getting and setting the displayName property',
			({ Given, When, Then }) => {
				Given('an EndUserDomainAdapter for the document', () => {
					adapter = new EndUserDomainAdapter(doc);
				});
		...
		Scenario(
			'Getting and setting the accessBlocked property',
			({ Given, When, Then }) => {
				Given('an EndUserDomainAdapter for the document', () => {
					adapter = new EndUserDomainAdapter(doc);
				});
		...
		Scenario(
			'Getting the personalInformation property',
			({ Given, When, Then }) => {
				Given('an EndUserDomainAdapter for the document', () => {
					adapter = new EndUserDomainAdapter(doc);
				});
				...
				Then(
					'it should return an EndUserPersonalInformationDomainAdapter instance with the correct data',
					() => {
						expect(result).toBeInstanceOf(
							EndUserPersonalInformationDomainAdapter,
						);
						expect(
							(result as EndUserPersonalInformationDomainAdapter)
		...
		Scenario(
			'Getting the personalInformation property when not defined on the document',
			({ Given, When, Then, And }) => {
				let docWithNoPersonalInfo: EndUser;
				let adapterWithNoPersonalInfo: EndUserDomainAdapter;
				let setCalled = false;
				Given(
					'an EndUserDomainAdapter for a document with no personalInformation',
					() => {
						...
						adapterWithNoPersonalInfo = new EndUserDomainAdapter(
							docWithNoPersonalInfo,
						);
					},
				);
		...
		Scenario(
			'Getting the identityDetails property from personalInformation',
			({ Given, When, Then, And }) => {
				...
				Then(
					'it should return an EndUserIdentityDetailsDomainAdapter instance with the correct data',
					() => {
						expect(result).toBeInstanceOf(EndUserIdentityDetailsDomainAdapter);
						expect(
							(result as EndUserIdentityDetailsDomainAdapter).lastName,
						).toBe('Doe');
					},
				);
			},
		...
		Scenario(
			'Getting the identityDetails property when not defined on personalInformation',
			({ Given, When, And, Then }) => {
				...
				let personalInfo: EndUserPersonalInformationDomainAdapter;
				Given(
					'an EndUserDomainAdapter for a document with no identityDetails',
					() => {
						docWithNoIdentityDetails = makeEndUserDoc({
							personalInformation: {
								...
								set: (
									key: keyof EndUserPersonalInformation,
									value: unknown,
								) => {
									...
									(
										docWithNoIdentityDetails.personalInformation as EndUserPersonalInformation
									)[key] = value as never;
								},
							},
						} as unknown as EndUser);
						adapterWithNoIdentityDetails = new EndUserDomainAdapter(
							docWithNoIdentityDetails,
						);
					},
				);
				When('I get the personalInformation property', () => {
		...
		Scenario(
			'Getting and setting the lastName property from identityDetails',
			({ Given, When, And, Then }) => {
				...
				And('I get the identityDetails property', () => {
					identityDetails = (result as EndUserPersonalInformationDomainAdapter)
						.identityDetails;
				});
				...
				Then('the identityDetails\' lastName should be "Smith"', () => {
					...
					expect(doc.personalInformation.identityDetails.lastName).toBe(
						'Smith',
					);
				});
			},
		...
		Scenario(
			'Getting and setting the legalNameConsistsOfOneName property from identityDetails',
			({ Given, When, And, Then }) => {
				let identityDetails: EndUserIdentityDetailsDomainAdapter;
				Given('an EndUserDomainAdapter for the document', () => {
					adapter = new EndUserDomainAdapter(doc);
				...
				Then(
					"the identityDetails' legalNameConsistsOfOneName should be true",
					() => {
						expect(identityDetails.legalNameConsistsOfOneName).toBe(true);
						expect(
							doc.personalInformation.identityDetails
		...
		Scenario(
			'Getting and setting the restOfName property from identityDetails',
			({ Given, When, And, Then }) => {
				let identityDetails: EndUserIdentityDetailsDomainAdapter;
				Given('an EndUserDomainAdapter for the document', () => {
					adapter = new EndUserDomainAdapter(doc);
				...
				Then('the identityDetails\' restOfName should be "John"', () => {
					...
					expect(doc.personalInformation.identityDetails.restOfName).toBe(
						'John',
					);
				});
			},
		...
		Scenario(
			'Getting the contactInformation property from personalInformation',
			({ Given, When, And, Then }) => {
				let personalInfo: EndUserPersonalInformationDomainAdapter;
				Given('an EndUserDomainAdapter for the document', () => {
					adapter = new EndUserDomainAdapter(doc);
				...
				Then(
					'it should return an EndUserContactInformationDomainAdapter instance with the correct data',
					() => {
						expect(result).toBeInstanceOf(
							EndUserContactInformationDomainAdapter,
						);
						expect(
							(result as EndUserContactInformationDomainAdapter).email,
		...
		Scenario(
			'Getting the contactInformation property when not defined on personalInformation',
			({ Given, When, And, Then }) => {
				...
				let personalInfo: EndUserPersonalInformationDomainAdapter;
				Given(
					'an EndUserDomainAdapter for a document with no contactInformation',
					() => {
						docWithNoContactInfo = makeEndUserDoc({
							personalInformation: {
								...
								set: (
									...
									(
										docWithNoContactInfo.personalInformation as EndUserPersonalInformation
									)[key] = value as never;
								},
							},
						} as unknown as EndUser);
						adapterWithNoContactInfo = new EndUserDomainAdapter(
							docWithNoContactInfo,
						);
					},
				);
				When('I get the personalInformation property', () => {
		...
		Scenario(
			'Getting and setting the email property from contactInformation',
			({ Given, When, And, Then }) => {
				...
				And('I get the contactInformation property', () => {
					contactInfo = (result as EndUserPersonalInformationDomainAdapter)
						.contactInformation;
				});
				...
				Then(
					'the contactInformation\'s email should be "contact@example.com"',
					() => {
						expect(contactInfo.email).toBe('contact@example.com');
						expect(doc.personalInformation.contactInformation.email).toBe(
							'contact@example.com',
						);
					},
				);
...
test.for(
	...
	({ Scenario, Background, BeforeEachScenario }) => {
		...
		Scenario(
			'Converting a Mongoose EndUser document to a domain object',
			({ Given, When, Then, And }) => {
				...
				And('the domain object\'s userType should be "end-user"', () => {
					expect(
						(
							result as Domain.Contexts.User.EndUser.EndUser<EndUserDomainAdapter>
						).userType,
					).toBe('end-user');
				});
				And(
					'the domain object\'s externalId should be "123e4567-e89b-12d3-a456-426614174001"',
					() => {
						expect(
							(
								result as Domain.Contexts.User.EndUser.EndUser<EndUserDomainAdapter>
		...
		Scenario(
			'Converting a domain object to a Mongoose EndUser document',
			({ Given, And, When, Then }) => {
				...
				And(
					'an EndUser domain object with userType "admin", externalId "123e4567-e89b-12d3-a456-426614174002", email "admin@example.com", displayName "Admin User", accessBlocked true, and tags ["admin"]',
					() => {
						const adapter = new EndUserDomainAdapter(
							makeEndUserDoc({
								userType: 'admin',
						...
						domainObj = new Domain.Contexts.User.EndUser.EndUser(
							adapter,
							passport,
						);
					},
				...
				And(
					'the document\'s externalId should be "123e4567-e89b-12d3-a456-426614174002"',
					() => {
						expect(resultDoc.externalId).toBe(
