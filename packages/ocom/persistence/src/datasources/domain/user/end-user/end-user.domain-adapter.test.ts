...
import { EndUserConverter, EndUserDomainAdapter, EndUserPersonalInformationDomainAdapter, EndUserIdentityDetailsDomainAdapter, EndUserContactInformationDomainAdapter } from './end-user.domain-adapter.ts';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const domainAdapterFeature = await loadFeature(path.resolve(__dirname, 'features/end-user.domain-adapter.feature'));
const typeConverterFeature = await loadFeature(path.resolve(__dirname, 'features/end-user.type-converter.feature'));
