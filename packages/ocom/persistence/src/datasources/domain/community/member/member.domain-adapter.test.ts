...
test.for(
	...
	({ Scenario, Background, BeforeEachScenario }) => {
		...
		Scenario(
			'Getting and setting the memberName property',
			({ Given, When, Then }) => {
		...
		Scenario(
			'Getting and setting the cybersourceCustomerId property',
			({ Given, When, Then }) => {
				Given('a MemberDomainAdapter for the document', () => {
					adapter = new MemberDomainAdapter(doc);
				});
				...
				When(
					'I set the cybersourceCustomerId property to "new-customer-id"',
					() => {
						adapter.cybersourceCustomerId = 'new-customer-id';
					},
				);
				Then(
					'the document\'s cybersourceCustomerId should be "new-customer-id"',
					() => {
		...
		Scenario(
			...
			({ Given, When, Then }) => {
				...
				Given(
					'a MemberDomainAdapter for a document with community as an ObjectId',
					() => {
		...
		Scenario(
			'Getting the role property when populated',
			({ Given, When, Then }) => {
				Given('a MemberDomainAdapter for the document', () => {
					adapter = new MemberDomainAdapter(doc);
				});
				...
				Then(
					'it should return an EndUserRoleDomainAdapter instance with the correct role data',
					() => {
		...
		Scenario(
			'Getting the role property when not populated',
			({ Given, When, Then }) => {
				let gettingRoleWhenNotPopulated: () => void;
				Given(
					'a MemberDomainAdapter for a document with role as an ObjectId',
					() => {
				...
				Then(
					...
					() => {
						...
						expect(gettingRoleWhenNotPopulated).throws(
		...
		Scenario(
			'Setting the role property with a valid EndUserRole domain object',
			({ Given, And, When, Then }) => {
				let roleAdapter: EndUserRoleDomainAdapter;
		...
		Scenario(
			'Setting the role property with an invalid value',
			({ Given, And, When, Then }) => {
				...
				Then(
					'an error should be thrown indicating "role reference is missing id"',
					() => {
						expect(settingRoleWithInvalidValue).toThrow();
						expect(settingRoleWithInvalidValue).throws(
							/role reference is missing id/,
						);
					},
				);
			},
		...
		Scenario('Getting the accounts property', ({ Given, When, Then }) => {
			...
			Then(
				'it should return a MongoosePropArray of MemberAccount references',
				() => {
		...
		Scenario('Getting the customViews property', ({ Given, When, Then }) => {
			...
			Then(
				'it should return a MongoosePropArray of MemberCustomView references',
				() => {
					expect(result).toBeInstanceOf(MongooseSeedwork.MongoosePropArray);
				},
			);
		...
		Scenario(
			'MemberAccountDomainAdapter getting and setting firstName property',
			({ Given, When, Then }) => {
				let accountDoc: MemberAccount;
				let accountAdapter: MemberAccountDomainAdapter;

				Given(
					'a MemberAccountDomainAdapter for a member account document',
					() => {
		...
		Scenario(
			'MemberAccountDomainAdapter getting and setting lastName property',
			({ Given, When, Then }) => {
				let accountDoc: MemberAccount;
				let accountAdapter: MemberAccountDomainAdapter;
		...
		Scenario(
			'MemberAccountDomainAdapter getting user property when populated',
			({ Given, When, Then }) => {
				let accountDoc: MemberAccount;
				let accountAdapter: MemberAccountDomainAdapter;
				let userDoc: EndUser;
				...
				Given(
					'a MemberAccountDomainAdapter for a member account document with populated user',
					() => {
						userDoc = makeUserDoc();
				...
				Then(
					'it should return an EndUserDomainAdapter instance with the correct user data',
					() => {
		...
		Scenario(
			'MemberAccountDomainAdapter setting user property with valid EndUser domain object',
			({ Given, And, When, Then }) => {
				let accountDoc: MemberAccount;
				let accountAdapter: MemberAccountDomainAdapter;
				...
				And('a valid EndUser domain object', () => {
					...
					userDomainObj = new Domain.Contexts.User.EndUser.EndUser(
						userAdapter,
						makeMockPassport(),
					);
				});
		...
		Scenario(
			'MemberAccountDomainAdapter getting and setting statusCode property',
			({ Given, When, Then }) => {
				let accountDoc: MemberAccount;
				let accountAdapter: MemberAccountDomainAdapter;
		...
		Scenario(
			'MemberAccountDomainAdapter getting createdBy property when populated',
			({ Given, When, Then }) => {
				let accountDoc: MemberAccount;
				let accountAdapter: MemberAccountDomainAdapter;
				let userDoc: EndUser;
				...
				Given(
					'a MemberAccountDomainAdapter for a member account document with populated createdBy',
					() => {
						userDoc = makeUserDoc();
		...
		Scenario(
			'MemberAccountDomainAdapter setting createdBy property with valid EndUser domain object',
			({ Given, And, When, Then }) => {
				let accountDoc: MemberAccount;
				let accountAdapter: MemberAccountDomainAdapter;
				let userAdapter: EndUserDomainAdapter;
				...
				When(
					'I set the createdBy property to the EndUser domain object',
					() => {
		...
		Scenario(
			'MemberCustomViewDomainAdapter getting and setting name property',
			({ Given, When, Then }) => {
				let viewDoc: MemberCustomView;
				let viewAdapter: MemberCustomViewDomainAdapter;

				Given(
					'a MemberCustomViewDomainAdapter for a custom view document',
					() => {
		...
		Scenario(
			'MemberCustomViewDomainAdapter getting and setting type property',
			({ Given, When, Then }) => {
				let viewDoc: MemberCustomView;
				let viewAdapter: MemberCustomViewDomainAdapter;
		...
		Scenario(
			'MemberCustomViewDomainAdapter getting and setting filters property',
			({ Given, When, Then }) => {
				let viewDoc: MemberCustomView;
				let viewAdapter: MemberCustomViewDomainAdapter;
		...
		Scenario(
			'MemberCustomViewDomainAdapter getting and setting sortOrder property',
			({ Given, When, Then }) => {
				let viewDoc: MemberCustomView;
				let viewAdapter: MemberCustomViewDomainAdapter;
		...
		Scenario(
			'MemberCustomViewDomainAdapter getting and setting columnsToDisplay property',
			({ Given, When, Then }) => {
				let viewDoc: MemberCustomView;
				let viewAdapter: MemberCustomViewDomainAdapter;
				...
				When(
					'I set the columnsToDisplay property to a new columns array',
					() => {
		...
		Scenario(
			'MemberProfileDomainAdapter getting and setting name property',
			({ Given, When, Then }) => {
				let profileDoc: MemberProfile;
		...
		Scenario(
			'MemberProfileDomainAdapter getting and setting email property',
			({ Given, When, Then }) => {
				let profileDoc: MemberProfile;
				let profileAdapter: MemberProfileDomainAdapter;
		...
		Scenario(
			'MemberProfileDomainAdapter getting and setting bio property',
			({ Given, When, Then }) => {
				let profileDoc: MemberProfile;
				let profileAdapter: MemberProfileDomainAdapter;
		...
		Scenario(
			'MemberProfileDomainAdapter getting and setting avatarDocumentId property',
			({ Given, When, Then }) => {
				let profileDoc: MemberProfile;
				let profileAdapter: MemberProfileDomainAdapter;
		...
		Scenario(
			'MemberProfileDomainAdapter getting and setting interests property',
			({ Given, When, Then }) => {
				let profileDoc: MemberProfile;
				let profileAdapter: MemberProfileDomainAdapter;
		...
		Scenario(
			'MemberProfileDomainAdapter getting and setting showInterests property',
			({ Given, When, Then }) => {
				let profileDoc: MemberProfile;
				let profileAdapter: MemberProfileDomainAdapter;
		...
		Scenario(
			'MemberProfileDomainAdapter getting and setting showEmail property',
			({ Given, When, Then }) => {
				let profileDoc: MemberProfile;
				let profileAdapter: MemberProfileDomainAdapter;
		...
		Scenario(
			'MemberProfileDomainAdapter getting and setting showProfile property',
			({ Given, When, Then }) => {
				let profileDoc: MemberProfile;
				let profileAdapter: MemberProfileDomainAdapter;
		...
		Scenario(
			'MemberProfileDomainAdapter getting and setting showLocation property',
			({ Given, When, Then }) => {
				let profileDoc: MemberProfile;
				let profileAdapter: MemberProfileDomainAdapter;
		...
		Scenario(
			'MemberProfileDomainAdapter getting and setting showProperties property',
			({ Given, When, Then }) => {
				let profileDoc: MemberProfile;
				let profileAdapter: MemberProfileDomainAdapter;
...
test.for(
	...
	({ Scenario, Background, BeforeEachScenario }) => {
		...
		Scenario(
			'Converting a Mongoose Member document to a domain object',
			({ Given, When, Then, And }) => {
				...
				And(
					'the domain object\'s cybersourceCustomerId should be "test-customer-id"',
					() => {
						expect(
							(
								result as Domain.Contexts.Community.Member.Member<MemberDomainAdapter>
							).cybersourceCustomerId,
						).toBe('test-customer-id');
					},
				);
				And(
				...
				And(
					"the domain object's role should be an EndUserRole domain object",
					() => {
						const { role } =
							result as Domain.Contexts.Community.Member.Member<MemberDomainAdapter>;
						expect(role).toBeInstanceOf(
							Domain.Contexts.Community.Role.EndUserRole.EndUserRole,
						);
					},
				);
				And(
					"the domain object's profile should be a MemberProfile domain object",
					() => {
						const { profile } =
							result as Domain.Contexts.Community.Member.Member<MemberDomainAdapter>;
		...
		Scenario(
			'Converting a domain object to a Mongoose Member document',
			({ Given, And, When, Then }) => {
				...
				And(
					'a Member domain object with memberName "New Member", cybersourceCustomerId "new-customer-id", and valid community, role, and profile',
					() => {
						communityAdapter = new CommunityDomainAdapter(communityDoc);
						roleAdapter = new EndUserRoleDomainAdapter(roleDoc);
						communityDomainObj =
						...
						adapter.community = communityDomainObj;
						adapter.role = roleDomainObj;
						domainObj = new Domain.Contexts.Community.Member.Member(
							adapter,
							passport,
						);
					},
				...
				And(
					"the document's role should be set to the correct role document",
					() => {
						expect(resultDoc.role).toBe(roleDoc);
					},
				);
				And(
					"the document's profile should be set to the correct profile document",
					() => {
