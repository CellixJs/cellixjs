...
import { MemberConverter, MemberDomainAdapter, MemberAccountDomainAdapter, MemberCustomViewDomainAdapter, MemberProfileDomainAdapter } from './member.domain-adapter.ts';
...
const domainAdapterFeature = await loadFeature(path.resolve(__dirname, 'features/member.domain-adapter.feature'));
const typeConverterFeature = await loadFeature(path.resolve(__dirname, 'features/member.type-converter.feature'));
