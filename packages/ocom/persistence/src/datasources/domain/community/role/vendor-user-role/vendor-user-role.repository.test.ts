...
import {
	...
	type VendorUserRoleDomainAdapter,
...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	BeforeEachScenario(() => {
		...
		const vendorUserRoleDoc = makeVendorUserRoleDoc({
		...
		Object.assign(ModelMock, {
			findById: vi.fn((id: string) => ({
				exec: vi.fn(async () =>
					id === '507f1f77bcf86cd799439011' ? vendorUserRoleDoc : null,
				),
			})),
			find: vi.fn(() => ({
	...
	Scenario(
		'Getting a vendor user role by id that does not exist',
		({ When, Then }) => {
			...
			Then(
				'an error should be thrown indicating "VendorUserRole with id nonexistent-id not found"',
				async () => {
					await expect(getByIdError()).rejects.toThrow(
						'VendorUserRole with id nonexistent-id not found',
					);
				},
			);
		},
	...
	Scenario(
		...
		({ Given, When, Then, And }) => {
			...
			When(
				'I call getNewInstance with roleName "New Vendor Role", isDefault false, and the community',
				async () => {
