...
const feature = await loadFeature(path.resolve(__dirname, 'features/end-user-role.repository.feature'));
...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	BeforeEachScenario(() => {
		...
		repository = new EndUserRoleRepository(passport, model, converter, eventBus, session);
		result = undefined;
	});
