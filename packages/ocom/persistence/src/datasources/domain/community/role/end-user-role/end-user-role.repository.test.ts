...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	BeforeEachScenario(() => {
		...
		Object.assign(ModelMock, {
			findById: vi.fn((id: string) => ({
				exec: vi.fn(async () =>
					id === '507f1f77bcf86cd799439011' ? endUserRoleDoc : null,
				),
			})),
			find: vi.fn(() => ({
	...
	Scenario(
		'Getting an end user role by id that does not exist',
		({ When, Then }) => {
			...
			Then(
				'an error should be thrown indicating "EndUserRole with id nonexistent-id not found"',
				async () => {
					await expect(getByIdError()).rejects.toThrow(
						'EndUserRole with id nonexistent-id not found',
					);
				},
			);
		},
	...
	Scenario(
		'Creating a new end user role instance',
		({ Given, When, Then, And }) => {
			...
			Given('a valid Community domain object as the community', () => {
				communityDomainObj = {
					id: communityDoc.id.toString(),
					name: 'Test Community',
				} as Domain.Contexts.Community.Community.CommunityEntityReference;
			});
			When(
				'I call getNewInstance with roleName "New Role", isDefault false, and the community',
				async () => {
	...
	Scenario(
		'Creating a new end user role instance with an invalid community',
		({ Given, When, Then }) => {
			...
			When(
				'I call getNewInstance with roleName "Invalid Role", isDefault true, and the invalid community',
				() => {
					getNewInstanceError = async () => {
						await repository.getNewInstance(
							'Invalid Role',
							true,
							invalidCommunity,
						);
