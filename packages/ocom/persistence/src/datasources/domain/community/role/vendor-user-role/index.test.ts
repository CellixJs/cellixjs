...
import type { VendorUserRoleModelType } from '@ocom/data-sources-mongoose-models';
...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	Scenario('Creating VendorUserRole Persistence', ({ When, Then, And }) => {
		...
		Then(
			'I should receive an object with VendorUserRoleUnitOfWork property',
			() => {
		...
		And('the VendorUserRoleUnitOfWork should be properly initialized', () => {
			...
			expect(result.VendorUserRoleUnitOfWork).toHaveProperty(
