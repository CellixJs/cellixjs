...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	BeforeEachScenario(() => {
		...
		result =
			{} as Domain.Contexts.Community.Role.EndUserRole.EndUserRoleUnitOfWork;
	});
	...
	Scenario('Creating an EndUserRole Unit of Work', ({ When, Then, And }) => {
		When(
			'I call getEndUserRoleUnitOfWork with the EndUserRole model and passport',
			() => {
		...
		Then(
			'I should receive a properly initialized EndUserRoleUnitOfWork',
			() => {
