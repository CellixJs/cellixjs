...
test.for(
	...
	({ Scenario, Background, BeforeEachScenario }) => {
		...
		Scenario('Accessing permissions property', ({ When, Then }) => {
			...
			Then(
				'I should get a VendorUserRolePermissionsDomainAdapter instance',
				() => {
		...
		Scenario('Accessing specific permission', ({ When, Then }) => {
			When(
				'I access permissions.communityPermissions.canManageVendorUserRolesAndPermissions',
				() => {
					result =
						adapter.permissions.communityPermissions
		...
		Scenario('Accessing property permissions', ({ When, Then }) => {
			When(
				'I access permissions.propertyPermissions.canManageProperties',
				() => {
		...
		Scenario('Accessing canManageCommunitySettings', ({ When, Then }) => {
			When(
				'I access permissions.communityPermissions.canManageCommunitySettings',
				() => {
					result =
						adapter.permissions.communityPermissions.canManageCommunitySettings;
				},
			);
			Then('I should get true', () => {
				expect(result).toBe(true);
		...
		Scenario('Accessing canEditOwnProperty', ({ When, Then }) => {
			When(
				'I access permissions.propertyPermissions.canEditOwnProperty',
				() => {
		...
		Scenario(
			'Accessing canCreateTickets for service tickets',
			({ When, Then }) => {
				When(
					'I access permissions.serviceTicketPermissions.canCreateTickets',
					() => {
						result =
							adapter.permissions.serviceTicketPermissions.canCreateTickets;
					},
				);
				Then('I should get true', () => {
					expect(result).toBe(true);
		...
		Scenario(
			'Setting canManageVendorUserRolesAndPermissions',
			({ When, Then }) => {
				When(
					'I set permissions.communityPermissions.canManageVendorUserRolesAndPermissions to false',
					() => {
						adapter.permissions.communityPermissions.canManageVendorUserRolesAndPermissions = false;
					},
				);
				Then(
					'permissions.communityPermissions.canManageVendorUserRolesAndPermissions should be false',
					() => {
						expect(
							adapter.permissions.communityPermissions
								.canManageVendorUserRolesAndPermissions,
		...
		Scenario('Accessing canManageSiteContent', ({ When, Then }) => {
			When(
				'I access permissions.communityPermissions.canManageSiteContent',
				() => {
					result =
						adapter.permissions.communityPermissions.canManageSiteContent;
				},
			);
			Then('I should get true', () => {
				expect(result).toBe(true);
		...
		Scenario(
			'Accessing canManageTickets for service tickets',
			({ When, Then }) => {
				When(
					'I access permissions.serviceTicketPermissions.canManageTickets',
					() => {
						result =
							adapter.permissions.serviceTicketPermissions.canManageTickets;
					},
				);
				Then('I should get true', () => {
					expect(result).toBe(true);
		...
		Scenario(
			'Accessing canCreateTickets for violation tickets',
			({ When, Then }) => {
				When(
					'I access permissions.violationTicketPermissions.canCreateTickets',
					() => {
						result =
							adapter.permissions.violationTicketPermissions.canCreateTickets;
					},
				);
				Then('I should get true', () => {
					expect(result).toBe(true);
		...
		Scenario('Setting canManageServices', ({ When, Then }) => {
			When(
				'I set permissions.servicePermissions.canManageServices to false',
				() => {
					adapter.permissions.servicePermissions.canManageServices = false;
				},
			);
			Then(
				'permissions.servicePermissions.canManageServices should be false',
				() => {
					expect(adapter.permissions.servicePermissions.canManageServices).toBe(
						false,
					);
				},
			);
		...
		Scenario(
			'Accessing canAssignTickets for service tickets',
			({ When, Then }) => {
				When(
					'I access permissions.serviceTicketPermissions.canAssignTickets',
					() => {
						result =
							adapter.permissions.serviceTicketPermissions.canAssignTickets;
					},
				);
				Then('I should get true', () => {
					expect(result).toBe(true);
		...
		Scenario(
			'Accessing canWorkOnTickets for service tickets',
			({ When, Then }) => {
				When(
					'I access permissions.serviceTicketPermissions.canWorkOnTickets',
					() => {
						result =
							adapter.permissions.serviceTicketPermissions.canWorkOnTickets;
					},
				);
				Then('I should get true', () => {
					expect(result).toBe(true);
		...
		Scenario(
			'Accessing canAssignTickets for violation tickets',
			({ When, Then }) => {
				When(
					'I access permissions.violationTicketPermissions.canAssignTickets',
					() => {
						result =
							adapter.permissions.violationTicketPermissions.canAssignTickets;
					},
				);
				Then('I should get true', () => {
					expect(result).toBe(true);
		...
		Scenario(
			'Accessing canWorkOnTickets for violation tickets',
			({ When, Then }) => {
				When(
					'I access permissions.violationTicketPermissions.canWorkOnTickets',
					() => {
						result =
							adapter.permissions.violationTicketPermissions.canWorkOnTickets;
					},
				);
				Then('I should get true', () => {
					expect(result).toBe(true);
		...
		Scenario(
			'Setting canAssignTickets for service tickets',
			({ When, Then }) => {
				When(
					'I set permissions.serviceTicketPermissions.canAssignTickets to false',
					() => {
						adapter.permissions.serviceTicketPermissions.canAssignTickets = false;
					},
				);
				Then(
					'permissions.serviceTicketPermissions.canAssignTickets should be false',
					() => {
						expect(
							adapter.permissions.serviceTicketPermissions.canAssignTickets,
						).toBe(false);
		...
		Scenario(
			'Setting canWorkOnTickets for service tickets',
			({ When, Then }) => {
				When(
					'I set permissions.serviceTicketPermissions.canWorkOnTickets to false',
					() => {
						adapter.permissions.serviceTicketPermissions.canWorkOnTickets = false;
					},
				);
				Then(
					'permissions.serviceTicketPermissions.canWorkOnTickets should be false',
					() => {
						expect(
							adapter.permissions.serviceTicketPermissions.canWorkOnTickets,
						).toBe(false);
		...
		Scenario(
			'Setting canAssignTickets for violation tickets',
			({ When, Then }) => {
				When(
					'I set permissions.violationTicketPermissions.canAssignTickets to false',
					() => {
						adapter.permissions.violationTicketPermissions.canAssignTickets = false;
					},
				);
				Then(
					'permissions.violationTicketPermissions.canAssignTickets should be false',
					() => {
						expect(
							adapter.permissions.violationTicketPermissions.canAssignTickets,
						).toBe(false);
		...
		Scenario(
			'Setting canWorkOnTickets for violation tickets',
			({ When, Then }) => {
				When(
					'I set permissions.violationTicketPermissions.canWorkOnTickets to false',
					() => {
						adapter.permissions.violationTicketPermissions.canWorkOnTickets = false;
					},
				);
				Then(
					'permissions.violationTicketPermissions.canWorkOnTickets should be false',
					() => {
						expect(
							adapter.permissions.violationTicketPermissions.canWorkOnTickets,
						).toBe(false);
		...
		Scenario('Setting canManageProperties', ({ When, Then }) => {
			When(
				'I set permissions.propertyPermissions.canManageProperties to false',
				() => {
					adapter.permissions.propertyPermissions.canManageProperties = false;
				},
			);
			Then(
				'permissions.propertyPermissions.canManageProperties should be false',
				() => {
					expect(
						adapter.permissions.propertyPermissions.canManageProperties,
					).toBe(false);
				},
			);
		});
		...
		Scenario('Setting canEditOwnProperty', ({ When, Then }) => {
			When(
				'I set permissions.propertyPermissions.canEditOwnProperty to false',
				() => {
		...
		Scenario('Setting canManageCommunitySettings', ({ When, Then }) => {
			When(
				'I set permissions.communityPermissions.canManageCommunitySettings to false',
				() => {
					adapter.permissions.communityPermissions.canManageCommunitySettings = false;
				},
			);
			Then(
				'permissions.communityPermissions.canManageCommunitySettings should be false',
				() => {
					expect(
						adapter.permissions.communityPermissions.canManageCommunitySettings,
					).toBe(false);
		...
		Scenario('Setting canManageSiteContent', ({ When, Then }) => {
			When(
				'I set permissions.communityPermissions.canManageSiteContent to false',
				() => {
					adapter.permissions.communityPermissions.canManageSiteContent = false;
				},
			);
			Then(
				'permissions.communityPermissions.canManageSiteContent should be false',
				() => {
					expect(
						adapter.permissions.communityPermissions.canManageSiteContent,
					).toBe(false);
		...
		Scenario('Setting canManageMembers', ({ When, Then }) => {
			When(
				'I set permissions.communityPermissions.canManageMembers to false',
				() => {
					adapter.permissions.communityPermissions.canManageMembers = false;
				},
			);
			Then(
				'permissions.communityPermissions.canManageMembers should be false',
				() => {
					expect(
						adapter.permissions.communityPermissions.canManageMembers,
					).toBe(false);
				},
			);
		});
		...
		Scenario('Setting canEditOwnMemberProfile', ({ When, Then }) => {
			When(
				'I set permissions.communityPermissions.canEditOwnMemberProfile to false',
				() => {
					adapter.permissions.communityPermissions.canEditOwnMemberProfile = false;
				},
			);
			Then(
				'permissions.communityPermissions.canEditOwnMemberProfile should be false',
				() => {
					expect(
						adapter.permissions.communityPermissions.canEditOwnMemberProfile,
					).toBe(false);
				},
			);
		});
		...
		Scenario('Setting canEditOwnMemberAccounts', ({ When, Then }) => {
			When(
				'I set permissions.communityPermissions.canEditOwnMemberAccounts to false',
				() => {
					adapter.permissions.communityPermissions.canEditOwnMemberAccounts = false;
				},
			);
			Then(
				'permissions.communityPermissions.canEditOwnMemberAccounts should be false',
				() => {
					expect(
						adapter.permissions.communityPermissions.canEditOwnMemberAccounts,
					).toBe(false);
				},
			);
		});
...
test.for(
	...
	({ Scenario, Background, BeforeEachScenario }) => {
		...
		Scenario(
			'Converting a Mongoose VendorUserRole document to a domain object',
			({ Given, When, Then, And }) => {
				Given('a VendorUserRoleConverter instance', () => {
					converter = new VendorUserRoleConverter();
				});
				When(
					'I call toDomain with the Mongoose VendorUserRole document',
					() => {
				...
				Then('I should receive a VendorUserRole domain object', () => {
					expect(result).toBeInstanceOf(
						Domain.Contexts.Community.Role.VendorUserRole.VendorUserRole,
					);
				});
				And(
					'the domain object\'s roleName should be "Test Vendor Role"',
					() => {
						expect(
							(
								result as Domain.Contexts.Community.Role.VendorUserRole.VendorUserRole<VendorUserRoleDomainAdapter>
							).roleName,
						).toBe('Test Vendor Role');
					},
				);
		...
		Scenario(
			'Converting a domain object to a Mongoose VendorUserRole document',
			({ Given, And, When, Then }) => {
				...
				And(
					'a VendorUserRole domain object with roleName "New Role", isDefault false, and valid community',
					() => {
						...
						domainObj =
							new Domain.Contexts.Community.Role.VendorUserRole.VendorUserRole(
								adapter,
								passport,
							);
					},
				...
				When(
					'I call toPersistence with the VendorUserRole domain object',
					() => {
