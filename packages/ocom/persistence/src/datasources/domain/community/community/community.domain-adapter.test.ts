...
} from './community.domain-adapter.ts';
...
test.for(
	...
	({ Scenario, Background, BeforeEachScenario }) => {
		...
		Scenario(
			'Getting and setting the domain property',
			({ Given, When, Then }) => {
				Given('a CommunityDomainAdapter for the document', () => {
					adapter = new CommunityDomainAdapter(doc);
				});
		...
		Scenario(
			'Getting and setting the whiteLabelDomain property',
			({ Given, When, Then }) => {
				Given('a CommunityDomainAdapter for the document', () => {
					adapter = new CommunityDomainAdapter(doc);
				});
				...
				Then(
					'the document\'s whiteLabelDomain should be "newwhite.com"',
					() => {
		...
		Scenario(
			'Getting and setting the handle property',
			({ Given, When, Then }) => {
				Given('a CommunityDomainAdapter for the document', () => {
					adapter = new CommunityDomainAdapter(doc);
				});
		...
		Scenario(
			'Getting the createdBy property when populated',
			({ Given, When, Then }) => {
				Given('a CommunityDomainAdapter for the document', () => {
					adapter = new CommunityDomainAdapter(doc);
				});
		...
		Scenario(
			'Getting the createdBy property when not populated',
			({ Given, When, Then }) => {
				let gettingCreatingByWhenNotPopulated: () => void;
				Given(
					'a CommunityDomainAdapter for a document with no createdBy',
					() => {
				...
				Then(
					'an error should be thrown indicating "createdBy is not populated"',
					() => {
						expect(gettingCreatingByWhenNotPopulated).toThrow();
						expect(gettingCreatingByWhenNotPopulated).throws(
		...
		Scenario(
			'Getting the createdBy property when it is an ObjectId',
			({ Given, When, Then }) => {
				let gettingCreatedByWhenObjectId: () => void;
				Given(
					'a CommunityDomainAdapter for a document with createdBy as an ObjectId',
					() => {
						doc = makeCommunityDoc({
							createdBy: new MongooseSeedwork.ObjectId(),
						});
						adapter = new CommunityDomainAdapter(doc);
					},
				);
				...
				Then(
					'an error should be thrown indicating "createdBy is not populated or is not of the correct type"',
					() => {
						expect(gettingCreatedByWhenObjectId).toThrow();
						expect(gettingCreatedByWhenObjectId).throws(
		...
		Scenario(
			'Setting the createdBy property with a valid EndUserDomainAdapter',
			({ Given, And, When, Then }) => {
				Given('a CommunityDomainAdapter for the document', () => {
					adapter = new CommunityDomainAdapter(doc);
				});
		...
		Scenario(
			'Setting the createdBy property with an invalid value',
			({ Given, And, When, Then }) => {
				...
				When(
					'I try to set the createdBy property to the invalid object',
					() => {
				...
				Then(
					'an error should be thrown indicating "user is not an instance of EndUserDomainAdapter"',
					() => {
						expect(settingCreatedByWithInvalidValue).toThrow();
						expect(settingCreatedByWithInvalidValue).throws(
							/user reference is missing id/,
						);
					},
				);
			},
...
test.for(
	...
	({ Scenario, Background, BeforeEachScenario }) => {
		...
		Scenario(
			'Converting a Mongoose Community document to a domain object',
			({ Given, When, Then, And }) => {
				...
				And(
					'the domain object\'s whiteLabelDomain should be "white.test.com"',
					() => {
						expect(
							(
								result as Domain.Contexts.Community.Community.Community<CommunityDomainAdapter>
				...
				And(
					"the domain object's createdBy should be an EndUser domain object with the correct user data",
					() => {
						const { createdBy } =
							result as Domain.Contexts.Community.Community.Community<CommunityDomainAdapter>;
						expect(createdBy).toBeInstanceOf(
							Domain.Contexts.User.EndUser.EndUser,
						);
		...
		Scenario(
			'Converting a domain object to a Mongoose Community document',
			({ Given, And, When, Then }) => {
				...
				And(
					'a Community domain object with name "New Community", domain "new.com", whiteLabelDomain "newwhite.com", handle "new-handle", and a valid createdBy',
					() => {
						userDoc = makeUserDoc();
						userAdapter = new EndUserDomainAdapter(userDoc);
						...
						domainObj = new Domain.Contexts.Community.Community.Community(
							adapter,
							passport,
						);
					},
				...
				And(
					"the document's createdBy should be set to the correct user document",
					() => {
