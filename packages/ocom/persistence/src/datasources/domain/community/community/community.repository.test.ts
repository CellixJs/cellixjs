...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	BeforeEachScenario(() => {
		...
		result =
			{} as Domain.Contexts.Community.Community.Community<CommunityDomainAdapter>;
	...
	Scenario(
		'Getting a community by id with createdBy populated',
		({ When, Then, And }) => {
	...
	Scenario(
		'Getting a community by id that does not exist',
		({ When, Then }) => {
			let gettingCommunityThatDoesNotExist: () => Promise<
				Domain.Contexts.Community.Community.Community<CommunityDomainAdapter>
			>;
			When('I call getByIdWithCreatedBy with "nonexistent-id"', () => {
				gettingCommunityThatDoesNotExist = async () =>
					await repo.getByIdWithCreatedBy('nonexistent-id');
			});
			Then(
				'an error should be thrown indicating "Community with id nonexistent-id not found"',
				async () => {
					await expect(gettingCommunityThatDoesNotExist).rejects.toThrow();
					await expect(gettingCommunityThatDoesNotExist).rejects.toThrow(
						/Community with id nonexistent-id not found/,
					);
				},
			);
		},
	...
	Scenario(
		...
		({ Given, When, Then, And }) => {
			...
			Given('a valid EndUser domain object as the user', () => {
				userDoc = makeUserDoc();
				userAdapter = new EndUserDomainAdapter(userDoc);
				userDomainObject = new Domain.Contexts.User.EndUser.EndUser(
					userAdapter,
					passport,
				);
			});
			When(
				'I call getNewInstance with name "New Community" and the user',
				async () => {
	...
	Scenario(
		'Creating a new community instance with an invalid user',
		({ Given, When, Then }) => {
			...
			When(
				'I call getNewInstance with name "Invalid Community" and the invalid user',
				() => {
					getNewInstanceWithInvalidUser = () =>
						repo.getNewInstance(
							'Invalid Community',
							invalidUser as Domain.Contexts.User.EndUser.EndUserEntityReference,
						);
				},
			);
			Then(
				'an error should be thrown indicating the user is not valid',
				async () => {
