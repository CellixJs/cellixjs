...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	BeforeEachScenario(() => {
		...
		vi.mocked(DomainDataSourceImplementation).mockReturnValue(
			mockDataSources.domainDataSource,
		);
		vi.mocked(ReadonlyDataSourceImplementation).mockReturnValue(
			mockDataSources.readonlyDataSource,
		);
	...
	Scenario(
		'Creating Data Sources Factory Implementation',
		({ When, Then, And }) => {
			...
			And(
				'the DataSourcesFactory should have withSystemPassport method',
				() => {
	...
	Scenario('Using withPassport method', ({ When, Then, And }) => {
		...
		And('the DataSources should have readonlyDataSource property', () => {
			...
			expect(result.readonlyDataSource).toBe(
				mockDataSources.readonlyDataSource,
			);
		});
	});
