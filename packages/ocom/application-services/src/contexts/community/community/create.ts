...
export const create = (dataSources: DataSources) => {
	return async (command: CommunityCreateCommand): Promise<CommunityEntityReference> => {
		const createdBy = await dataSources.readonlyDataSource.User.EndUser.EndUserReadRepo.getByExternalId(
			command.endUserExternalId,
		);
		if (!createdBy) {
