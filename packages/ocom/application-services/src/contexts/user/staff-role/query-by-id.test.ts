...
const feature = await loadFeature(path.resolve(__dirname, 'features/query-by-id.feature'));
...
test.for(feature, ({ Scenario, BeforeEachScenario }) => {
	...
	let queryStaffRoleById: (command: { roleId: string }) => Promise<Domain.Contexts.User.StaffRole.StaffRoleEntityReference | null>;
	...
	Scenario('Querying a staff role by ID that does not exist', ({ Given, When, Then }) => {
		...
		Scenario('Querying a logically deleted staff role by ID', ({ Given, When, Then }) => {
			let result: Domain.Contexts.User.StaffRole.StaffRoleEntityReference | null;
