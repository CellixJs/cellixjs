...
export const queryByRoleName = (dataSources: DataSources) => {
	return async (command: StaffRoleQueryByRoleNameCommand): Promise<Domain.Contexts.User.StaffRole.StaffRoleEntityReference | null> => {
		...
		try {
			await dataSources.domainDataSource.User.StaffRole.StaffRoleUnitOfWork.withScopedTransaction(async (repository) => {
				staffRole = await repository.getByRoleName(command.roleName);
			});
		} catch (error) {
