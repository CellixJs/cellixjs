...
export const createIfNotExists = (dataSources: DataSources) => {
	return async (command: EndUserCreateCommand): Promise<Domain.Contexts.User.EndUser.EndUserEntityReference> => {
		...
		await dataSources.domainDataSource.User.EndUser.EndUserUnitOfWork.withScopedTransaction(async (repo) => {
