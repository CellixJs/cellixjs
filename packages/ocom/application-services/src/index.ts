...
import { TechAdmin, type TechAdminContextApplicationService } from './contexts/tech-admin/index.ts';
...
export interface ApplicationServices {
	...
	TechAdmin: TechAdminContextApplicationService;
...
export const buildApplicationServicesFactory = (context: ApiContextSpec): ApplicationServicesFactory => {
	const forRequest = async (rawAuthHeader?: string, hints?: PrincipalHints): Promise<ApplicationServices> => {
		...
		return {
			...
			TechAdmin: TechAdmin(dataSources, queueStorageService, tokenValidationResult?.verifiedJwt.sub),
