...
const feature = await loadFeature(path.resolve(__dirname, 'features/service-apollo-server.feature'));
...
test.for(feature, ({ Scenario, BeforeEachScenario }) => {
	...
	BeforeEachScenario(() => {
		...
		vi.mocked(ApolloServer).mockImplementation(function MockApolloServer() {
			return mockApolloServer as unknown as ApolloServer;
		});
	...
	Scenario('Accessing server before startup', ({ Given, When, Then }) => {
		...
		When('the server property is accessed', () => {
			expect(() => service.server).toThrow('ServiceApolloServer is not started - cannot access server');
		});
	...
	Scenario('Shutting down before startup', ({ Given, When, Then }) => {
		...
		When('shutDown is called', async () => {
			await expect(service.shutDown()).rejects.toThrow('ServiceApolloServer is not started - shutdown cannot proceed');
		});
