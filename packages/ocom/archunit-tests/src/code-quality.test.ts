...
describe('Code Quality', () => {
	describe('Domain (ocom/domain)', () => {
		it.skip('should have reasonable cohesion (LCOM96b)', async () => {
			...
			const rule = metrics(tsconfigPath).inPath('../domain/src/**').lcom().lcom96b().shouldBeBelowOrEqual(1.0);

			await expect(rule).toPassAsync();
		}, 10000);

		it.skip('should avoid excessive methods per class', async () => {
			const rule = metrics(tsconfigPath).inPath('../domain/src/**').count().methodCount().shouldBeBelow(30);
	...
	describe('UI (ui packages)', () => {
		it.skip('should have higher cohesion (LCOM96b)', async () => {
			const rule = metrics(tsconfigPath).inPath('../ui-shared/src/**').inPath('../../../apps/ui-community/src/**').lcom().lcom96b().shouldBeBelow(0.85);
		...
		it.skip('should limit imports and surface area in UI code', async () => {
			const rule = metrics(tsconfigPath).inPath('../ui-shared/src/**').inPath('../../../apps/ui-community/src/**').count().imports().shouldBeBelowOrEqual(20);
	...
	describe('Service / Infrastructure', () => {
		it.skip('should have reasonable cohesion (LCOM96b)', async () => {
			const rule = metrics(tsconfigPath).inPath('../service-*/src/**').lcom().lcom96b().shouldBeBelow(0.95);

			await expect(rule).toPassAsync();
		});

		it.skip('should limit imports per file for services', async () => {
			const rule = metrics(tsconfigPath).inPath('../service-*/src/**').count().imports().shouldBeBelowOrEqual(25);
	...
	describe('Global / Cross-cutting checks', () => {
		it.skip('baseline cohesion for all src files (lenient)', async () => {
			...
			const rule = metrics(tsconfigPath).inPath('../**/src/**').inPath('../../../apps/**/src/**').lcom().lcom96b().shouldBeBelow(0.98);
			await expect(rule).toPassAsync({ allowEmptyTests: true });
		});
