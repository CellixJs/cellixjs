/**
 * Countries and their first-level subdivisions offered by the address
 * dropdowns of the shared property form.
 *
 * Shape mirrors the sibling-repo `countries.json` asset
 * (`{countryName, countryCode, states: [{stateName, stateCode}] | null}`;
 * only the US and Canada carry `states`). Kept as a TypeScript module so
 * every consumer's module mode (bundler and NodeNext) compiles it without
 * JSON import attributes.
 */

/** A first-level country subdivision (US state, Canadian province or territory). */
interface CountryState {
	readonly stateName: string;
	readonly stateCode: string;
}

/** A country option: display name, ISO 3166-1 alpha-2 code, and optional subdivisions. */
interface Country {
	readonly countryName: string;
	readonly countryCode: string;
	readonly states: ReadonlyArray<CountryState> | null;
}

export const COUNTRIES: ReadonlyArray<Country> = [
	{
		countryName: 'Afghanistan',
		countryCode: 'AF',
		states: null,
	},
	{
		countryName: 'Åland Islands',
		countryCode: 'AX',
		states: null,
	},
	{
		countryName: 'Albania',
		countryCode: 'AL',
		states: null,
	},
	{
		countryName: 'Algeria',
		countryCode: 'DZ',
		states: null,
	},
	{
		countryName: 'American Samoa',
		countryCode: 'AS',
		states: null,
	},
	{
		countryName: 'Andorra',
		countryCode: 'AD',
		states: null,
	},
	{
		countryName: 'Angola',
		countryCode: 'AO',
		states: null,
	},
	{
		countryName: 'Anguilla',
		countryCode: 'AI',
		states: null,
	},
	{
		countryName: 'Antarctica',
		countryCode: 'AQ',
		states: null,
	},
	{
		countryName: 'Antigua & Barbuda',
		countryCode: 'AG',
		states: null,
	},
	{
		countryName: 'Argentina',
		countryCode: 'AR',
		states: null,
	},
	{
		countryName: 'Armenia',
		countryCode: 'AM',
		states: null,
	},
	{
		countryName: 'Aruba',
		countryCode: 'AW',
		states: null,
	},
	{
		countryName: 'Australia',
		countryCode: 'AU',
		states: null,
	},
	{
		countryName: 'Austria',
		countryCode: 'AT',
		states: null,
	},
	{
		countryName: 'Azerbaijan',
		countryCode: 'AZ',
		states: null,
	},
	{
		countryName: 'Bahamas',
		countryCode: 'BS',
		states: null,
	},
	{
		countryName: 'Bahrain',
		countryCode: 'BH',
		states: null,
	},
	{
		countryName: 'Bangladesh',
		countryCode: 'BD',
		states: null,
	},
	{
		countryName: 'Barbados',
		countryCode: 'BB',
		states: null,
	},
	{
		countryName: 'Belarus',
		countryCode: 'BY',
		states: null,
	},
	{
		countryName: 'Belgium',
		countryCode: 'BE',
		states: null,
	},
	{
		countryName: 'Belize',
		countryCode: 'BZ',
		states: null,
	},
	{
		countryName: 'Benin',
		countryCode: 'BJ',
		states: null,
	},
	{
		countryName: 'Bermuda',
		countryCode: 'BM',
		states: null,
	},
	{
		countryName: 'Bhutan',
		countryCode: 'BT',
		states: null,
	},
	{
		countryName: 'Bolivia',
		countryCode: 'BO',
		states: null,
	},
	{
		countryName: 'Bosnia & Herzegovina',
		countryCode: 'BA',
		states: null,
	},
	{
		countryName: 'Botswana',
		countryCode: 'BW',
		states: null,
	},
	{
		countryName: 'Bouvet Island',
		countryCode: 'BV',
		states: null,
	},
	{
		countryName: 'Brazil',
		countryCode: 'BR',
		states: null,
	},
	{
		countryName: 'British Indian Ocean Territory',
		countryCode: 'IO',
		states: null,
	},
	{
		countryName: 'British Virgin Islands',
		countryCode: 'VG',
		states: null,
	},
	{
		countryName: 'Brunei',
		countryCode: 'BN',
		states: null,
	},
	{
		countryName: 'Bulgaria',
		countryCode: 'BG',
		states: null,
	},
	{
		countryName: 'Burkina Faso',
		countryCode: 'BF',
		states: null,
	},
	{
		countryName: 'Burundi',
		countryCode: 'BI',
		states: null,
	},
	{
		countryName: 'Cambodia',
		countryCode: 'KH',
		states: null,
	},
	{
		countryName: 'Cameroon',
		countryCode: 'CM',
		states: null,
	},
	{
		countryName: 'Canada',
		countryCode: 'CA',
		states: [
			{
				stateName: 'Alberta',
				stateCode: 'AB',
			},
			{
				stateName: 'British Columbia',
				stateCode: 'BC',
			},
			{
				stateName: 'Manitoba',
				stateCode: 'MB',
			},
			{
				stateName: 'New Brunswick',
				stateCode: 'NB',
			},
			{
				stateName: 'Newfoundland and Labrador',
				stateCode: 'NL',
			},
			{
				stateName: 'Northwest Territories',
				stateCode: 'NT',
			},
			{
				stateName: 'Nova Scotia',
				stateCode: 'NS',
			},
			{
				stateName: 'Nunavut',
				stateCode: 'NU',
			},
			{
				stateName: 'Ontario',
				stateCode: 'ON',
			},
			{
				stateName: 'Prince Edward Island',
				stateCode: 'PE',
			},
			{
				stateName: 'Quebec',
				stateCode: 'QC',
			},
			{
				stateName: 'Saskatchewan',
				stateCode: 'SK',
			},
			{
				stateName: 'Yukon',
				stateCode: 'YT',
			},
		],
	},
	{
		countryName: 'Cape Verde',
		countryCode: 'CV',
		states: null,
	},
	{
		countryName: 'Caribbean Netherlands',
		countryCode: 'BQ',
		states: null,
	},
	{
		countryName: 'Cayman Islands',
		countryCode: 'KY',
		states: null,
	},
	{
		countryName: 'Central African Republic',
		countryCode: 'CF',
		states: null,
	},
	{
		countryName: 'Chad',
		countryCode: 'TD',
		states: null,
	},
	{
		countryName: 'Chile',
		countryCode: 'CL',
		states: null,
	},
	{
		countryName: 'China',
		countryCode: 'CN',
		states: null,
	},
	{
		countryName: 'Christmas Island',
		countryCode: 'CX',
		states: null,
	},
	{
		countryName: 'Cocos (Keeling) Islands',
		countryCode: 'CC',
		states: null,
	},
	{
		countryName: 'Colombia',
		countryCode: 'CO',
		states: null,
	},
	{
		countryName: 'Comoros',
		countryCode: 'KM',
		states: null,
	},
	{
		countryName: 'Congo - Brazzaville',
		countryCode: 'CG',
		states: null,
	},
	{
		countryName: 'Congo - Kinshasa',
		countryCode: 'CD',
		states: null,
	},
	{
		countryName: 'Cook Islands',
		countryCode: 'CK',
		states: null,
	},
	{
		countryName: 'Costa Rica',
		countryCode: 'CR',
		states: null,
	},
	{
		countryName: 'Côte d’Ivoire',
		countryCode: 'CI',
		states: null,
	},
	{
		countryName: 'Croatia',
		countryCode: 'HR',
		states: null,
	},
	{
		countryName: 'Cuba',
		countryCode: 'CU',
		states: null,
	},
	{
		countryName: 'Curaçao',
		countryCode: 'CW',
		states: null,
	},
	{
		countryName: 'Cyprus',
		countryCode: 'CY',
		states: null,
	},
	{
		countryName: 'Czechia',
		countryCode: 'CZ',
		states: null,
	},
	{
		countryName: 'Denmark',
		countryCode: 'DK',
		states: null,
	},
	{
		countryName: 'Djibouti',
		countryCode: 'DJ',
		states: null,
	},
	{
		countryName: 'Dominica',
		countryCode: 'DM',
		states: null,
	},
	{
		countryName: 'Dominican Republic',
		countryCode: 'DO',
		states: null,
	},
	{
		countryName: 'Ecuador',
		countryCode: 'EC',
		states: null,
	},
	{
		countryName: 'Egypt',
		countryCode: 'EG',
		states: null,
	},
	{
		countryName: 'El Salvador',
		countryCode: 'SV',
		states: null,
	},
	{
		countryName: 'Equatorial Guinea',
		countryCode: 'GQ',
		states: null,
	},
	{
		countryName: 'Eritrea',
		countryCode: 'ER',
		states: null,
	},
	{
		countryName: 'Estonia',
		countryCode: 'EE',
		states: null,
	},
	{
		countryName: 'Eswatini',
		countryCode: 'SZ',
		states: null,
	},
	{
		countryName: 'Ethiopia',
		countryCode: 'ET',
		states: null,
	},
	{
		countryName: 'Falkland Islands',
		countryCode: 'FK',
		states: null,
	},
	{
		countryName: 'Faroe Islands',
		countryCode: 'FO',
		states: null,
	},
	{
		countryName: 'Fiji',
		countryCode: 'FJ',
		states: null,
	},
	{
		countryName: 'Finland',
		countryCode: 'FI',
		states: null,
	},
	{
		countryName: 'France',
		countryCode: 'FR',
		states: null,
	},
	{
		countryName: 'French Guiana',
		countryCode: 'GF',
		states: null,
	},
	{
		countryName: 'French Polynesia',
		countryCode: 'PF',
		states: null,
	},
	{
		countryName: 'French Southern Territories',
		countryCode: 'TF',
		states: null,
	},
	{
		countryName: 'Gabon',
		countryCode: 'GA',
		states: null,
	},
	{
		countryName: 'Gambia',
		countryCode: 'GM',
		states: null,
	},
	{
		countryName: 'Georgia',
		countryCode: 'GE',
		states: null,
	},
	{
		countryName: 'Germany',
		countryCode: 'DE',
		states: null,
	},
	{
		countryName: 'Ghana',
		countryCode: 'GH',
		states: null,
	},
	{
		countryName: 'Gibraltar',
		countryCode: 'GI',
		states: null,
	},
	{
		countryName: 'Greece',
		countryCode: 'GR',
		states: null,
	},
	{
		countryName: 'Greenland',
		countryCode: 'GL',
		states: null,
	},
	{
		countryName: 'Grenada',
		countryCode: 'GD',
		states: null,
	},
	{
		countryName: 'Guadeloupe',
		countryCode: 'GP',
		states: null,
	},
	{
		countryName: 'Guam',
		countryCode: 'GU',
		states: null,
	},
	{
		countryName: 'Guatemala',
		countryCode: 'GT',
		states: null,
	},
	{
		countryName: 'Guernsey',
		countryCode: 'GG',
		states: null,
	},
	{
		countryName: 'Guinea',
		countryCode: 'GN',
		states: null,
	},
	{
		countryName: 'Guinea-Bissau',
		countryCode: 'GW',
		states: null,
	},
	{
		countryName: 'Guyana',
		countryCode: 'GY',
		states: null,
	},
	{
		countryName: 'Haiti',
		countryCode: 'HT',
		states: null,
	},
	{
		countryName: 'Heard & McDonald Islands',
		countryCode: 'HM',
		states: null,
	},
	{
		countryName: 'Honduras',
		countryCode: 'HN',
		states: null,
	},
	{
		countryName: 'Hong Kong SAR China',
		countryCode: 'HK',
		states: null,
	},
	{
		countryName: 'Hungary',
		countryCode: 'HU',
		states: null,
	},
	{
		countryName: 'Iceland',
		countryCode: 'IS',
		states: null,
	},
	{
		countryName: 'India',
		countryCode: 'IN',
		states: null,
	},
	{
		countryName: 'Indonesia',
		countryCode: 'ID',
		states: null,
	},
	{
		countryName: 'Iran',
		countryCode: 'IR',
		states: null,
	},
	{
		countryName: 'Iraq',
		countryCode: 'IQ',
		states: null,
	},
	{
		countryName: 'Ireland',
		countryCode: 'IE',
		states: null,
	},
	{
		countryName: 'Isle of Man',
		countryCode: 'IM',
		states: null,
	},
	{
		countryName: 'Israel',
		countryCode: 'IL',
		states: null,
	},
	{
		countryName: 'Italy',
		countryCode: 'IT',
		states: null,
	},
	{
		countryName: 'Jamaica',
		countryCode: 'JM',
		states: null,
	},
	{
		countryName: 'Japan',
		countryCode: 'JP',
		states: null,
	},
	{
		countryName: 'Jersey',
		countryCode: 'JE',
		states: null,
	},
	{
		countryName: 'Jordan',
		countryCode: 'JO',
		states: null,
	},
	{
		countryName: 'Kazakhstan',
		countryCode: 'KZ',
		states: null,
	},
	{
		countryName: 'Kenya',
		countryCode: 'KE',
		states: null,
	},
	{
		countryName: 'Kiribati',
		countryCode: 'KI',
		states: null,
	},
	{
		countryName: 'Kuwait',
		countryCode: 'KW',
		states: null,
	},
	{
		countryName: 'Kyrgyzstan',
		countryCode: 'KG',
		states: null,
	},
	{
		countryName: 'Laos',
		countryCode: 'LA',
		states: null,
	},
	{
		countryName: 'Latvia',
		countryCode: 'LV',
		states: null,
	},
	{
		countryName: 'Lebanon',
		countryCode: 'LB',
		states: null,
	},
	{
		countryName: 'Lesotho',
		countryCode: 'LS',
		states: null,
	},
	{
		countryName: 'Liberia',
		countryCode: 'LR',
		states: null,
	},
	{
		countryName: 'Libya',
		countryCode: 'LY',
		states: null,
	},
	{
		countryName: 'Liechtenstein',
		countryCode: 'LI',
		states: null,
	},
	{
		countryName: 'Lithuania',
		countryCode: 'LT',
		states: null,
	},
	{
		countryName: 'Luxembourg',
		countryCode: 'LU',
		states: null,
	},
	{
		countryName: 'Macao SAR China',
		countryCode: 'MO',
		states: null,
	},
	{
		countryName: 'Madagascar',
		countryCode: 'MG',
		states: null,
	},
	{
		countryName: 'Malawi',
		countryCode: 'MW',
		states: null,
	},
	{
		countryName: 'Malaysia',
		countryCode: 'MY',
		states: null,
	},
	{
		countryName: 'Maldives',
		countryCode: 'MV',
		states: null,
	},
	{
		countryName: 'Mali',
		countryCode: 'ML',
		states: null,
	},
	{
		countryName: 'Malta',
		countryCode: 'MT',
		states: null,
	},
	{
		countryName: 'Marshall Islands',
		countryCode: 'MH',
		states: null,
	},
	{
		countryName: 'Martinique',
		countryCode: 'MQ',
		states: null,
	},
	{
		countryName: 'Mauritania',
		countryCode: 'MR',
		states: null,
	},
	{
		countryName: 'Mauritius',
		countryCode: 'MU',
		states: null,
	},
	{
		countryName: 'Mayotte',
		countryCode: 'YT',
		states: null,
	},
	{
		countryName: 'Mexico',
		countryCode: 'MX',
		states: null,
	},
	{
		countryName: 'Micronesia',
		countryCode: 'FM',
		states: null,
	},
	{
		countryName: 'Moldova',
		countryCode: 'MD',
		states: null,
	},
	{
		countryName: 'Monaco',
		countryCode: 'MC',
		states: null,
	},
	{
		countryName: 'Mongolia',
		countryCode: 'MN',
		states: null,
	},
	{
		countryName: 'Montenegro',
		countryCode: 'ME',
		states: null,
	},
	{
		countryName: 'Montserrat',
		countryCode: 'MS',
		states: null,
	},
	{
		countryName: 'Morocco',
		countryCode: 'MA',
		states: null,
	},
	{
		countryName: 'Mozambique',
		countryCode: 'MZ',
		states: null,
	},
	{
		countryName: 'Myanmar (Burma)',
		countryCode: 'MM',
		states: null,
	},
	{
		countryName: 'Namibia',
		countryCode: 'NA',
		states: null,
	},
	{
		countryName: 'Nauru',
		countryCode: 'NR',
		states: null,
	},
	{
		countryName: 'Nepal',
		countryCode: 'NP',
		states: null,
	},
	{
		countryName: 'Netherlands',
		countryCode: 'NL',
		states: null,
	},
	{
		countryName: 'New Caledonia',
		countryCode: 'NC',
		states: null,
	},
	{
		countryName: 'New Zealand',
		countryCode: 'NZ',
		states: null,
	},
	{
		countryName: 'Nicaragua',
		countryCode: 'NI',
		states: null,
	},
	{
		countryName: 'Niger',
		countryCode: 'NE',
		states: null,
	},
	{
		countryName: 'Nigeria',
		countryCode: 'NG',
		states: null,
	},
	{
		countryName: 'Niue',
		countryCode: 'NU',
		states: null,
	},
	{
		countryName: 'Norfolk Island',
		countryCode: 'NF',
		states: null,
	},
	{
		countryName: 'North Korea',
		countryCode: 'KP',
		states: null,
	},
	{
		countryName: 'North Macedonia',
		countryCode: 'MK',
		states: null,
	},
	{
		countryName: 'Northern Mariana Islands',
		countryCode: 'MP',
		states: null,
	},
	{
		countryName: 'Norway',
		countryCode: 'NO',
		states: null,
	},
	{
		countryName: 'Oman',
		countryCode: 'OM',
		states: null,
	},
	{
		countryName: 'Pakistan',
		countryCode: 'PK',
		states: null,
	},
	{
		countryName: 'Palau',
		countryCode: 'PW',
		states: null,
	},
	{
		countryName: 'Palestinian Territories',
		countryCode: 'PS',
		states: null,
	},
	{
		countryName: 'Panama',
		countryCode: 'PA',
		states: null,
	},
	{
		countryName: 'Papua New Guinea',
		countryCode: 'PG',
		states: null,
	},
	{
		countryName: 'Paraguay',
		countryCode: 'PY',
		states: null,
	},
	{
		countryName: 'Peru',
		countryCode: 'PE',
		states: null,
	},
	{
		countryName: 'Philippines',
		countryCode: 'PH',
		states: null,
	},
	{
		countryName: 'Pitcairn Islands',
		countryCode: 'PN',
		states: null,
	},
	{
		countryName: 'Poland',
		countryCode: 'PL',
		states: null,
	},
	{
		countryName: 'Portugal',
		countryCode: 'PT',
		states: null,
	},
	{
		countryName: 'Puerto Rico',
		countryCode: 'PR',
		states: null,
	},
	{
		countryName: 'Qatar',
		countryCode: 'QA',
		states: null,
	},
	{
		countryName: 'Réunion',
		countryCode: 'RE',
		states: null,
	},
	{
		countryName: 'Romania',
		countryCode: 'RO',
		states: null,
	},
	{
		countryName: 'Russia',
		countryCode: 'RU',
		states: null,
	},
	{
		countryName: 'Rwanda',
		countryCode: 'RW',
		states: null,
	},
	{
		countryName: 'Samoa',
		countryCode: 'WS',
		states: null,
	},
	{
		countryName: 'San Marino',
		countryCode: 'SM',
		states: null,
	},
	{
		countryName: 'São Tomé & Príncipe',
		countryCode: 'ST',
		states: null,
	},
	{
		countryName: 'Saudi Arabia',
		countryCode: 'SA',
		states: null,
	},
	{
		countryName: 'Senegal',
		countryCode: 'SN',
		states: null,
	},
	{
		countryName: 'Serbia',
		countryCode: 'RS',
		states: null,
	},
	{
		countryName: 'Seychelles',
		countryCode: 'SC',
		states: null,
	},
	{
		countryName: 'Sierra Leone',
		countryCode: 'SL',
		states: null,
	},
	{
		countryName: 'Singapore',
		countryCode: 'SG',
		states: null,
	},
	{
		countryName: 'Sint Maarten',
		countryCode: 'SX',
		states: null,
	},
	{
		countryName: 'Slovakia',
		countryCode: 'SK',
		states: null,
	},
	{
		countryName: 'Slovenia',
		countryCode: 'SI',
		states: null,
	},
	{
		countryName: 'Solomon Islands',
		countryCode: 'SB',
		states: null,
	},
	{
		countryName: 'Somalia',
		countryCode: 'SO',
		states: null,
	},
	{
		countryName: 'South Africa',
		countryCode: 'ZA',
		states: null,
	},
	{
		countryName: 'South Georgia & South Sandwich Islands',
		countryCode: 'GS',
		states: null,
	},
	{
		countryName: 'South Korea',
		countryCode: 'KR',
		states: null,
	},
	{
		countryName: 'South Sudan',
		countryCode: 'SS',
		states: null,
	},
	{
		countryName: 'Spain',
		countryCode: 'ES',
		states: null,
	},
	{
		countryName: 'Sri Lanka',
		countryCode: 'LK',
		states: null,
	},
	{
		countryName: 'St. Barthélemy',
		countryCode: 'BL',
		states: null,
	},
	{
		countryName: 'St. Helena',
		countryCode: 'SH',
		states: null,
	},
	{
		countryName: 'St. Kitts & Nevis',
		countryCode: 'KN',
		states: null,
	},
	{
		countryName: 'St. Lucia',
		countryCode: 'LC',
		states: null,
	},
	{
		countryName: 'St. Martin',
		countryCode: 'MF',
		states: null,
	},
	{
		countryName: 'St. Pierre & Miquelon',
		countryCode: 'PM',
		states: null,
	},
	{
		countryName: 'St. Vincent & Grenadines',
		countryCode: 'VC',
		states: null,
	},
	{
		countryName: 'Sudan',
		countryCode: 'SD',
		states: null,
	},
	{
		countryName: 'Suriname',
		countryCode: 'SR',
		states: null,
	},
	{
		countryName: 'Svalbard & Jan Mayen',
		countryCode: 'SJ',
		states: null,
	},
	{
		countryName: 'Sweden',
		countryCode: 'SE',
		states: null,
	},
	{
		countryName: 'Switzerland',
		countryCode: 'CH',
		states: null,
	},
	{
		countryName: 'Syria',
		countryCode: 'SY',
		states: null,
	},
	{
		countryName: 'Taiwan',
		countryCode: 'TW',
		states: null,
	},
	{
		countryName: 'Tajikistan',
		countryCode: 'TJ',
		states: null,
	},
	{
		countryName: 'Tanzania',
		countryCode: 'TZ',
		states: null,
	},
	{
		countryName: 'Thailand',
		countryCode: 'TH',
		states: null,
	},
	{
		countryName: 'Timor-Leste',
		countryCode: 'TL',
		states: null,
	},
	{
		countryName: 'Togo',
		countryCode: 'TG',
		states: null,
	},
	{
		countryName: 'Tokelau',
		countryCode: 'TK',
		states: null,
	},
	{
		countryName: 'Tonga',
		countryCode: 'TO',
		states: null,
	},
	{
		countryName: 'Trinidad & Tobago',
		countryCode: 'TT',
		states: null,
	},
	{
		countryName: 'Tunisia',
		countryCode: 'TN',
		states: null,
	},
	{
		countryName: 'Türkiye',
		countryCode: 'TR',
		states: null,
	},
	{
		countryName: 'Turkmenistan',
		countryCode: 'TM',
		states: null,
	},
	{
		countryName: 'Turks & Caicos Islands',
		countryCode: 'TC',
		states: null,
	},
	{
		countryName: 'Tuvalu',
		countryCode: 'TV',
		states: null,
	},
	{
		countryName: 'U.S. Outlying Islands',
		countryCode: 'UM',
		states: null,
	},
	{
		countryName: 'U.S. Virgin Islands',
		countryCode: 'VI',
		states: null,
	},
	{
		countryName: 'Uganda',
		countryCode: 'UG',
		states: null,
	},
	{
		countryName: 'Ukraine',
		countryCode: 'UA',
		states: null,
	},
	{
		countryName: 'United Arab Emirates',
		countryCode: 'AE',
		states: null,
	},
	{
		countryName: 'United Kingdom',
		countryCode: 'GB',
		states: null,
	},
	{
		countryName: 'United States',
		countryCode: 'US',
		states: [
			{
				stateName: 'Alabama',
				stateCode: 'AL',
			},
			{
				stateName: 'Alaska',
				stateCode: 'AK',
			},
			{
				stateName: 'Arizona',
				stateCode: 'AZ',
			},
			{
				stateName: 'Arkansas',
				stateCode: 'AR',
			},
			{
				stateName: 'California',
				stateCode: 'CA',
			},
			{
				stateName: 'Colorado',
				stateCode: 'CO',
			},
			{
				stateName: 'Connecticut',
				stateCode: 'CT',
			},
			{
				stateName: 'Delaware',
				stateCode: 'DE',
			},
			{
				stateName: 'District of Columbia',
				stateCode: 'DC',
			},
			{
				stateName: 'Florida',
				stateCode: 'FL',
			},
			{
				stateName: 'Georgia',
				stateCode: 'GA',
			},
			{
				stateName: 'Hawaii',
				stateCode: 'HI',
			},
			{
				stateName: 'Idaho',
				stateCode: 'ID',
			},
			{
				stateName: 'Illinois',
				stateCode: 'IL',
			},
			{
				stateName: 'Indiana',
				stateCode: 'IN',
			},
			{
				stateName: 'Iowa',
				stateCode: 'IA',
			},
			{
				stateName: 'Kansas',
				stateCode: 'KS',
			},
			{
				stateName: 'Kentucky',
				stateCode: 'KY',
			},
			{
				stateName: 'Louisiana',
				stateCode: 'LA',
			},
			{
				stateName: 'Maine',
				stateCode: 'ME',
			},
			{
				stateName: 'Maryland',
				stateCode: 'MD',
			},
			{
				stateName: 'Massachusetts',
				stateCode: 'MA',
			},
			{
				stateName: 'Michigan',
				stateCode: 'MI',
			},
			{
				stateName: 'Minnesota',
				stateCode: 'MN',
			},
			{
				stateName: 'Mississippi',
				stateCode: 'MS',
			},
			{
				stateName: 'Missouri',
				stateCode: 'MO',
			},
			{
				stateName: 'Montana',
				stateCode: 'MT',
			},
			{
				stateName: 'Nebraska',
				stateCode: 'NE',
			},
			{
				stateName: 'Nevada',
				stateCode: 'NV',
			},
			{
				stateName: 'New Hampshire',
				stateCode: 'NH',
			},
			{
				stateName: 'New Jersey',
				stateCode: 'NJ',
			},
			{
				stateName: 'New Mexico',
				stateCode: 'NM',
			},
			{
				stateName: 'New York',
				stateCode: 'NY',
			},
			{
				stateName: 'North Carolina',
				stateCode: 'NC',
			},
			{
				stateName: 'North Dakota',
				stateCode: 'ND',
			},
			{
				stateName: 'Ohio',
				stateCode: 'OH',
			},
			{
				stateName: 'Oklahoma',
				stateCode: 'OK',
			},
			{
				stateName: 'Oregon',
				stateCode: 'OR',
			},
			{
				stateName: 'Pennsylvania',
				stateCode: 'PA',
			},
			{
				stateName: 'Rhode Island',
				stateCode: 'RI',
			},
			{
				stateName: 'South Carolina',
				stateCode: 'SC',
			},
			{
				stateName: 'South Dakota',
				stateCode: 'SD',
			},
			{
				stateName: 'Tennessee',
				stateCode: 'TN',
			},
			{
				stateName: 'Texas',
				stateCode: 'TX',
			},
			{
				stateName: 'Utah',
				stateCode: 'UT',
			},
			{
				stateName: 'Vermont',
				stateCode: 'VT',
			},
			{
				stateName: 'Virginia',
				stateCode: 'VA',
			},
			{
				stateName: 'Washington',
				stateCode: 'WA',
			},
			{
				stateName: 'West Virginia',
				stateCode: 'WV',
			},
			{
				stateName: 'Wisconsin',
				stateCode: 'WI',
			},
			{
				stateName: 'Wyoming',
				stateCode: 'WY',
			},
		],
	},
	{
		countryName: 'Uruguay',
		countryCode: 'UY',
		states: null,
	},
	{
		countryName: 'Uzbekistan',
		countryCode: 'UZ',
		states: null,
	},
	{
		countryName: 'Vanuatu',
		countryCode: 'VU',
		states: null,
	},
	{
		countryName: 'Vatican City',
		countryCode: 'VA',
		states: null,
	},
	{
		countryName: 'Venezuela',
		countryCode: 'VE',
		states: null,
	},
	{
		countryName: 'Vietnam',
		countryCode: 'VN',
		states: null,
	},
	{
		countryName: 'Wallis & Futuna',
		countryCode: 'WF',
		states: null,
	},
	{
		countryName: 'Western Sahara',
		countryCode: 'EH',
		states: null,
	},
	{
		countryName: 'Yemen',
		countryCode: 'YE',
		states: null,
	},
	{
		countryName: 'Zambia',
		countryCode: 'ZM',
		states: null,
	},
	{
		countryName: 'Zimbabwe',
		countryCode: 'ZW',
		states: null,
	},
];
