...
const feature = await loadFeature(path.resolve(__dirname, 'features/service-mongoose.feature'));
...
test.for(feature, ({ Scenario, BeforeEachScenario, AfterEachScenario }) => {
	...
	Scenario('Initializing the Mongoose Service with a valid URI', ({ Given, When, Then }) => {
	...
	Scenario('Initializing the Mongoose Service with a valid URI and no options', ({ Given, When, Then }) => {
	...
	Scenario('Initializing the Mongoose Service with an empty URI', ({ Given, When, Then }) => {
		...
		Then('it should throw an error indicating the MongoDB uri is required', () => {
			expect(() => new ServiceMongoose(uri)).toThrow('MongoDB uri is required');
		});
	});
	...
	Scenario('Shutting down the service when started', ({ Given, When, Then }) => {
	...
	Scenario('Shutting down the service when not started', ({ Given, When, Then }) => {
		...
		Then('it should throw an error indicating shutdown cannot proceed', async () => {
			await expect(service.shutDown()).rejects.toThrow('ServiceMongoose is not started - shutdown cannot proceed');
		});
	});

	Scenario('Accessing the service property when started', ({ Given, When, Then }) => {
		Given('a started mongoose service instance', async () => {
			service = new ServiceMongoose(uri, options);
			await service.startUp();
		});
	...
	Scenario('Accessing the service property when not started', ({ Given, When, Then }) => {
		Given('a mongoose service instance that has not been started', () => {
			service = new ServiceMongoose(uri, options);
		});
		When('the service property is accessed', () => {
		...
		Then('it should throw an error indicating the service is not started', () => {
			expect(() => service.service).toThrow('ServiceMongoose is not started - cannot access service');
		});
	});
