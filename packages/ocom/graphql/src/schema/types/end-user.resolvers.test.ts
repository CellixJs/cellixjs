...
const feature = await loadFeature(path.resolve(__dirname, 'features/end-user.resolvers.feature'));
...
test.for(feature, ({ Scenario, BeforeEachScenario }) => {
	...
	Scenario('Querying the current end user and creating if not exists', ({ Given, When, Then, And }) => {
		...
		When('the currentEndUserAndCreateIfNotExists query is executed', async () => {
			...
			result = await (endUserResolvers.Query?.currentEndUserAndCreateIfNotExists as unknown as (parent: object, args: Record<string, never>, context: GraphContext, info: GraphQLResolveInfo) => Promise<EndUserEntity | null>)(
				{},
				{},
				context,
				mockInfo,
	...
	Scenario('Querying an end user by ID', ({ Given, When, Then, And }) => {
		...
		When('the endUserById query is executed with that ID', async () => {
			...
			result = await (endUserResolvers.Query?.endUserById as unknown as (parent: object, args: { id: string }, context: GraphContext, info: GraphQLResolveInfo) => Promise<EndUserEntity | null>)(
				{},
				{ id: endUserId },
				context,
				mockInfo,
	...
	Scenario('Unauthorized access to currentEndUserAndCreateIfNotExists', ({ Given, When, Then }) => {
		...
		When('the currentEndUserAndCreateIfNotExists query is executed', async () => {
			...
			await expect(
				(endUserResolvers.Query?.currentEndUserAndCreateIfNotExists as unknown as (parent: object, args: Record<string, never>, context: GraphContext, info: GraphQLResolveInfo) => Promise<EndUserEntity | null>)(
					{},
					{},
					context,
					mockInfo,
