...
const feature = await loadFeature(path.resolve(__dirname, 'features/community.resolvers.feature'));
...
test.for(feature, ({ Scenario, BeforeEachScenario }) => {
	...
	Scenario('Creating a community', ({ Given, And, When, Then }) => {
		...
		When('the communityCreate mutation is executed with the input', async () => {
			...
			result = await (
				communityResolvers.Mutation?.communityCreate as unknown as (
					parent: unknown,
					args: { input: CommunityCreateInput },
					context: GraphContext,
					info?: unknown,
				) => Promise<{ status: { success: boolean }; community?: CommunityEntity } | null>
			)(null, { input }, context);
		});
	...
	Scenario('Unauthorized access to currentCommunity', ({ Given, When, Then }) => {
		...
		When('the currentCommunity query is executed', async () => {
			await expect(
				(communityResolvers.Query?.currentCommunity as unknown as (parent: unknown, args: Record<string, never>, context: GraphContext, info: unknown) => Promise<CommunityEntity | null>)(null, {}, context, {}),
	...
	Scenario('Unauthorized access to communitiesForCurrentEndUser', ({ Given, When, Then }) => {
		...
		When('the communitiesForCurrentEndUser query is executed', async () => {
			await expect(
				(communityResolvers.Query?.communitiesForCurrentEndUser as unknown as (parent: unknown, args: Record<string, never>, context: GraphContext, info: unknown) => Promise<CommunityEntity[]>)(null, {}, context, {}),
			).rejects.toThrow('Unauthorized');
		});
		...
		When('the communityCreate mutation is executed', async () => {
			result = await (
				communityResolvers.Mutation?.communityCreate as unknown as (
					...
					context: GraphContext,
					info?: unknown,
				) => Promise<{ status: { success: boolean; errorMessage?: string } } | null>
			)(null, { input }, context);
		});
