...
test.for(feature, ({ Scenario, BeforeEachScenario }) => {
	...
	Scenario('Creating a staff user when caller has canManageUsers permission', ({ Given, When, Then }) => {
		Given('a user with canManageUsers permission', () => {
			const currentUser = createMockStaffUser({ role: { permissions: { userPermissions: { canManageUsers: true }, techAdminPermissions: { canManageTechAdmin: false } } } as unknown as StaffRoleEntity });
			context = makeMockGraphContext();
			vi.mocked(context.applicationServices.User.StaffUser.queryByExternalId).mockResolvedValue(currentUser);
		});

		When('the staffUserCreate mutation is executed with firstName "Alice" lastName "Wonder" and email "alice@example.com"', async () => {
			const created = createMockStaffUser({ firstName: 'Alice', lastName: 'Wonder', email: 'alice@example.com' });
			vi.mocked(context.applicationServices.User.StaffUser.create).mockResolvedValue(created);
			result = await callMutation('staffUserCreate', context, { input: { firstName: 'Alice', lastName: 'Wonder', email: 'alice@example.com' } });
		});
	...
	Scenario('Creating a staff user when caller lacks permission', ({ Given, When, Then }) => {
		Given('a user with no user-management permissions', () => {
			const currentUser = createMockStaffUser({ role: { permissions: { userPermissions: { canManageUsers: false }, techAdminPermissions: { canManageTechAdmin: false } } } as unknown as StaffRoleEntity });
			context = makeMockGraphContext();
			vi.mocked(context.applicationServices.User.StaffUser.queryByExternalId).mockResolvedValue(currentUser);
		});
	...
	Scenario('Creating a staff user with a role as TechAdmin', ({ Given, When, Then, And }) => {
		Given('a user with a verifiedJwt that includes the TechAdmin role', () => {
			context = makeMockGraphContext({ jwt: { roles: ['Staff.TechAdmin'] } });
		});
		...
		When('the staffUserCreate mutation is executed with a roleId "role-001"', async () => {
			const created = createMockStaffUser();
			vi.mocked(context.applicationServices.User.StaffUser.create).mockResolvedValue(created);
			result = await callMutation('staffUserCreate', context, { input: { firstName: 'Alice', lastName: 'Wonder', email: 'alice@example.com', roleId: 'role-001' } });
		});
	...
	Scenario('Creating a staff user with a forbidden role as non-TechAdmin', ({ Given, When, Then, And }) => {
		Given('a user with a verifiedJwt that includes the CaseManager role', () => {
			context = makeMockGraphContext({ jwt: { roles: ['Staff.CaseManager'] } });
		});
	...
	Scenario('Creating a staff user when unauthenticated', ({ Given, When, Then }) => {
		Given('a user without a verifiedJwt in their context', () => {
			context = makeMockGraphContext({ jwt: null });
		});
