...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	Background(({ Given, And }) => {
		...
		And(
			'a valid StaffUserEntityReference with id "staff-1" and a role with communityPermissions',
			() => {
	...
	Scenario(
		'Creating a StaffUserCommunityVisa with a staff user and community',
		({ When, Then }) => {
			When(
				'I create a StaffUserCommunityVisa with the community and staff user',
				() => {
	...
	Scenario(
		...
		({ Given, When, Then }) => {
			...
			When(
				'I call determineIf with a function that returns true if canManageCommunitySettings is true',
				() => {
					result = visa.determineIf(
						(p) =>
							p.canManageCommunitySettings === false ||
							p.canManageCommunitySettings === true,
					);
				},
			);
	...
	Scenario(
		'determineIf returns false if the staff user has no role',
		({ Given, And, When, Then }) => {
			let result: boolean;
	...
	Scenario(
		"determineIf returns true if the staff user's role has the required community permission",
		({ Given, And, When, Then }) => {
			let result: boolean;
			Given(
				'a StaffUserEntityReference with communityPermissions where canManageCommunitySettings is true',
				() => {
					staffUser = makeStaffUser('staff-1', {
						communityPermissions: {
			...
			And(
				'I call determineIf with a function that returns canManageCommunitySettings',
				() => {
					result = visa.determineIf(
						(p) => p.canManageCommunitySettings === true,
					);
				},
			);
			Then('the result should be true', () => {
	...
	Scenario(
		"determineIf returns false if the staff user's role does not have the required community permissionchecks all permission flags",
		({ Given, And, When, Then }) => {
			let result: boolean;
			Given(
				'a StaffUserEntityReference with communityPermissions where canManageAllCommunities is false',
				() => {
					staffUser = makeStaffUser('staff-1', {
						communityPermissions: { canManageAllCommunities: false },
					});
				},
			);
			And('a CommunityEntityReference with id "community-1"', () => {
