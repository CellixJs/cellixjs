...
const feature = await loadFeature(path.resolve(__dirname, 'features/system.user.passport.feature'));
...
test.for(feature, ({ Scenario, Background }) => {
	...
	Scenario('Creating SystemUserPassport and getting visa for end user', ({ Given, When, Then, And }) => {
		...
		And('the visa should allow determining permissions', () => {
			expect(typeof visa.determineIf).toBe('function');
			const result = visa.determineIf((perms: Readonly<UserDomainPermissions>) => perms.canManageEndUsers);
			expect(typeof result).toBe('boolean');
		});
	});

	Scenario('Creating SystemUserPassport and getting visa for staff user', ({ Given, When, Then, And }) => {
		Given('I create a SystemUserPassport with permissions', () => {
			passport = new SystemUserPassport(permissions);
		});
	...
	Scenario('Creating SystemUserPassport and getting visa for staff role', ({ Given, When, Then, And }) => {
		Given('I create a SystemUserPassport with permissions', () => {
			passport = new SystemUserPassport(permissions);
		});
	...
	Scenario('Creating SystemUserPassport and getting visa for vendor user', ({ Given, When, Then, And }) => {
		Given('I create a SystemUserPassport with permissions', () => {
			passport = new SystemUserPassport(permissions);
		});
	...
	Scenario('Creating SystemUserPassport with no permissions', ({ Given, When, Then, And }) => {
	...
	Scenario('Using visa to determine permissions', ({ Given, When, Then, And }) => {
		...
		And('I use determineIf to check if canManageEndUsers is true', () => {
			permissionResult = visa.determineIf((perms: Readonly<UserDomainPermissions>) => perms.canManageEndUsers === true);
		});
