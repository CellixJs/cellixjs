...
test.for(feature, ({ Scenario, Background }) => {
	...
	Background(({ Given }) => {
		Given(
			'I have community domain permissions with canManageMembers true',
			() => {
				permissions = {
	...
	Scenario(
		'Creating SystemCommunityPassport and getting visa for community',
		({ Given, When, Then, And }) => {
			...
			And('the visa should allow determining permissions', () => {
				...
				const result = visa.determineIf(
					(perms: Readonly<CommunityDomainPermissions>) =>
	...
	Scenario(
		'Creating SystemCommunityPassport with no permissions',
		({ Given, When, Then, And }) => {
			...
			Then(
				'it should return a CommunityVisa that works with empty permissions',
				() => {
					expect(visa).toBeDefined();
					expect(typeof visa.determineIf).toBe('function');
				},
	...
	Scenario(
		...
		({ Given, When, Then, And }) => {
			Given(
				'I create a SystemCommunityPassport with canManageMembers permission',
				() => {
