...
const feature = await loadFeature(path.resolve(__dirname, 'features/system.property.passport.feature'));
...
test.for(feature, ({ Scenario, Background }) => {
	...
	Scenario('Creating SystemPropertyPassport and getting visa for property', ({ Given, When, Then, And }) => {
		...
		And('the visa should allow determining permissions', () => {
			expect(typeof visa.determineIf).toBe('function');
			const result = visa.determineIf((perms: Readonly<PropertyDomainPermissions>) => perms.canManageProperties);
			expect(typeof result).toBe('boolean');
		});
	});

	Scenario('Creating SystemPropertyPassport with no permissions', ({ Given, When, Then, And }) => {
	...
	Scenario('Using visa to determine permissions', ({ Given, When, Then, And }) => {
		...
		And('I use determineIf to check if canManageProperties is true', () => {
			permissionResult = visa.determineIf((perms: Readonly<PropertyDomainPermissions>) => perms.canManageProperties === true);
		});
