...
const feature = await loadFeature(path.resolve(__dirname, 'features/system.service.passport.feature'));
...
test.for(feature, ({ Scenario, Background }) => {
	...
	Scenario('Creating SystemServicePassport and getting visa for service', ({ Given, When, Then, And }) => {
		...
		And('the visa should allow determining permissions', () => {
			expect(typeof visa.determineIf).toBe('function');
			const result = visa.determineIf((perms: Readonly<ServiceDomainPermissions>) => perms.canManageServices);
			expect(typeof result).toBe('boolean');
		});
	});

	Scenario('Creating SystemServicePassport with no permissions', ({ Given, When, Then, And }) => {
	...
	Scenario('Using visa to determine permissions', ({ Given, When, Then, And }) => {
		...
		And('I use determineIf to check if canManageServices is true', () => {
			permissionResult = visa.determineIf((perms: Readonly<ServiceDomainPermissions>) => perms.canManageServices === true);
		});
