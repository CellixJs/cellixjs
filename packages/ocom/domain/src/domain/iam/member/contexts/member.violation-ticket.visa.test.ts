...
const feature = await loadFeature(path.resolve(__dirname, 'features/member.violation-ticket.visa.feature'));

function makeViolationTicket(id = 'ticket-1', communityId = 'community-1', requestorId = 'member-1', assignedToId = 'member-2') {
	return {
		id,
		communityId,
		requestorId,
...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	BeforeEachScenario(() => {
		...
		member = makeMember();
		visa = undefined as unknown as MemberViolationTicketVisa<ViolationTicketV1EntityReference>;
	});
	...
	Background(({ Given, And }) => {
		Given('a valid ViolationTicketV1EntityReference with id "ticket-1", communityId "community-1", requestorId "member-1", assignedToId "member-2"', () => {
			ticket = makeViolationTicket('ticket-1', 'community-1', 'member-1', 'member-2');
		});
	...
	Scenario('Creating a MemberViolationTicketVisa with a member belonging to the community', ({ When, Then }) => {
