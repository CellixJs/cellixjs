...
const feature = await loadFeature(path.resolve(__dirname, 'features/member.community.visa.feature'));
...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	Scenario('Creating a MemberCommunityVisa with a member belonging to the community', ({ When, Then }) => {
	...
	Scenario('determineIf sets canCreateCommunities to true', ({ Given, When, Then }) => {
		let result: boolean;
		Given('a MemberCommunityVisa for the member and community', () => {
			visa = new MemberCommunityVisa(community, member);
		});
	...
	Scenario('determineIf sets canManageVendorUserRolesAndPermissions to false', ({ Given, When, Then }) => {
		let result: boolean;
		Given('a MemberCommunityVisa for the member and community', () => {
			visa = new MemberCommunityVisa(community, member);
		});
