...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	Scenario(
		...
		({ Given, And, When, Then }) => {
			let result: boolean;
			Given(
				'a MemberEntityReference with communityPermissions where canManageCommunitySettings is true',
				() => {
					member = makeMember('member-1', 'community-1', {
	...
	Scenario(
		...
		({ Given, And, When, Then }) => {
			let result: boolean;
			Given(
				'a MemberEntityReference with communityPermissions where canManageCommunitySettings is false',
				() => {
					member = makeMember('member-1', 'community-1', {
	...
	Scenario(
		...
		({ Given, When, Then }) => {
			...
			When(
				...
				() => {
					result = visa.determineIf(
						(p) => p.canManageVendorUserRolesAndPermissions,
					);
				},
			);
			Then('the result should be false', () => {
