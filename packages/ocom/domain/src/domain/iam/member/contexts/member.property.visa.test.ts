...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	Background(({ Given, And }) => {
		Given(
			'a valid PropertyEntityReference with id "property-1", community id "community-1", owner id "member-1"',
			() => {
				property = makeProperty('property-1', 'community-1', 'member-1');
			},
		);
		And(
			'a valid MemberEntityReference with id "member-1", community id "community-1", and role with property permissions',
			() => {
	...
	Scenario(
		'Creating a MemberPropertyVisa with a member belonging to the community',
		({ When, Then }) => {
	...
	Scenario(
		...
		({ Given, When, Then }) => {
			...
			When(
				'I call determineIf with a function that returns true if canManageProperties is true',
				() => {
	...
	Scenario(
		...
		({ Given, And, When, Then }) => {
			let result: boolean;
			Given(
				'a MemberEntityReference with propertyPermissions where canManageProperties is true',
				() => {
					member = makeMember('member-1', 'community-1', {
			...
			And(
				'I call determineIf with a function that returns canManageProperties',
				() => {
					result = visa.determineIf((p) => p.canManageProperties === true);
				},
			);
	...
	Scenario(
		...
		({ Given, And, When, Then }) => {
			let result: boolean;
			Given(
				'a MemberEntityReference with propertyPermissions where canManageProperties is false',
				() => {
					member = makeMember('member-1', 'community-1', {
	...
	Scenario(
		'determineIf sets isEditingOwnProperty to true when member is the owner',
		({ Given, When, Then }) => {
			let result: boolean;
			Given('a MemberPropertyVisa for the property and member', () => {
				visa = new MemberPropertyVisa(property, member);
			...
			When(
				'I call determineIf with a function that returns isEditingOwnProperty',
				() => {
	...
	Scenario(
		'determineIf sets isEditingOwnProperty to false when member is not the owner',
		({ Given, When, Then, And }) => {
			let result: boolean;
	...
	Scenario(
		'determineIf sets canEditOwnProperty based on role permissions',
		({ Given, When, Then, And }) => {
			let result: boolean;
			Given(
				'a MemberEntityReference with propertyPermissions where canEditOwnProperty is true',
				() => {
					member = makeMember('member-1', 'community-1', {
			...
			And(
				'I call determineIf with a function that returns canEditOwnProperty',
				() => {
