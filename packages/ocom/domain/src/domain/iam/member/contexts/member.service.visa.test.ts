...
const feature = await loadFeature(path.resolve(__dirname, 'features/member.service.visa.feature'));

function makeService(id = 'service-1', communityId = 'community-1') {
	return { id, community: { id: communityId } } as ServiceEntityReference;
}

function makeMember(id = 'member-1', communityId = 'community-1', roleOverrides: Partial<{ servicePermissions: Record<string, unknown> }> = {}) {
	return {
		id,
