...
import type { ServiceEntityReference } from '../../../contexts/service/service/service.aggregate.ts';
...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	Background(({ Given, And }) => {
		Given(
			'a valid ServiceEntityReference with id "service-1" and community id "community-1"',
			() => {
				service = makeService('service-1', 'community-1');
			},
		);
		And(
			'a valid MemberEntityReference with id "member-1", community id "community-1", and role with service permissions',
			() => {
	...
	Scenario(
		"Creating a MemberServiceVisa with a member belonging to the service's community",
		({ When, Then }) => {
	...
	Scenario(
		...
		({ Given, When, Then }) => {
			...
			When(
				'I call determineIf with a function that returns true if canManageServices is true',
				() => {
	...
	Scenario(
		"determineIf returns false if the member does not belong to the service's community",
		({ Given, And, When, Then }) => {
			let result: boolean;
			...
			And(
				'a ServiceEntityReference with id "service-1" and community id "community-1"',
				() => {
					service = makeService('service-1', 'community-1');
				},
			);
	...
	Scenario(
		"determineIf returns true if the member's role has the required service permission",
		({ Given, And, When, Then }) => {
			let result: boolean;
			Given(
				'a MemberEntityReference with servicePermissions where canManageServices is true',
				() => {
					member = makeMember('member-1', 'community-1', {
			...
			And(
				'I call determineIf with a function that returns canManageServices',
				() => {
					result = visa.determineIf((p) => p.canManageServices === true);
				},
			);
	...
	Scenario(
		"determineIf returns false if the member's role does not have the required service permission",
		({ Given, And, When, Then }) => {
			let result: boolean;
			Given(
				'a MemberEntityReference with servicePermissions where canManageServices is false',
				() => {
					member = makeMember('member-1', 'community-1', {
