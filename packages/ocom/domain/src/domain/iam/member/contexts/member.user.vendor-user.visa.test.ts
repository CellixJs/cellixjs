...
const feature = await loadFeature(path.resolve(__dirname, 'features/member.user.vendor-user.feature'));
...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	Scenario('determineIf always receives all permissions as false', ({ Given, When, Then }) => {
		...
		When('I call determineIf with a function that checks all permission flags', () => {
			result = visa.determineIf((p) => !p.canManageEndUsers && !p.canManageStaffRolesAndPermissions && !p.canManageVendorUsers && !p.canManageVendorUsers && !p.isEditingOwnAccount && !p.isSystemAccount);
		});
