...
const feature = await loadFeature(path.resolve(__dirname, 'features/member.service-ticket.visa.feature'));

function makeServiceTicket(id = 'ticket-1', communityId = 'community-1', requestorId = 'member-1', assignedToId = 'member-2') {
	return {
		id,
		communityId,
		requestorId,
...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	BeforeEachScenario(() => {
		...
		member = makeMember();
		visa = undefined as unknown as MemberServiceTicketVisa<ServiceTicketV1EntityReference>;
	});

	Background(({ Given, And }) => {
		Given('a valid ServiceTicketV1EntityReference with id "ticket-1", communityId "community-1", requestorId "member-1", assignedToId "member-2"', () => {
			ticket = makeServiceTicket('ticket-1', 'community-1', 'member-1', 'member-2');
		});
	...
	Scenario('Creating a MemberServiceTicketVisa with a member belonging to the community', ({ When, Then }) => {
		When('I create a MemberServiceTicketVisa with the ticket and member', () => {
	...
	Scenario('determineIf returns false if the member does not belong to the community', ({ Given, And, When, Then }) => {
		...
		And('a ServiceTicketV1EntityReference with communityId "community-1"', () => {
			ticket = makeServiceTicket('ticket-1', 'community-1', 'member-1', 'member-2');
		});
		When('I create a MemberServiceTicketVisa with the ticket and member', () => {
			visa = new MemberServiceTicketVisa(ticket, member);
	...
	Scenario('determineIf sets canCreateTickets to true', ({ Given, When, Then }) => {
		let result: boolean;
		Given('a MemberServiceTicketVisa for the ticket and member', () => {
			visa = new MemberServiceTicketVisa(ticket, member);
		});
	...
	Scenario('determineIf sets canManageTickets to false', ({ Given, When, Then }) => {
		let result: boolean;
		Given('a MemberServiceTicketVisa for the ticket and member', () => {
			visa = new MemberServiceTicketVisa(ticket, member);
		});
	...
	Scenario('determineIf sets canAssignTickets to false', ({ Given, When, Then }) => {
		let result: boolean;
		Given('a MemberServiceTicketVisa for the ticket and member', () => {
			visa = new MemberServiceTicketVisa(ticket, member);
		});
	...
	Scenario('determineIf sets canWorkOnTickets to false', ({ Given, When, Then }) => {
		let result: boolean;
		Given('a MemberServiceTicketVisa for the ticket and member', () => {
			visa = new MemberServiceTicketVisa(ticket, member);
		});
	...
	Scenario('determineIf sets isEditingOwnTicket to true when member is the requestor', ({ Given, When, Then }) => {
		let result: boolean;
		Given('a MemberServiceTicketVisa for the ticket and member', () => {
			visa = new MemberServiceTicketVisa(ticket, member);
		});
	...
	Scenario('determineIf sets isEditingOwnTicket to false when member is not the requestor', ({ Given, When, Then, And }) => {
		let result: boolean;
	...
	Scenario('determineIf sets isEditingAssignedTicket to false when member is not assigned', ({ Given, When, Then }) => {
		let result: boolean;
		Given('a MemberServiceTicketVisa for the ticket and member', () => {
			visa = new MemberServiceTicketVisa(ticket, member);
		});
	...
	Scenario('determineIf sets isEditingAssignedTicket to true when member is assigned', ({ Given, When, Then, And }) => {
		let result: boolean;
		Given('a MemberEntityReference with id "member-2"', () => {
			member = makeMember('member-2', 'community-1');
		});
