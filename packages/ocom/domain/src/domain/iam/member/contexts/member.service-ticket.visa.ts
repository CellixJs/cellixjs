...
export class MemberServiceTicketVisa<root extends ServiceTicketV1EntityReference> implements ServiceTicketV1Visa {
	...
	determineIf(func: (permissions: CaseDomainPermissions) => boolean): boolean {
		...
		if (this.member.community.id !== this.root.communityId) {
			console.log('Member Visa: member is not a member of the community that owns this service ticket', this.member, this.root);
			return false;
		}
