...
export abstract class MemberPassportBase {
	...
	constructor(user: EndUserEntityReference, member: MemberEntityReference, community: CommunityEntityReference) {
		if (!user) {
		...
		if (member.community.id !== community.id) {
			throw new Error(`Member ${member.id} is not part of the community ${community.id}`);
		}
