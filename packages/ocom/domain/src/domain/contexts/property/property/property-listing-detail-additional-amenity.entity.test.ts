...
const feature = await loadFeature(
	path.resolve(
		__dirname,
		'features/property-listing-detail-additional-amenity.entity.feature',
	),
);
...
test.for(feature, ({ Scenario }) => {
	...
	Scenario(
		'Creating an additional amenity with valid props',
		({ When, Then }) => {
			let amenity: PropertyListingDetailAdditionalAmenityEntity.PropertyListingDetailAdditionalAmenity;
			When(
				'I create an additional amenity with valid category and amenities',
				() => {
					amenity =
						new PropertyListingDetailAdditionalAmenityEntity.PropertyListingDetailAdditionalAmenity(
							validProps,
							mockVisa,
						);
				},
			...
			Then('the additional amenity should be created successfully', () => {
				expect(amenity).toBeInstanceOf(
					PropertyListingDetailAdditionalAmenityEntity.PropertyListingDetailAdditionalAmenity,
				);
	...
	Scenario(
		'Setting category with proper permissions',
		({ Given, When, Then }) => {
			let amenity: PropertyListingDetailAdditionalAmenityEntity.PropertyListingDetailAdditionalAmenity;
	...
	Scenario(
		'Setting category without proper permissions',
		({ Given, When, Then }) => {
			let amenity: PropertyListingDetailAdditionalAmenityEntity.PropertyListingDetailAdditionalAmenity;
			Given('an additional amenity exists', () => {
				amenity =
