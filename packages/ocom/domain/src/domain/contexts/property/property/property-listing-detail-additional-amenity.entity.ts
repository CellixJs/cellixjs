...
export interface PropertyListingDetailAdditionalAmenityProps extends DomainEntityProps {
	category: string;
	amenities: string[];
}

export interface PropertyListingDetailAdditionalAmenityEntityReference extends Readonly<PropertyListingDetailAdditionalAmenityProps> {}

export class PropertyListingDetailAdditionalAmenity extends DomainEntity<PropertyListingDetailAdditionalAmenityProps> implements PropertyListingDetailAdditionalAmenityEntityReference {
	...
	private validateVisa(): void {
		if (!this.visa.determineIf((permissions) => permissions.isSystemAccount || permissions.canManageProperties || (permissions.canEditOwnProperty && permissions.isEditingOwnProperty))) {
			throw new PermissionError('You do not have permission to update property amenities');
		}
	}
