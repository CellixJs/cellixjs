...
const feature = await loadFeature(path.resolve(__dirname, 'features/property-listing-detail-bedroom-detail.value-objects.feature'));
...
test.for(feature, ({ Scenario }) => {
	...
	Scenario('Creating a room name with leading and trailing whitespace', ({ When, Then }) => {
		let value: string;
		When('I create a room name with "  Master Bedroom  "', () => {
			value = new ValueObjects.RoomName('  Master Bedroom  ').valueOf();
		});
	...
	Scenario('Creating a room name with maximum allowed length', ({ When, Then }) => {
		let value: string;
	...
	Scenario('Creating a room name with more than maximum allowed length', ({ When, Then }) => {
	...
	Scenario('Creating a room name with minimum allowed length', ({ When, Then }) => {
		let value: string;
	...
	Scenario('Creating a room name with less than minimum allowed length', ({ When, Then }) => {
	...
	Scenario('Creating bed descriptions with valid array', ({ When, Then }) => {
		...
		When('I create bed descriptions with ["King bed", "Queen bed"]', () => {
			value = new ValueObjects.BedDescriptions(['King bed', 'Queen bed']).valueOf();
		});
	...
	Scenario('Creating bed descriptions with array above maximum length', ({ When, Then }) => {
