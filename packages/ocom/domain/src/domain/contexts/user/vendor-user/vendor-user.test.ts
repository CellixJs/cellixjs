...
const feature = await loadFeature(path.resolve(__dirname, 'features/vendor-user.feature'));

function makePassport(canManageVendorUsers = false, isEditingOwnAccount = false): Passport {
	return vi.mocked({
		user: {
			forVendorUser: vi.fn(() => ({
				determineIf: (fn: (p: { canManageVendorUsers: boolean; isEditingOwnAccount: boolean }) => boolean) => fn({ canManageVendorUsers, isEditingOwnAccount }),
			})),
		},
	} as unknown as Passport);
}

function makeBaseProps(overrides: Partial<VendorUserProps> = {}): VendorUserProps {
	return {
		id: 'vendor-1',
...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	Scenario('Creating a new vendor user instance with restOfName', ({ When, Then, And }) => {
		When('I create a new VendorUser aggregate using getNewUser with externalId "123e4567-e89b-12d3-a456-426614174000", lastName "Smith", and restOfName "Alice"', () => {
			newVendorUser = VendorUser.getNewUser(makeBaseProps(), passport, '123e4567-e89b-12d3-a456-426614174000', 'Smith', 'Alice');
		});
		...
		And('a VendorUserCreatedEvent should be added to integration events', () => {
			const event = getIntegrationEvent(newVendorUser.getIntegrationEvents(), VendorUserCreatedEvent);
	...
	Scenario('Creating a new vendor user instance with no restOfName', ({ When, Then, And }) => {
		When('I create a new VendorUser aggregate using getNewUser with externalId "123e4567-e89b-12d3-a456-426614174000", lastName "Smith", and no restOfName', () => {
			newVendorUser = VendorUser.getNewUser(makeBaseProps(), passport, '123e4567-e89b-12d3-a456-426614174000', 'Smith');
		});
