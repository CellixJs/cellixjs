...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	Scenario(
		'Creating a new vendor user instance with restOfName',
		({ When, Then, And }) => {
			When(
			...
			Then(
				'the vendor user\'s externalId should be "123e4567-e89b-12d3-a456-426614174000"',
				() => {
					expect(newVendorUser.externalId).toBe(
						'123e4567-e89b-12d3-a456-426614174000',
					);
				},
			);
			...
			And(
				'a VendorUserCreatedEvent should be added to integration events',
				() => {
	...
	Scenario(
		'Creating a new vendor user instance with no restOfName',
		({ When, Then, And }) => {
			When(
	...
	// displayName
	Scenario(
		'Changing the displayName with permission to edit own account',
		({ Given, When, Then }) => {
			Given(
				'a VendorUser aggregate with permission to edit own account',
				() => {
	...
	Scenario(
		'Changing the displayName with permission to manage vendor users',
		({ Given, When, Then }) => {
			Given(
				'a VendorUser aggregate with permission to manage vendor users',
				() => {
	...
	Scenario(
		...
		({ Given, When, Then }) => {
			...
			Given(
				'a VendorUser aggregate without permission to manage vendor users or edit own account',
				() => {
	...
	Scenario(
		...
		({ Given, When, Then }) => {
			...
			Given(
				'a VendorUser aggregate with permission to manage vendor users or edit own account',
				() => {
					passport = makePassport(true, true);
					vendorUser = new VendorUser(makeBaseProps(), passport);
				},
			...
			When(
				'I try to set the displayName to an invalid value (e.g., empty string)',
				() => {
	...
	// externalId
	Scenario(
		'Changing the externalId with permission to manage vendor users',
		({ Given, When, Then }) => {
			Given(
				'a VendorUser aggregate with permission to manage vendor users',
				() => {
			...
			When(
				'I set the externalId to "123e4567-e89b-12d3-a456-426614174001"',
				() => {
					vendorUser.externalId = '123e4567-e89b-12d3-a456-426614174001';
				},
			);
			Then(
				'the vendor user\'s externalId should be "123e4567-e89b-12d3-a456-426614174001"',
				() => {
					expect(vendorUser.externalId).toBe(
	...
	Scenario(
		'Changing the externalId without permission to manage vendor users',
		({ Given, When, Then }) => {
			let changingExternalIdWithoutPermission: () => void;
			Given(
				'a VendorUser aggregate without permission to manage vendor users',
				() => {
					passport = makePassport(false, true);
					vendorUser = new VendorUser(makeBaseProps(), passport);
				},
			...
			When(
				'I try to set the externalId to "123e4567-e89b-12d3-a456-426614174001"',
				() => {
	...
	Scenario(
		...
		({ Given, When, Then }) => {
			...
			Then('an error should be thrown indicating the value is invalid', () => {
				expect(changingExternalIdToInvalid).toThrow(
	...
	// accessBlocked
	Scenario(
		'Changing accessBlocked with permission to manage vendor users',
		({ Given, When, Then }) => {
			Given(
				'a VendorUser aggregate with permission to manage vendor users',
				() => {
	...
	Scenario(
		'Changing accessBlocked without permission to manage vendor users',
		({ Given, When, Then }) => {
	...
	// tags
	Scenario(
		'Changing tags with permission to manage vendor users',
		({ Given, When, Then }) => {
			Given(
				'a VendorUser aggregate with permission to manage vendor users',
				() => {
	...
	Scenario(
		'Changing tags without permission to manage vendor users',
		({ Given, When, Then }) => {
	...
	Scenario(
		...
		({ Given, Then, And }) => {
			...
			And('the createdAt property should return the correct date', () => {
				expect(vendorUser.createdAt).toEqual(
					new Date('2020-01-01T00:00:00.000Z'),
				);
			});
			And('the updatedAt property should return the correct date', () => {
				expect(vendorUser.updatedAt).toEqual(
