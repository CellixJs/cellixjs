...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	Background(({ Given, And }) => {
		Given(
			'valid VendorUserContactInformationProps with email "alice@cellix.com"',
			() => {
				props = makeProps();
			},
		);
	...
	Scenario(
		'Creating a new VendorUserContactInformation instance',
		({ When, Then }) => {
			let newEntity: VendorUserContactInformation;
			When(
				'I create a new VendorUserContactInformation using getNewInstance with the email',
				() => {
	...
	Scenario(
		...
		({ Given, When, Then }) => {
			Given(
				'an VendorUserContactInformation entity with permission to edit own account',
				() => {
					visa = makeVisa({
	...
	Scenario(
		'Changing the email with permission to manage vendor users',
		({ Given, When, Then }) => {
			Given(
				'an VendorUserContactInformation entity with permission to manage vendor users',
				() => {
					visa = makeVisa({
	...
	Scenario('Changing the email without permission', ({ Given, When, Then }) => {
		...
		Given(
			'an VendorUserContactInformation entity without permission to edit own account or manage vendor users',
			() => {
				visa = makeVisa({
					isEditingOwnAccount: false,
					canManageVendorUsers: false,
	...
	Scenario(
		...
		({ Given, When, Then }) => {
			...
			Then('an error should be thrown indicating the email is invalid', () => {
				expect(changingEmailToInvalidValue).throws(
