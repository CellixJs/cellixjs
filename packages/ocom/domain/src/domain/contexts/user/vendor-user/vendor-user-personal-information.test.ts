...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	Background(({ Given, And }) => {
		Given(
			'valid VendorUserIdentityDetailsProps and VendorUserContactInformationProps',
			() => {
				identityDetailsProps = makeIdentityDetailsProps();
				contactInformationProps = makeContactInformationProps();
				personalInformationProps = makePersonalInformationProps();
	...
	Scenario(
		'Creating a new VendorUserPersonalInformation instance',
		({ When, Then, And }) => {
			let newEntity: VendorUserPersonalInformation;
			When(
				'I create a new VendorUserPersonalInformation using getNewInstance with the identity details and contact information',
				() => {
		...
		({ Given, When, Then }) => {
			...
			When('I try to set the identity details', () => {
				changingIdentityDetailsAfterCreation = () => {
					// @ts-expect-error
					entity.identityDetails = makeIdentityDetailsProps({
						lastName: 'NewLast',
					});
				};
			});
			Then(
				'an error should be thrown indicating identity details cannot be set',
				() => {
	...
	Scenario(
		'Setting contact information after creation',
		({ Given, When, Then }) => {
			...
			When('I try to set the contact information', () => {
				changingContactInformationAfterCreation = () => {
					// @ts-expect-error
					entity.contactInformation = makeContactInformationProps({
						email: 'bob@cellix.com',
					});
				};
			});
			Then(
				'an error should be thrown indicating contact information cannot be set',
				() => {
	...
	Scenario(
		...
		({ Given, When, Then }) => {
			...
			Then(
				'I should receive VendorUserIdentityDetails and VendorUserContactInformation entity instances',
				() => {
