...
const feature = await loadFeature(path.resolve(__dirname, 'features/vendor-user-identity-details.feature'));
...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	Scenario('Creating a new VendorUserIdentityDetails instance', ({ When, Then, And }) => {
		...
		When('I create a new VendorUserIdentityDetails using getNewInstance with lastName "Smith", legalNameConsistsOfOneName false, and restOfName "Alice"', () => {
			newEntity = VendorUserIdentityDetails.getNewInstance(props, visa, 'Smith', false, 'Alice');
		});
