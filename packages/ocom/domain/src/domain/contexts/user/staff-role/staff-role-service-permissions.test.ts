...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	Background(({ Given, And }) => {
		Given(
			'valid StaffRoleServicePermissionsProps with all permission flags set to false',
			() => {
				props = makeProps();
			},
		);
	...
	// canManageServices
	Scenario(
		'Changing canManageServices with manage staff roles permission',
		({ Given, When, Then }) => {
			Given(
				'a StaffRoleServicePermissions entity with permission to manage staff roles',
				() => {
					visa = makeVisa({
						canManageStaffRolesAndPermissions: true,
						isSystemAccount: false,
	...
	Scenario(
		...
		({ Given, When, Then }) => {
			Given(
				'a StaffRoleServicePermissions entity with system account permission',
				() => {
					visa = makeVisa({
	...
	Scenario(
		...
		({ Given, When, Then }) => {
			...
			Given(
				'a StaffRoleServicePermissions entity without permission to manage staff roles or system account',
				() => {
					visa = makeVisa({
						canManageStaffRolesAndPermissions: false,
						isSystemAccount: false,
