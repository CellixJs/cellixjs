...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	Background(({ Given, And }) => {
		Given(
			'valid StaffRolePropertyPermissionsProps with all permission flags set to false',
			() => {
				props = makeProps();
			},
		);
	...
	// canManageProperties
	Scenario(
		'Changing canManageProperties with manage staff roles permission',
		({ Given, When, Then }) => {
			Given(
				'a StaffRolePropertyPermissions entity with permission to manage staff roles',
				() => {
					visa = makeVisa({
						canManageStaffRolesAndPermissions: true,
						isSystemAccount: false,
	...
	Scenario(
		...
		({ Given, When, Then }) => {
			Given(
				'a StaffRolePropertyPermissions entity with system account permission',
				() => {
					visa = makeVisa({
	...
	Scenario(
		...
		({ Given, When, Then }) => {
			...
			Given(
				'a StaffRolePropertyPermissions entity without permission to manage staff roles or system account',
				() => {
					visa = makeVisa({
						canManageStaffRolesAndPermissions: false,
						isSystemAccount: false,
	...
	// canEditOwnProperty
	Scenario(
		'Changing canEditOwnProperty with manage staff roles permission',
		({ Given, When, Then }) => {
			Given(
				'a StaffRolePropertyPermissions entity with permission to manage staff roles',
				() => {
