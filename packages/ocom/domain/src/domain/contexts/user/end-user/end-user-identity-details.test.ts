...
const feature = await loadFeature(path.resolve(__dirname, 'features/end-user-identity-details.feature'));
...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	Background(({ Given, And }) => {
		Given('valid EndUserIdentityDetailsProps with lastName "Smith", legalNameConsistsOfOneName false, and restOfName "Alice"', () => {
			props = makeProps();
		});
	...
	Scenario('Creating a new EndUserIdentityDetails instance', ({ When, Then, And }) => {
		...
		When('I create a new EndUserIdentityDetails using getNewInstance with lastName "Smith", legalNameConsistsOfOneName false, and restOfName "Alice"', () => {
