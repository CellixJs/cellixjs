...
const feature = await loadFeature(path.resolve(__dirname, 'features/end-user.feature'));

function makePassport(canManageEndUsers = false, isEditingOwnAccount = false): Passport {
	return vi.mocked({
		user: {
			forEndUser: vi.fn(() => ({
				determineIf: (fn: (p: { canManageEndUsers: boolean; isEditingOwnAccount: boolean }) => boolean) => fn({ canManageEndUsers, isEditingOwnAccount }),
			})),
		},
...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	Scenario('Creating a new end user instance with restOfName', ({ When, Then, And }) => {
		When('I create a new EndUser aggregate using getNewInstance with externalId "123e4567-e89b-12d3-a456-426614174000", lastName "Smith", restOfName "Alice", and email "alice@cellix.com"', () => {
			newEndUser = EndUser.getNewInstance(makeBaseProps(), passport, '123e4567-e89b-12d3-a456-426614174000', 'Smith', 'Alice', 'alice@cellix.com');
		});
		...
		And('a EndUserCreatedEvent should be added to integration events', () => {
			const event = getIntegrationEvent(newEndUser.getIntegrationEvents(), EndUserCreatedEvent);
	...
	Scenario('Creating a new end user instance with no restOfName', ({ When, Then, And }) => {
		When('I create a new EndUser aggregate using getNewInstance with externalId "123e4567-e89b-12d3-a456-426614174000", lastName "Smith", restOfName "", and email "alice@cellix.com"', () => {
			newEndUser = EndUser.getNewInstance(makeBaseProps(), passport, '123e4567-e89b-12d3-a456-426614174000', 'Smith', '', 'alice@cellix.com');
		});
	...
	Scenario('Setting the externalId during creation', ({ Given, When, Then }) => {
		...
		When('I set the externalId to "123e4567-e89b-12d3-a456-426614174001" during creation', () => {
			newEndUser = EndUser.getNewInstance(baseProps, passport, '123e4567-e89b-12d3-a456-426614174001', 'Smith', 'Alice', 'alice@cellix.com');
		});
