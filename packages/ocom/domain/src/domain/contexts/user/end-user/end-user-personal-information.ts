...
export class EndUserPersonalInformation extends ValueObject<EndUserPersonalInformationProps> implements EndUserPersonalInformationEntityReference {
	...
	constructor(props: EndUserPersonalInformationProps, visa: UserVisa, root: EndUserAggregateRoot) {
		super(props);
		this.visa = visa;
		this.root = root;
	}
	...
	get identityDetails() {
		return new EndUserIdentityDetails(this.props.identityDetails, this.visa, this.root);
	}

	get contactInformation() {
		return new EndUserContactInformation(this.props.contactInformation, this.visa, this.root);
	}
}
