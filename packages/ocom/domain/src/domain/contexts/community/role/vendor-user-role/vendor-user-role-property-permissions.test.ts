...
const feature = await loadFeature(
	path.resolve(
		__dirname,
		'features/vendor-user-role-property-permissions.feature',
	),
);
...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	Background(({ Given, And }) => {
		Given(
			'valid VendorUserRolePropertyPermissionsProps with canManageProperties and canEditOwnProperty set to false',
			() => {
				props = makeProps();
			},
		);
	...
	// canManageProperties
	Scenario(
		'Changing canManageProperties with manage vendor user roles permission',
		({ Given, When, Then }) => {
			Given(
				'an VendorUserRolePropertyPermissions entity with permission to manage vendor user roles',
				() => {
	...
	Scenario(
		...
		({ Given, When, Then }) => {
			Given(
				'an VendorUserRolePropertyPermissions entity with system account permission',
				() => {
					visa = makeVisa({
	...
	Scenario(
		...
		({ Given, When, Then }) => {
			...
			Given(
				'an VendorUserRolePropertyPermissions entity without permission to manage vendor user roles or system account',
				() => {
					visa = makeVisa({
						canManageVendorUserRolesAndPermissions: false,
	...
	// canEditOwnProperty
	Scenario(
		'Changing canEditOwnProperty with manage vendor user roles permission',
		({ Given, When, Then }) => {
			Given(
				'an VendorUserRolePropertyPermissions entity with permission to manage vendor user roles',
				() => {
