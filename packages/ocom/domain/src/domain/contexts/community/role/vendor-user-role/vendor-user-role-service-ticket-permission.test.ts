...
const feature = await loadFeature(
	path.resolve(
		__dirname,
		'features/vendor-user-role-service-ticket-permissions.feature',
	),
);
...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	Background(({ Given, And }) => {
		Given(
			'valid VendorUserRoleServiceTicketPermissionsProps with all permissions set to false',
			() => {
				props = makeProps();
			},
		);
	...
	Scenario(
		...
		({ Given, When, Then }) => {
			Given(
				'an VendorUserRoleServiceTicketPermissions entity with permission to manage vendor user roles',
				() => {
					visa = makeVisa({ canManageVendorUserRolesAndPermissions: true });
					entity = new VendorUserRoleServiceTicketPermissions(
						makeProps(),
						visa,
					);
				},
	...
	Scenario(
		...
		({ Given, When, Then }) => {
			Given(
				'an VendorUserRoleServiceTicketPermissions entity with system account permission',
				() => {
					visa = makeVisa({
	...
	Scenario(
		...
		({ Given, When, Then }) => {
			...
			Given(
				'an VendorUserRoleServiceTicketPermissions entity without permission to manage vendor user roles or system account',
				() => {
					visa = makeVisa({
						canManageVendorUserRolesAndPermissions: false,
