...
const feature = await loadFeature(
	path.resolve(
		__dirname,
		'features/end-user-role-violation-ticket-permissions.feature',
	),
);
...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	Background(({ Given, And }) => {
		Given(
			'valid EndUserRoleViolationTicketPermissionsProps with all permissions set to false',
			() => {
				props = makeProps();
			},
		);
	...
	Scenario(
		...
		({ Given, When, Then }) => {
			Given(
				'an EndUserRoleViolationTicketPermissions entity with permission to manage end user roles',
				() => {
	...
	Scenario(
		...
		({ Given, When, Then }) => {
			Given(
				'an EndUserRoleViolationTicketPermissions entity with system account permission',
				() => {
					visa = makeVisa({
	...
	Scenario(
		...
		({ Given, When, Then }) => {
			...
			Given(
				'an EndUserRoleViolationTicketPermissions entity without permission to manage end user roles or system account',
				() => {
					visa = makeVisa({
						canManageEndUserRolesAndPermissions: false,
