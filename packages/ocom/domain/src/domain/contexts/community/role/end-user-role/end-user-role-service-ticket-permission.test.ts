...
const feature = await loadFeature(
  path.resolve(__dirname, 'features/end-user-role-service-ticket-permissions.feature'),
);
...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
  ...
  let entity: EndUserRoleServiceTicketPermissions;

  BeforeEachScenario(() => {
    visa = makeVisa();
    props = makeProps();
    entity = new EndUserRoleServiceTicketPermissions(props, visa);
  });
