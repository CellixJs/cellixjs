...
import { RoleName } from './end-user-role.value-objects.ts';

const test = { for: describeFeature };
const __dirname = path.dirname(fileURLToPath(import.meta.url));
const feature = await loadFeature(
	path.resolve(__dirname, 'features/end-user-role.value-objects.feature'),
);
...
test.for(feature, ({ Scenario }) => {
	...
	Scenario(
		'Creating a role name with leading and trailing whitespace',
		({ When, Then }) => {
			let value: string;
			When('I create a role name with "  Support  "', () => {
				value = new RoleName('  Support  ').valueOf();
	...
	Scenario(
		'Creating a role name with maximum allowed length',
		({ When, Then }) => {
			let value: string;
	...
	Scenario(
		'Creating a role name with more than maximum allowed length',
		({ When, Then }) => {
			...
			Then(
				'an error should be thrown indicating the role name is too long',
				() => {
	...
	Scenario(
		'Creating a role name with minimum allowed length',
		({ When, Then }) => {
			let value: string;
	...
	Scenario(
		'Creating a role name with less than minimum allowed length',
		({ When, Then }) => {
			...
			Then(
				'an error should be thrown indicating the role name is too short',
				() => {
