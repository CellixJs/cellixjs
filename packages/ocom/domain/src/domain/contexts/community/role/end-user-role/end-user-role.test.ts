...
const feature = await loadFeature(path.resolve(__dirname, 'features/end-user-role.feature'));
...
function makeBaseProps(overrides: Partial<EndUserRoleProps> = {}): EndUserRoleProps {
	return {
		id: 'role-1',
		roleName: 'Member',
		isDefault: false,
...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	Scenario('Changing the roleName with permission to manage end user roles', ({ Given, When, Then }) => {
	...
	Scenario('Changing isDefault with permission to manage end user roles', ({ Given, When, Then }) => {
		Given('an EndUserRole aggregate with permission to manage end user roles', () => {
			passport = makePassport({
				canManageEndUserRolesAndPermissions: true,
			});
	...
	Scenario('Deleting a non-default end user role with permission to manage end user roles', ({ Given, When, Then, And }) => {
		...
		Given('an EndUserRole aggregate that is not deleted and is not default, with permission to manage end user roles', () => {
			...
			reassignedRole = new EndUserRole(makeBaseProps({ id: 'role-2' }), passport);
		});
	...
	Scenario('Deleting a non-default end user role without permission', ({ Given, When, Then, And }) => {
		let reassignedRole: EndUserRole<EndUserRoleProps>;
	...
	Scenario('Deleting a default end user role', ({ Given, When, Then, And }) => {
		...
		Then('a PermissionError should be thrown', () => {
			...
			expect(deleteDefault).toThrow('You cannot delete a default end user role');
		});
		And('no RoleDeletedReassignEvent should be emitted', () => {
			expect(getIntegrationEvent(role.getIntegrationEvents(), RoleDeletedReassignEvent)).toBeUndefined();
		});
