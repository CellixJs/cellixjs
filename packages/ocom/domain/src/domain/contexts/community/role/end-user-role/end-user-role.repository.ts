...
export interface EndUserRoleRepository<props extends EndUserRoleProps> extends Repository<EndUserRole<props>> {
	getNewInstance(roleName: string, isDefault: boolean, community: CommunityEntityReference): Promise<EndUserRole<props>>;
	getById(id: string): Promise<EndUserRole<props>>;
	getByCommunityId(communityId: string): Promise<EndUserRole<props>[]>;
}
