...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	Background(({ Given, And }) => {
		...
		And(
			'base member properties with memberName "Alice", cybersourceCustomerId "cs_123", a valid community, at least one account, a valid role, a valid profile, and valid timestamps',
			() => {
				baseProps = makeBaseProps();
				member = new Member(baseProps, passport);
			},
	...
	Scenario(
		'Creating a new member instance with permission to manage members',
		({ Given, When, Then, And }) => {
			...
			When(
				'I create a new Member aggregate using getNewInstance with memberName "Alice", cybersourceCustomerId "cs_123", and a CommunityEntityReference',
				() => {
			...
			And(
				"the member's community should reference the provided CommunityEntityReference",
				() => {
	...
	Scenario(
		'Creating a new member instance with system account permission',
		({ Given, When, Then, And }) => {
	...
	Scenario(
		'Creating a new member instance without permission',
		({ Given, When, Then }) => {
			let createWithoutPermission: () => void;
			Given(
				'a passport without permission to manage members or system account',
				() => {
					passport = makePassport({
						canManageMembers: false,
						isSystemAccount: false,
			...
			When(
				'I try to create a new Member aggregate using getNewInstance with memberName "Alice", cybersourceCustomerId "cs_123", and a CommunityEntityReference',
				() => {
	...
	Scenario(
		'Changing the memberName with permission to manage members',
		({ Given, When, Then }) => {
	...
	Scenario(
		'Changing the memberName with system account permission',
		({ Given, When, Then }) => {
	...
	Scenario(
		'Changing the memberName without permission',
		({ Given, When, Then }) => {
			let changeMemberNameWithoutPermission: () => void;
			Given(
				'a Member aggregate without permission to manage members or system account',
				() => {
					passport = makePassport({
						canManageMembers: false,
						isSystemAccount: false,
			...
			Then('a PermissionError should be thrown', () => {
				expect(changeMemberNameWithoutPermission).toThrow(
					DomainSeedwork.PermissionError,
				);
				expect(changeMemberNameWithoutPermission).toThrow(
					'Cannot set member name',
				);
			});
		},
	);
	...
	Scenario(
		'Changing the memberName to an invalid value',
		({ Given, When, Then }) => {
			...
			When(
				'I try to set the memberName to an invalid value (e.g., null or empty string)',
				() => {
	...
	Scenario(
		'Changing the cybersourceCustomerId with permission to manage members',
		({ Given, When, Then }) => {
			Given('a Member aggregate with permission to manage members', () => {
				passport = makePassport({ canManageMembers: true });
				member = new Member(makeBaseProps(), passport);
	...
	Scenario(
		'Changing the cybersourceCustomerId with system account permission',
		({ Given, When, Then }) => {
			Given('a Member aggregate with system account permission', () => {
				passport = makePassport({
					canManageMembers: false,
	...
	Scenario(
		'Changing the cybersourceCustomerId without permission',
		({ Given, When, Then }) => {
	...
	Scenario(
		'Changing the cybersourceCustomerId to an invalid value',
		({ Given, When, Then }) => {
			...
			When(
				'I try to set the cybersourceCustomerId to an invalid value (e.g., null or empty string)',
				() => {
	...
	Scenario(
		'Changing the community with permission to manage members',
		({ Given, When, Then }) => {
			...
			Then(
				"the member's community should reference the new CommunityEntityReference",
				() => {
	...
	Scenario(
		'Changing the community with system account permission',
		({ Given, When, Then }) => {
			let newCommunityRef: CommunityEntityReference;
			Given('a Member aggregate with system account permission', () => {
				passport = makePassport({
	...
	Scenario(
		'Changing the community without permission',
		({ Given, When, Then }) => {
			...
			When(
				'I try to set the community to a new CommunityEntityReference',
				() => {
			...
			Then('a PermissionError should be thrown', () => {
				expect(changeCommunityWithoutPermission).toThrow(
					DomainSeedwork.PermissionError,
				);
				expect(changeCommunityWithoutPermission).toThrow(
					'Cannot set community',
				);
			});
		},
	);
	...
	Scenario(
		'Changing the role with permission to manage members',
		({ Given, When, Then }) => {
			...
			Then(
				"the member's role should reference the new EndUserRoleEntityReference",
				() => {
	...
	Scenario(
		'Changing the role with system account permission',
		({ Given, When, Then }) => {
			let newRoleRef: EndUserRoleEntityReference;
			Given('a Member aggregate with system account permission', () => {
				passport = makePassport({
	...
	Scenario(
		'Requesting a new account with permission to manage members',
		({ Given, When, Then }) => {
	...
	Scenario(
		'Requesting a new account with system account permission',
		({ Given, When, Then }) => {
			let newAccount: MemberAccount;
			Given('a Member aggregate with system account permission', () => {
				passport = makePassport({
	...
	Scenario(
		'Requesting a new account without permission',
		({ Given, When, Then }) => {
			...
			Then('a PermissionError should be thrown', () => {
				expect(requestNewAccountWithoutPermission).toThrow(
					DomainSeedwork.PermissionError,
				);
	...
	Scenario(
		'Requesting to remove an account with permission to manage members',
		({ Given, When, Then }) => {
			let accountRef: MemberAccountProps;
			Given(
				'a Member aggregate with permission to manage members and at least one account',
				() => {
					passport = makePassport({ canManageMembers: true });
					baseProps = makeBaseProps();
					member = new Member(baseProps, passport);
			...
			When(
				'I call requestRemoveAccount with a valid MemberAccountEntityReference',
				() => {
	...
	Scenario(
		'Requesting to remove an account with system account permission',
		({ Given, When, Then }) => {
			let accountRef: MemberAccountProps;
			Given(
				'a Member aggregate with system account permission and at least one account',
				() => {
					passport = makePassport({
						canManageMembers: false,
						isSystemAccount: true,
	...
	Scenario(
		'Requesting to remove an account without permission',
		({ Given, When, Then }) => {
			let requestRemoveAccount: () => void;
			let accountRef: MemberAccountProps;
			Given(
				'a Member aggregate without permission to manage members or system account and at least one account',
				() => {
					passport = makePassport({
						canManageMembers: false,
						isSystemAccount: false,
			...
			When(
				'I try to call requestRemoveAccount with a valid MemberAccountEntityReference',
				() => {
	...
	Scenario(
		'Requesting a new custom view with permission to manage members',
		({ Given, When, Then }) => {
	...
	Scenario(
		'Requesting a new custom view with system account permission',
		({ Given, When, Then }) => {
			let newCustomView: MemberCustomViewProps;
			Given('a Member aggregate with system account permission', () => {
				passport = makePassport({
	...
	Scenario(
		'Requesting a new custom view without permission',
		({ Given, When, Then }) => {
	...
	Scenario(
		'Requesting to remove a custom view with permission to manage members',
		({ Given, When, Then }) => {
			let customView: MemberCustomView;
			Given(
				'a Member aggregate with permission to manage members and at least one custom view',
				() => {
					passport = makePassport({ canManageMembers: true });
					baseProps = makeBaseProps();
					member = new Member(baseProps, passport);
			...
			When(
				'I call requestRemoveCustomView with a valid MemberCustomView',
				() => {
					member.requestRemoveCustomView(customView);
				},
			);
			Then(
				"the custom view should be removed from the member's customViews",
				() => {
					expect(baseProps.customViews.removeItem).toHaveBeenCalledWith(
						customView.props,
					);
				},
			);
	...
	Scenario(
		'Requesting to remove a custom view with system account permission',
		({ Given, When, Then }) => {
			let customView: MemberCustomView;
			Given(
				'a Member aggregate with system account permission and at least one custom view',
				() => {
					passport = makePassport({
						canManageMembers: false,
						isSystemAccount: true,
	...
	Scenario(
		'Requesting to remove a custom view without permission',
		({ Given, When, Then }) => {
			let requestRemoveCustomView: () => void;
			let customView: MemberCustomView;
			Given(
				'a Member aggregate without permission to manage members or system account and at least one custom view',
				() => {
					passport = makePassport({
						canManageMembers: false,
						isSystemAccount: false,
			...
			When(
				'I try to call requestRemoveCustomView with a valid MemberCustomView',
				() => {
	...
	Scenario(
		'Getting profile, accounts, customViews, createdAt, updatedAt, and schemaVersion',
		({ Given, Then, And }) => {
			...
			And(
				'the accounts property should return an array of MemberAccount entities',
				() => {
					expect(Array.isArray(member.accounts)).toBe(true);
				},
			);
			And(
				'the customViews property should return an array of MemberCustomView entities',
				() => {
