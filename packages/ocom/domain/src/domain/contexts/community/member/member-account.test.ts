...
const feature = await loadFeature(path.resolve(__dirname, 'features/member-account.feature'));
...
function makeVisa(
	...
	return vi.mocked({
		determineIf: (fn: (p: { isSystemAccount: boolean; canManageMembers: boolean; canEditOwnMemberAccounts: boolean; isEditingOwnMemberAccount: boolean }) => boolean) =>
			fn({
