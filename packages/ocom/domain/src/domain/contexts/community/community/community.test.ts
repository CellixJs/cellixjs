...
const feature = await loadFeature(path.resolve(__dirname, 'features/community.feature'));

function makePassport(canManageCommunitySettings = true): Passport {
	return vi.mocked({
		community: {
			forCommunity: vi.fn(() => ({
				determineIf: (fn: (p: { canManageCommunitySettings: boolean }) => boolean) => fn({ canManageCommunitySettings }),
			})),
		},
		user: {
			forEndUser: vi.fn(() => ({
				determineIf: (fn: (p: { canManageEndUsers: boolean }) => boolean) => fn({ canManageEndUsers: true }),
			})),
		},
...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	Scenario('Creating a new community instance', ({ When, Then, And }) => {
		...
		And('a CommunityCreatedEvent should be emitted', () => {
			const event = findEvent(newCommunity.getIntegrationEvents(), CommunityCreatedEvent);
	...
	Scenario('Changing the domain with permission to manage community settings', ({ Given, When, Then, And }) => {
		Given('a Community aggregate with permission to manage community settings', () => {
			passport = makePassport(true);
			community = new Community(makeBaseProps(), passport);
		});
		...
		And('a CommunityDomainUpdatedEvent should be emitted with the new and old domain', () => {
			const event = findEvent(community.getIntegrationEvents(), CommunityDomainUpdatedEvent);
			expect(event).toBeDefined();
	...
	Scenario('Changing the domain without permission', ({ Given, When, Then, And }) => {
		...
		Then('a PermissionError should be thrown', () => {
			expect(changingDomainWithoutPermission).toThrow(PermissionError);
			expect(changingDomainWithoutPermission).throws('You do not have permission to change the domain of this community');
		});
	...
	Scenario('Changing the domain to an invalid value', ({ Given, When, Then, And }) => {
	...
	Scenario('Changing the domain to the same value', ({ Given, When, Then, And }) => {
		Given('a Community aggregate with permission to manage community settings', () => {
			passport = makePassport(true);
			community = new Community(makeBaseProps(), passport);
		});
	...
	Scenario('Changing the white label domain without permission', ({ Given, When, Then }) => {
		...
		Then('a PermissionError should be thrown', () => {
			expect(updatingWhiteLabelDomainWithoutPermission).toThrow(PermissionError);
			expect(updatingWhiteLabelDomainWithoutPermission).throws('You do not have permission to change the white label domain of this community');
		});
	});
	...
	Scenario('Changing the handle without permission', ({ Given, When, Then }) => {
		...
		Then('a PermissionError should be thrown', () => {
			expect(updatingHandleWithoutPermission).toThrow(PermissionError);
