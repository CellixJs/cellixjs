...
import { type ViolationTicketV1FinanceDetailEntityReference, type ViolationTicketV1FinanceDetailProps, ViolationTicketV1FinanceDetails } from './violation-ticket-v1-finance-details.ts';
...
import { ViolationTicketV1RevisionRequest, type ViolationTicketV1RevisionRequestEntityReference, type ViolationTicketV1RevisionRequestProps } from './violation-ticket-v1-revision-request.ts';
 ...
 */
export interface ViolationTicketV1EntityReference extends Readonly<Omit<ViolationTicketV1Props, 'activityLog' | 'messages' | 'photos' | 'financeDetails' | 'revisionRequest'>> {
 ...
 */
export class ViolationTicketV1<props extends ViolationTicketV1Props> extends AggregateRoot<props, Passport> implements ViolationTicketV1EntityReference {
	//#region Fields
	...
	public requestAddMessage(message: string, sentBy: string, embedding: string, initiatedBy?: MemberEntityReference): void {
		if (
			!this.visa.determineIf(
				(permissions) => permissions.isSystemAccount || (permissions.canCreateTickets && permissions.isEditingOwnTicket) || (permissions.canWorkOnTickets && permissions.isEditingAssignedTicket) || permissions.canManageTickets,
			)
		) {
