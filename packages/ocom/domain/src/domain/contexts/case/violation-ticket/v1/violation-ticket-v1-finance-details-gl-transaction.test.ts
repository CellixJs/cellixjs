...
import { ViolationTicketV1FinanceDetailsGlTransaction, type ViolationTicketV1FinanceDetailsGlTransactionProps } from './violation-ticket-v1-finance-details-gl-transaction.ts';

const test = { for: describeFeature };
const __dirname = path.dirname(fileURLToPath(import.meta.url));
const feature = await loadFeature(
  path.resolve(__dirname, 'features/violation-ticket-v1-finance-details-gl-transaction.feature'),
);
...
test.for(feature, ({ Scenario, BeforeEachScenario }) => {
  ...
  Scenario('Creating a new ViolationTicketV1FinanceDetailsGlTransaction instance', ({ When, Then, And }) => {
  ...
  Scenario('Setting amount', ({ Given, When, Then }) => {
    ...
    When('I set the amount to 250.50', () => {
      glTransaction.amount = 250.50;
    });

    Then('the amount should be 250.50', () => {
      expect(glTransaction.amount).toBe(250.50);
    });
  });
