...
const feature = await loadFeature(path.resolve(__dirname, 'features/violation-ticket-v1-activity-detail.feature'));
...
test.for(feature, ({ Scenario, BeforeEachScenario }) => {
	...
	Scenario('Creating a new ViolationTicketV1ActivityDetail instance', ({ When, Then, And }) => {
		...
		And('the activity type should be set correctly', () => {
			expect(activityDetail.activityType).toBe(ValueObjects.ActivityTypeCodes.Created);
		});
	...
	Scenario('Setting activity type with proper permissions', ({ When, Then, And }) => {
		...
		Then('the activity type should be updated', () => {
			expect(activityDetail.activityType).toBe(ValueObjects.ActivityTypeCodes.Updated);
		});
	});

	Scenario('Setting activity type without permissions', ({ When, Then, And }) => {
		When('I have a ViolationTicketV1ActivityDetail instance', () => {
			activityDetail = new ViolationTicketV1ActivityDetail(props, visa);
		});
	...
	Scenario('Setting activity description with proper permissions', ({ When, Then, And }) => {
		When('I have a ViolationTicketV1ActivityDetail instance', () => {
			activityDetail = new ViolationTicketV1ActivityDetail(props, visa);
		});
	...
	Scenario('Setting activity description without permissions', ({ When, Then, And }) => {
		When('I have a ViolationTicketV1ActivityDetail instance', () => {
			activityDetail = new ViolationTicketV1ActivityDetail(props, visa);
		});
