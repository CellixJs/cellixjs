...
import { ViolationTicketV1FinanceDetailsAdhocTransactionsApproval, type ViolationTicketV1FinanceDetailsAdhocTransactionsApprovalProps } from './violation-ticket-v1-finance-details-adhoc-transactions-approval.ts';

const test = { for: describeFeature };
const __dirname = path.dirname(fileURLToPath(import.meta.url));
const feature = await loadFeature(path.resolve(__dirname, 'features/violation-ticket-v1-finance-details-adhoc-transactions-approval.feature'));
...
test.for(feature, ({ Scenario, BeforeEachScenario }) => {
	...
	Scenario('Creating a new ViolationTicketV1FinanceDetailsAdhocTransactionsApproval instance', ({ When, Then, And }) => {
		When('I create a new ViolationTicketV1FinanceDetailsAdhocTransactionsApproval with valid properties', () => {
			approval = new ViolationTicketV1FinanceDetailsAdhocTransactionsApproval(props);
		});
	...
	Scenario('Setting is applicant approval required', ({ Given, When, Then }) => {
