...
import { ViolationTicketV1FinanceDetails, type ViolationTicketV1FinanceDetailProps } from './violation-ticket-v1-finance-details.ts';
...
const feature = await loadFeature(
  path.resolve(__dirname, 'features/violation-ticket-v1-finance-details.feature'),
);
...
test.for(feature, ({ Scenario, BeforeEachScenario }) => {
  ...
  BeforeEachScenario(() => {
    transactions = { id: 'transactions-123' } as unknown as ViolationTicketV1FinanceDetailsTransactionsEntityReference;
    revenueRecognition = { id: 'revenue-recognition-123' } as unknown as ViolationTicketV1FinanceDetailsRevenueRecognitionEntityReference;
  ...
  Scenario('Creating a new ViolationTicketV1FinanceDetails instance', ({ When, Then, And }) => {
    ...
    And('the service fee should be set correctly', () => {
      expect(financeDetails.serviceFee).toBe(100.50);
    });
