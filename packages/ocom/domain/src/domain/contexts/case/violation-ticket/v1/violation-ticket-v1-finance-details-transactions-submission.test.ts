...
import { ViolationTicketV1FinanceDetailsTransactionsSubmission, type ViolationTicketV1FinanceDetailsTransactionsSubmissionProps } from './violation-ticket-v1-finance-details-transactions-submission.ts';
...
const feature = await loadFeature(
  path.resolve(__dirname, 'features/violation-ticket-v1-finance-details-transactions-submission.feature'),
);
...
test.for(feature, ({ Scenario, BeforeEachScenario }) => {
  ...
  Scenario('Creating a new ViolationTicketV1FinanceDetailsTransactionsSubmission instance', ({ When, Then, And }) => {
    When('I create a new ViolationTicketV1FinanceDetailsTransactionsSubmission with valid properties', () => {
      submission = new ViolationTicketV1FinanceDetailsTransactionsSubmission(props);
    });

    Then('the amount should be accessible', () => {
      expect(submission.amount).toBe(100.50);
    });
