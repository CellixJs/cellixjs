...
import { ServiceTicketV1ActivityDetail, type ServiceTicketV1ActivityDetailProps } from './service-ticket-v1-activity-detail.entity.ts';
import * as ValueObjects from './service-ticket-v1-activity-detail.value-objects.ts';

const test = { for: describeFeature };
const __dirname = path.dirname(fileURLToPath(import.meta.url));
const feature = await loadFeature(path.resolve(__dirname, 'features/service-ticket-v1-activity-detail.entity.feature'));
...
test.for(feature, ({ Scenario, BeforeEachScenario }) => {
	...
	Scenario('Creating a new ServiceTicketV1ActivityDetail instance', ({ When, Then, And }) => {
		When('I create a new ServiceTicketV1ActivityDetail with valid properties', () => {
			activityDetail = ServiceTicketV1ActivityDetail.getNewInstance(props, memberRef);
		});
	...
	Scenario('Setting activityType', ({ When, Then, And }) => {
		...
		And('I set the activityType to "UPDATED"', () => {
			activityDetail.activityType = new ValueObjects.ActivityTypeCode('UPDATED');
		});
