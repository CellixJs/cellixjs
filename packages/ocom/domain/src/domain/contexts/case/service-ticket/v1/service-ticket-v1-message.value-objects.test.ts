...
const feature = await loadFeature(path.resolve(__dirname, 'features/service-ticket-v1-message.value-objects.feature'));
...
test.for(feature, ({ Scenario }) => {
	...
	Scenario('Creating a message with minimum allowed length', ({ When, Then }) => {
		let value: string;
	...
	Scenario('Creating a message with less than minimum allowed length', ({ When, Then }) => {
	...
	// SentBy
	Scenario('Creating a sentBy with valid value "internal"', ({ When, Then }) => {
		let value: string;
	...
	Scenario('Creating a sentBy with valid value "external"', ({ When, Then }) => {
		let value: string;
	...
	Scenario('Creating a sentBy with invalid value', ({ When, Then }) => {
		...
		Then('an error should be thrown indicating the sentBy is invalid', () => {
			expect(createSentByWithInvalidValue).toThrow('SentBy must be one of: internal, external');
		});
	});
