...
export class Service<props extends ServiceProps> extends AggregateRoot<props, Passport> implements ServiceEntityReference {
	...
	public static getNewInstance<props extends ServiceProps>(newProps: props, serviceName: string, description: string, community: CommunityEntityReference, passport: Passport): Service<props> {
	...
	set serviceName(serviceName: string) {
		if (!this.visa.determineIf((permissions) => permissions.canManageServices)) {
			throw new PermissionError('You do not have permission to change the service name');
		}
	...
	set description(description: string) {
		if (!this.visa.determineIf((permissions) => permissions.canManageServices)) {
			throw new PermissionError('You do not have permission to change the service description');
		}
	...
	set isActive(isActive: boolean) {
		if (!this.visa.determineIf((permissions) => permissions.canManageServices)) {
			throw new PermissionError('You do not have permission to change the service status');
		}
