...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	Background(({ Given, And }) => {
		...
		And(
			'base service properties with serviceName "Test Service", description "A test service", isActive true, a valid community, and valid timestamps',
			() => {
				baseProps = makeBaseProps();
				service = new Service(baseProps, passport);
			},
	...
	Scenario('Creating a new service instance', ({ When, Then, And }) => {
		When(
			'I create a new Service aggregate using getNewInstance with serviceName "New Service", description "A new service", and a CommunityEntityReference',
			() => {
		...
		And(
			"the service's community should reference the provided CommunityEntityReference",
			() => {
	...
	Scenario(
		'Changing the serviceName with permission to manage services',
		({ Given, When, Then }) => {
	...
	Scenario(
		'Changing the serviceName without permission',
		({ Given, When, Then }) => {
			...
			Then('a PermissionError should be thrown', () => {
				expect(changeServiceNameWithoutPermission).toThrow(
	...
	Scenario(
		'Changing the serviceName to an invalid value',
		({ Given, When, Then }) => {
			...
			When(
				'I try to set the serviceName to an invalid value (e.g., null or empty string)',
				() => {
	...
	Scenario(
		'Changing the description with permission to manage services',
		({ Given, When, Then }) => {
			Given('a Service aggregate with permission to manage services', () => {
				passport = makePassport({ canManageServices: true });
				service = new Service(makeBaseProps(), passport);
	...
	Scenario(
		...
		({ Given, When, Then }) => {
			...
			Then('a PermissionError should be thrown', () => {
				expect(changeDescriptionWithoutPermission).toThrow(
					DomainSeedwork.PermissionError,
				);
				expect(changeDescriptionWithoutPermission).toThrow(
	...
	Scenario(
		'Changing the description to an invalid value',
		({ Given, When, Then }) => {
			...
			When(
				'I try to set the description to an invalid value (e.g., null or empty string)',
				() => {
	...
	Scenario(
		'Changing isActive with permission to manage services',
		({ Given, When, Then }) => {
			Given('a Service aggregate with permission to manage services', () => {
				passport = makePassport({ canManageServices: true });
				service = new Service(makeBaseProps(), passport);
	...
	Scenario('Changing isActive without permission', ({ Given, When, Then }) => {
		...
		Then('a PermissionError should be thrown', () => {
			expect(changeIsActiveWithoutPermission).toThrow(
				DomainSeedwork.PermissionError,
			);
			expect(changeIsActiveWithoutPermission).toThrow(
