...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	Background(({ Given, And }) => {
		...
		And(
			'a valid domainDataSource with community, role, and member repositories',
			() => {
				// Already created in BeforeEachScenario
			},
		);
		...
		And(
			'a valid community with id "community-123" and createdBy user "user-123"',
			() => {
				// Already created in BeforeEachScenario
			},
		);
	...
	Scenario(
		'Successfully provisioning member and default role',
		({ Given, When, Then, And }) => {
			...
			And(
				'the community has a valid createdBy user with displayName "John Doe"',
				() => {
			...
			And(
				'the user has personal information with firstName "John" and lastName "Doe"',
				() => {
					// Values are already set in makeMockCommunity
				},
			);
			...
			When(
				'I call provisionMemberAndDefaultRole with communityId "community-123"',
				async () => {
					try {
						await service.provisionMemberAndDefaultRole(
							'community-123',
			...
			Then('a default admin role should be created for the community', () => {
				expect(mockRoleRepo.getNewInstance).toHaveBeenCalledWith(
					'admin',
					true,
					mockCommunity,
				);
			...
			And(
				'the role permissions should be set to default admin permissions',
				() => {
					expect(
						(
						...
						).setDefaultAdminPermissions,
					).toHaveBeenCalled();
				},
			);

			And('a new member should be created with the createdBy user', () => {
				expect(mockMemberRepo.getNewInstance).toHaveBeenCalledWith(
					'John Doe',
					mockCommunity,
				);
	...
	Scenario(
		'Failing to provision when community not found',
		({ Given, When, Then }) => {
			...
			When(
				'I try to call provisionMemberAndDefaultRole with communityId "nonexistent-community"',
				async () => {
					try {
						await service.provisionMemberAndDefaultRole(
	...
	Scenario(
		'Failing to provision when role creation fails',
		({ Given, And, When, Then }) => {
			Given('a community with id "community-123" exists', () => {
				mockCommunityRepo.getByIdWithCreatedBy.mockResolvedValue(mockCommunity);
				mockRoleRepo.getNewInstance.mockResolvedValue(mockRole);
			...
			When(
				'I try to call provisionMemberAndDefaultRole with communityId "community-123"',
				async () => {
					try {
						await service.provisionMemberAndDefaultRole(
							'community-123',
			...
			Then(
				'a "Failed to provision default role for Community ID community-123" error should be thrown',
				() => {
					expect(thrownError).not.toBeNull();
					expect(thrownError?.message).toBe(
						'Failed to provision default role for Community ID community-123',
					);
				},
			);
		},
	...
	Scenario(
		'Failing to provision when community has no createdBy user',
		({ Given, And, When, Then }) => {
			Given('a community with id "community-123" exists', () => {
			...
			Then(
				'a "CreatedBy ID is required to provision member and default role for Community ID community-123" error should be thrown',
				() => {
					expect(thrownError).not.toBeNull();
					...
					expect(thrownError?.message).toMatch(
						/Cannot read properties of undefined|CreatedBy ID is required/,
					);
				},
			);
		},
	...
	Scenario(
		'Provisioning with missing user identity details',
		({ Given, When, Then, And }) => {
			Given('a community with id "community-123" exists', () => {
				mockCommunityRepo.getByIdWithCreatedBy.mockResolvedValue(mockCommunity);
				mockRoleRepo.getNewInstance.mockResolvedValue(mockRole);
