...
const feature = await loadFeature(path.resolve(__dirname, 'features/community-provisioning.service.feature'));
...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	...
	BeforeEachScenario(() => {
		...
		mockDomainDataSource.Community.Community.CommunityUnitOfWork.withScopedTransaction = vi.fn((callback) => callback(mockCommunityRepo));
		mockDomainDataSource.Community.Role.EndUserRole.EndUserRoleUnitOfWork.withTransaction = vi.fn((_, callback) => callback(mockRoleRepo));
		mockDomainDataSource.Community.Member.MemberUnitOfWork.withTransaction = vi.fn((_, callback) => callback(mockMemberRepo));
