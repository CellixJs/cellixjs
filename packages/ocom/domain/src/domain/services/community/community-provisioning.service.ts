...
export class CommunityProvisioningService {
    async provisionMemberAndDefaultRole(
        ...
        await domainDataSource.Community.CommunityUnitOfWork.withScopedTransaction(async (repo) => {
        ...
        await domainDataSource.Community.Role.EndUserRoleUnitOfWork.withTransaction(systemPassportForEndUserRole, async (repo) => {
            const newRole = await repo.getNewInstance('admin', true, communityDo as CommunityEntityReference);
        ...
        await domainDataSource.Community.Member.MemberUnitOfWork.withTransaction(systemPassportForMember, async (repo) => {
            const newMember = await repo.getNewInstance(createdBy.displayName, communityDo as CommunityEntityReference);
