...
export class CreateCommunity extends Interaction {
	...
	performAs(actor: UsesAbilities & AnswersQuestions): Promise<void> {
		...
		try {
			const community = Community.getNewInstance(this.communityData, this.communityName, this.createdBy, passport);
			...
			CommunityCreationResults.creationError = error instanceof Error ? error : new Error(`Unknown error: ${String(error)}`);
