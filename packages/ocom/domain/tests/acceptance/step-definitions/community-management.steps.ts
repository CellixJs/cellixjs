...
Given('I am an authorized user with community management permissions', function (this: SerenityCommunityWorld) {
...
When('I try to create a community with a name longer than {int} characters', async function (this: SerenityCommunityWorld, maxLength: number) {
	...
	await this.actor.attemptsTo(CommunityManagement.createCommunityWithInvalidName(maxLength + 1));
...
Then('I should receive an error message containing {string}', async function (this: SerenityCommunityWorld, expectedMessage: string) {
	// Use Screenplay Questions to verify the error message
	const errorContainsMessage = await CommunityState.errorMessageContains(expectedMessage).answeredBy(this.actor);

	assert.ok(errorContainsMessage, `Error message should contain "${expectedMessage}". Actual message: "${CommunityCreationResults.creationError?.message}"`);
...
Given('I am a {string} with {string} permissions', function (this: SerenityCommunityWorld, userRole: string, permissionLevel: string) {
...
When('I attempt to create a community named {string}', async function (this: SerenityCommunityWorld, communityName: string) {
	this.communityName = communityName;

	console.log(`➤ Attempting to create community named: "${communityName}"`);

	// Use Screenplay pattern to perform the action
	await this.actor.attemptsTo(CommunityManagement.createCommunityNamed(communityName));
...
Given('I am an authorized system administrator', function (this: SerenityCommunityWorld) {
...
Given('I have access to community management functions', function (this: SerenityCommunityWorld) {
...
Given('I am preparing to create a new community', function (this: SerenityCommunityWorld) {
...
Given('I am a regular user without administrative privileges', function (this: SerenityCommunityWorld) {
...
When('I attempt to create a community with a name exceeding {int} characters', async function (this: SerenityCommunityWorld, maxLength: number) {
	...
	await this.actor.attemptsTo(CommunityManagement.createCommunityWithInvalidName(excessiveName.length));

	this.syncResultsFromScreenplay();
});

When('I create a community named {string}', async function (this: SerenityCommunityWorld, communityName: string) {
	this.communityName = communityName;
...
When('I create a community with a name of exactly {int} characters', async function (this: SerenityCommunityWorld, exactLength: number) {
	...
	await this.actor.attemptsTo(CommunityManagement.createCommunityNamed(exactLengthName));

	this.syncResultsFromScreenplay();
});

When('I attempt to create a community with a name of only {int} characters', async function (this: SerenityCommunityWorld, shortLength: number) {
	...
	await this.actor.attemptsTo(CommunityManagement.createCommunityNamed(shortName));

	this.syncResultsFromScreenplay();
});

Then('the system should reject the community creation', async function (this: SerenityCommunityWorld) {
	const creationFailed = await CommunityState.creationFailed().answeredBy(this.actor);
	const wasNotCreated = !(await CommunityState.wasCreated().answeredBy(this.actor));
...
Then('I should be informed that the name is {string}', async function (this: SerenityCommunityWorld, expectedMessage: string) {
	const errorContainsMessage = await CommunityState.errorMessageContains(expectedMessage).answeredBy(this.actor);

	assert.ok(errorContainsMessage, `User should be informed that the name is "${expectedMessage}"`);

	console.log(`✓ User informed: name is "${expectedMessage}"`);
});

Then('no community should be persisted in the system', async function (this: SerenityCommunityWorld) {
	const wasNotCreated = !(await CommunityState.wasCreated().answeredBy(this.actor));
...
Then('I should see confirmation of the new community', async function (this: SerenityCommunityWorld) {
	const wasCreated = await CommunityState.wasCreated().answeredBy(this.actor);
...
Then('the community should be available in the system', async function (this: SerenityCommunityWorld) {
	const wasCreated = await CommunityState.wasCreated().answeredBy(this.actor);
...
Then('the full name should be preserved', function (this: SerenityCommunityWorld) {
	this.syncResultsFromScreenplay();
...
Then('I should be informed that the name is too short', async function (this: SerenityCommunityWorld) {
	const errorContainsShort = (await CommunityState.errorMessageContains('short').answeredBy(this.actor)) || (await CommunityState.errorMessageContains('minimum').answeredBy(this.actor));
...
Then('the system should deny access', async function (this: SerenityCommunityWorld) {
	const creationFailed = await CommunityState.creationFailed().answeredBy(this.actor);
...
Then('I should receive an authorization error', async function (this: SerenityCommunityWorld) {
	const hasAuthError =
		(await CommunityState.errorMessageContains('authorization').answeredBy(this.actor)) ||
		(await CommunityState.errorMessageContains('permission').answeredBy(this.actor)) ||
		(await CommunityState.errorMessageContains('access').answeredBy(this.actor));
...
Then('no community should be created', async function (this: SerenityCommunityWorld) {
	const wasNotCreated = !(await CommunityState.wasCreated().answeredBy(this.actor));
