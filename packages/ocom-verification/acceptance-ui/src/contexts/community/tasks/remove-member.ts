...
export const RemoveMember = (memberName: string): Task =>
	Task.where(
		'#actor removes the member',
		new TaskStep<Actor>('#actor clicks Remove for the member', async (actor) => {
			const page: AcceptanceUiMemberListPage = new MemberListPage(new DomPageAdapter(RenderInDom.as(actor).container));
			await page.clickRemoveMember(memberName);
