...
export const MemberListedInCommunity = (memberName: string, route?: { communityId: string; principalMemberId: string }) =>
	Question.about(`whether member "${memberName}" appears in the browser member list`, async (serenityActor) => {
		...
		if (route) {
			await page.goto(`/community/${encodeURIComponent(route.communityId)}/admin/${encodeURIComponent(route.principalMemberId)}/members`, { waitUntil: 'networkidle' });
		}
