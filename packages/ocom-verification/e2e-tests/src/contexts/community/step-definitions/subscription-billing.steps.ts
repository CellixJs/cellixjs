...
Given('subscription plans are available', () => {
	// Plan catalogs are provided by Task 2 seed data; e2e scenarios assume they exist.
});
...
When('{word} attempts to process a subscription charge', async (actorName: string) => {
	...
	try {
		...
		await actor.attemptsTo(notes<CommunityE2ENotes>().set('lastBillingError', errorMessageOf(error)));
	}
});
