...
async function buildTokenResponse(profile: TokenProfile, privateKey: KeyLike | Uint8Array, jwk: { alg?: string; kid?: string }, existingRefreshToken?: string) {
...
async function main() {
	...
	app.post('/token', async (req, res) => {
		...
		if (!email) {
			throw new Error('Email is not defined in .env.local');
		}
		// biome-ignore lint:useLiteralKeys
		const given_name = process.env['Given_Name'];
		if (!given_name) {
			throw new Error('Given_Name is not defined in .env.local');
		}
		// biome-ignore lint:useLiteralKeys
		const family_name = process.env['Family_Name'];
		if (!family_name) {
			throw new Error('Family_Name is not defined in .env.local');
		}
		// biome-ignore lint:useLiteralKeys
