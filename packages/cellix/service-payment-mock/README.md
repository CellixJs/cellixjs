...
An in-memory `PaymentService` implementation for local development and tests. It supports stored payment instruments, transactions, recurring-payment plans, and recurring payments without accepting or retaining vendor-specific payment details.
...
`ServicePaymentMock` exposes the mock-only `ServicePaymentMockFailure` type for tests that need a provider-style failure. Pass one of `'declined'`, `'processing-error'`, `'provider-unavailable'`, or `'invalid-payment-instrument'` as the optional second argument to `chargePayment`, `refundPayment`, or `createRecurringPayment`. Pass `'token-invalid'` or `'token-expired'` to `createPaymentInstrument`:
...
Only an explicit second argument simulates a failure. A failed instrument registration returns the mock-only `ServicePaymentMockRegistrationFailure`; failed transactions return the normal vendor-neutral payment result with `isSuccess: false` and populated error fields, and recurring payments return `status: 'failed'`. It does not return raw vendor response data. Malformed requests and unavailable mock resources continue to reject.

Recurring-payment plans store the current amount. Create a recurring payment with either a stored plan reference or inline terms, update its stored instrument or amount while active, and cancel it when future billing must stop.
