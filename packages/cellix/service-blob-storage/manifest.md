...
- Public exports are limited to two service classes, Blob-backed feature-flag document contracts, and framework-level request and response contracts needed by consumers and adapters
...
- `ServiceBlobStorage.getFeatureFlags` reads and validates the optional configured feature-flag document using the base service lifecycle
...
- This package owns the shared Blob-backed feature-flag document format and base-service feature-flag access, but not application-specific flag names, values, or fallback configuration
